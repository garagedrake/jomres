...
// Project pups
header('X-Clacks-Overhead-Castor-ProjectPups: GNU Tosie, Bonnie, Penny, Lily, Sheba, Jason, Sally, Lena');
...
try {
	...
	if (empty($thisJRUser->authorisedProperties) && $thisJRUser->userIsManager) {
		...
		$castor_users->delete_user( $thisJRUser->id );
		$thisJRUser->reset_manager_to_non_manager();
	}

	//castor timezones - mostly unused with an exception
	jr_import('castor_timezones');
	$tz = new castor_timezones();
	...
	//temp booking handler object, init castor session
	...
	$popup = (int)castorGetParam($_REQUEST, 'popup', 0);
	$no_html = (int)castorGetParam($_REQUEST, 'no_html', 0);
	$task = str_replace('&#60;x&#62;', '', castorGetParam($_REQUEST, 'task', ''));
	...
	if ($no_html == 1) {
		define('CASTOR_NOHTML', 1);
	} else {
		define('CASTOR_NOHTML', 0);
	}
	...
	if (get_showtime('task') != 'error') {
		...
		if (!$thisJRUser->userIsManager && $thisJRUser->userIsRegistered && !AJAXCALL && !preg_match('/bot|crawl|curl|dataprovider|search|get|spider|find|java|majesticsEO|google|yahoo|teoma|contaxe|yandex|libwww-perl|facebookexternalhit/i', $_SERVER['HTTP_USER_AGENT']) && get_showtime('task') != 'background_process' ) {
			...
			if (get_showtime('task') != 'handlereq' && get_showtime('task') != 'completebk' && get_showtime('task') != 'processpayment' && get_showtime('task') != 'confirmbooking') {
				if ($thisJRUser->profile_id == 0) {
					$query = "SELECT guests_uid,enc_firstname,enc_surname,enc_house,enc_street,enc_town,enc_postcode,enc_county,enc_country,enc_tel_landline,enc_tel_mobile,enc_email,discount FROM #__castor_guests WHERE mos_userid = '".(int) $thisJRUser->id."' LIMIT 1";
					$guestData = doSelectSql($query, 2);

					if ( isset($guestData[ 'id' ]) ) {
						if ($guestData) {
							//$query = "INSERT INTO #__castor_guest_profile (`cms_user_id`,`enc_firstname`,`enc_surname`,`enc_house`,`enc_street`,`enc_town`,`enc_county`,`enc_country`,`enc_postcode`,`enc_tel_landline`,`enc_tel_mobile`,`enc_email`) VALUES ('".(int) $thisJRUser->id."','$firstname','$surname','$house','$street','$town','$region','$country','$postcode','$landline','$mobile','$email')";
							//doInsertSql($query, '');

							$tmpBookingHandler->updateGuestField('guests_uid', $guestData[ 'id' ]);
							$tmpBookingHandler->updateGuestField('firstname', $castor_encryption->decrypt($guestData[ 'enc_firstname' ]));
							$tmpBookingHandler->updateGuestField('surname', $castor_encryption->decrypt($guestData[ 'enc_surname' ]));
							$tmpBookingHandler->updateGuestField('house', $castor_encryption->decrypt($guestData[ 'enc_house' ]));
							$tmpBookingHandler->updateGuestField('street', $castor_encryption->decrypt($guestData[ 'enc_street' ]));
							$tmpBookingHandler->updateGuestField('town', $castor_encryption->decrypt($guestData[ 'enc_town' ]));
							$tmpBookingHandler->updateGuestField('region', $castor_encryption->decrypt($guestData[ 'enc_county' ]));
							$tmpBookingHandler->updateGuestField('country', $castor_encryption->decrypt($guestData[ 'enc_country' ]));
							$tmpBookingHandler->updateGuestField('postcode', $castor_encryption->decrypt($guestData[ 'enc_postcode' ]));
							$tmpBookingHandler->updateGuestField('tel_landline', $castor_encryption->decrypt($guestData[ 'enc_tel_landline' ]));
							$tmpBookingHandler->updateGuestField('tel_mobile', $castor_encryption->decrypt($guestData[ 'enc_tel_mobile' ]));
							$tmpBookingHandler->updateGuestField('email', $castor_encryption->decrypt($guestData[ 'enc_email' ]));
						} else {
		...
		$thisJRUser->castor_manager_id = 0;
		$thisJRUser->userid = 0;
		$thisJRUser->username = '';
	...
	if ($thisJRUser->userIsSuspended) {
		...
		$castor_suspensions->set_manager_id($thisJRUser->userid);
		if ($castor_suspensions->suspended_manager_denied_task(get_showtime('task'))) {
			castorRedirect(castorURL(CASTOR_SITEPAGE_URL.'&task=cpanel'), '');
		}
	}
	...
	if ($property_uid > 0) {
		...
		if (!$thisJRUser->userIsManager && $current_property_details->published == 0 && get_showtime('task') != '') {
			if (!AJAXCALL && $no_html == 0) {
				...
				$warning = new castor_sanity_check(false);
				echo $warning->construct_warning(jr_gettext('_CASTOR_PROPERTYNOTOUBLISHED', '_CASTOR_PROPERTYNOTOUBLISHED', false));
				$property_uid = 0;
				$task = '';
				set_showtime('task', '');
	...
	if (!AJAXCALL && $no_html == 0) {
		...
		//core menu items
		// Older functionality, kept for backwards compatibility. Bootstrap 5 templates now use {castor_script show_main_menu} to display the menu
	...
	//register castor frontend widgets
	...
	if (!AJAXCALL && $no_html == 0) {
		...
		if ( isset($jrConfig['platform_connected']) && $jrConfig['platform_connected'] == 1 ) {
			if (!file_exists(CASTOR_COREPLUGINS_ABSPATH.'connect'.JRDS.'plugin_info.php') && file_exists(CASTOR_COREPLUGINS_ABSPATH.'plugin_manager'.JRDS.'plugin_info.php') ) { // Something has gone horribly wrong
	...
	$castor_language_definitions->reset_lang_and_property_type();
