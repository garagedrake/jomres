...
// ################################################################
	 ...
	 *
	 * Creates pathways  that can be built in Joomla's pathway functionality.
	 *
	 */
	...
	public function __construct($componentArgs)
	{
		...
		if ($popup == 0 && $numberOfPropertiesInSystem > 1) {
			...
			if ($property_uid > 0) {
				...
				$tasks[ 'property_type' ] = array('text' => $property_type, 'url' => castorURL(CASTOR_SITEPAGE_URL.'&task=search&ptype='.$current_property_details->ptype_id));
				$tasks[ 'country' ] = array('text' => $current_property_details->property_country, 'url' => CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&country='.$current_property_details->property_country_code);
				$tasks[ 'region' ] = array('text' => $current_property_details->property_region, 'url' => CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&region='.$current_property_details->property_region);
				$tasks[ 'town' ] = array('text' => $current_property_details->property_town, 'url' => CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&town='.$current_property_details->property_town);
			} else {
			...
			$tasks[ 'show_property_tariffs' ] = array('text' => jr_gettext('_CASTOR_COM_MR_LISTTARIFF_TITLE', '_CASTOR_COM_MR_LISTTARIFF_TITLE').' '.str_replace('&#39;', "'", getPropertyName($property_uid)), 'url' => CASTOR_SITEPAGE_URL.'&task=show_property_tariffs&property_uid='.$property_uid);
			$tasks[ 'dobooking' ] = array('text' => jr_gettext('_CASTOR_PATHWAY_BOOKINGFORM', '_CASTOR_PATHWAY_BOOKINGFORM'), 'url' => get_booking_url($property_uid));
			$tasks[ 'show_property_slideshow' ] = array('text' => jr_gettext('_CASTOR_FRONT_SLIDESHOW', '_CASTOR_FRONT_SLIDESHOW'), 'url' => CASTOR_SITEPAGE_URL.'&task=show_property_slideshow&property_uid='.$property_uid);
