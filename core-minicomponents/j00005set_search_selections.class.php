		...
		public function __construct()
		{
			...
			if (isset($_REQUEST[ 'sleeps_adults' ])) {
				if (is_array($_REQUEST[ 'sleeps_adults' ])) {
					$sleeps_adults = castorGetParam($_REQUEST, 'sleeps_adults', array());
				} else {
					$sleeps_adults = array(castorGetParam($_REQUEST, 'sleeps_adults', ''));
				}
			...
			if (isset($_REQUEST[ 'sleeps_children' ])) {
				if (is_array($_REQUEST[ 'sleeps_children' ])) {
					$sleeps_children = castorGetParam($_REQUEST, 'sleeps_children', array());
				} else {
					$sleeps_children = array(castorGetParam($_REQUEST, 'sleeps_children', ''));
				}
			...
			if (isset($_REQUEST[ 'stars' ])) {
				if (is_array($_REQUEST[ 'stars' ])) {
					$stars = castorGetParam($_REQUEST, 'stars', array());
				} else {
					$stars = array(castorGetParam($_REQUEST, 'stars', ''));
				}
			...
			if (isset($_REQUEST[ 'pricerange_value_from' ])) {
				$pricerange_value_from = (int) castorGetParam($_REQUEST, 'pricerange_value_from', 0);
				$pricerange_value_to = (int) castorGetParam($_REQUEST, 'pricerange_value_to', 0);

				$priceranges = castorGetParam($_REQUEST, 'priceranges', array());
			...
			if (isset($_REQUEST[ 'feature_uids' ])) {
				$feature_uids = castorGetParam($_REQUEST, 'feature_uids', array());
				$tmpBookingHandler->tmpsearch_data['ajax_search_composite_selections']['feature_uids'] = $feature_uids;
			}

			//////////////////////////////////// COUNTRIES /////////////////////////////////////////////////////////

			if (isset($_REQUEST[ 'autocomplete_value' ]) || isset($_REQUEST[ 'autocomplete_value' ])) {
			...
			if (isset($_REQUEST[ 'countries' ]) || isset($_REQUEST[ 'country' ])) {
				if (isset($_REQUEST[ 'countries' ])) {
					$countries = castorGetParam($_REQUEST, 'countries', array());
				} else {
					$countries = array(castorGetParam($_REQUEST, 'country', ''));
				}
			...
			if (isset($_REQUEST[ 'regions' ]) || isset($_REQUEST[ 'region' ])) {
				if (isset($_REQUEST[ 'regions' ])) {
					$regions = castorGetParam($_REQUEST, 'regions', array());
				} else {
					$regions = array(find_region_name(castorGetParam($_REQUEST, 'region', '')));
				}
			...
			if (isset($_REQUEST[ 'towns' ]) || isset($_REQUEST[ 'town' ])) {
				if (isset($_REQUEST[ 'towns' ])) {
					$towns = castorGetParam($_REQUEST, 'towns', array());
				} else {
					$towns = array(castorGetParam($_REQUEST, 'town', ''));
				}
			...
			if (isset($_REQUEST[ 'room_type_uids' ]) || isset($_REQUEST[ 'room_type' ])) {
				if (isset($_REQUEST[ 'room_type_uids' ])) {
					$room_type_uids = castorGetParam($_REQUEST, 'room_type_uids', array());
				} else {
					$room_type_uids = array(castorGetParam($_REQUEST, 'room_type', 0));
				}
			...
			if (isset($_REQUEST[ 'property_type_uids' ]) || isset($_REQUEST[ 'ptype' ])) {
				if (isset($_REQUEST[ 'room_type_uids' ])) {
					$property_type_uids = castorGetParam($_REQUEST, 'property_type_uids', array());
				} else {
					$property_type_uids = array(castorGetParam($_REQUEST, 'ptype', 0));
				}
			...
			if (isset($_REQUEST[ 'guestnumbers' ]) || isset($_REQUEST[ 'guestnumber' ])) {
				if (isset($_REQUEST[ 'guestnumbers' ])) {
					$guestnumbers = castorGetParam($_REQUEST, 'guestnumbers', array());
				} else {
					$guestnumbers = array(castorGetParam($_REQUEST, 'guestnumber', 0));
				}
			...
			if (isset($_REQUEST[ 'arrivalDate' ])) {
				...
				if ($arrivalDate != '') {
					...
					$arrivalDate = JSCalConvertInputDates($arrivalDate, true);
					$test1 = strtotime($arrivalDate);
					$test2 = date('Y-m-d H:i:s', strtotime('+2 years', strtotime($today)));;
					if ( $test1 > $test2 ) {
						$arrivalDate = $today;
					}
			...
			if (isset($_REQUEST[ 'departureDate' ])) {
				...
				$departureDate = JSCalConvertInputDates($departureDate, true);
