...
class j16000view_invoice
{
	...
	public function __construct($componentArgs)
	{
		...
		$query = 'SELECT enc_firstname,enc_surname,enc_house,enc_street,enc_town,enc_county,enc_country,enc_postcode,enc_tel_landline,enc_tel_mobile,enc_email,enc_vat_number FROM #__castor_guest_profile WHERE cms_user_id = '.(int) $invoice->cms_user_id.'';
		...
		if (!empty($managerData)) {
			foreach ($managerData as $data) {
				...
				$output[ 'VATNO' ] = $castor_encryption->decrypt($data->enc_vat_number);
			}
		}
		...
		if ($popup != 1) {
			...
			if ($invoice->status != 1 && $invoice->raised_date > '1970-01-01 00:00:01') {
				$jrtb .= $jrtbar->toolbarItem('save', castorUrl(CASTOR_SITEPAGE_URL_ADMIN.'&task=mark_invoice_paid&id='.$invoice->id), jr_gettext('_CASTOR_INVOICE_MARKASPAID', '_CASTOR_INVOICE_MARKASPAID', false));
			}
			//$jrtb .= $jrtbar->toolbarItem( 'printer', castorUrl(CASTOR_SITEPAGE_URL_ADMIN . '&tmpl=' .get_showtime("tmplcomponent") . '&popup=1&task=view_invoice&id=' . $invoice->id), jr_gettext( '_CASTOR_COM_INVOICE_PRINT', _CASTOR_COM_INVOICE_PRINT, false) );
			$jrtb .= $jrtbar->toolbarItem('cancel', castorURL(CASTOR_SITEPAGE_URL_ADMIN.'&task=list_invoices'), '');
		...
		$output[ 'HBUSINESSNAME' ] = jr_gettext('_CASTOR_COM_YOURBUSINESS_NAME', '_CASTOR_COM_YOURBUSINESS_NAME');
		...
		$output[ 'HVATNO' ] = jr_gettext('_CASTOR_COM_YOURBUSINESS_VATNO', '_CASTOR_COM_YOURBUSINESS_VATNO');
		$output[ 'HHOUSENO' ] = jr_gettext('_CASTOR_COM_YOURBUSINESSADDRESS', '_CASTOR_COM_YOURBUSINESSADDRESS');
		...
		if (isset($castor_media_centre_images->site_images['logo'])) {
