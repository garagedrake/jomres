...
class j16000edit_taxrate
{
	...
	public function __construct()
	{
		...
		$output[ 'IS_EU_COUNTRY' ] = castorHTML::selectList($yesno, 'is_eu_country', '', 'value', 'text', $selected);

		$jrtbar = castor_singleton_abstract::getInstance('castor_toolbar');
		$jrtb = $jrtbar->startTable();
		$jrtb .= $jrtbar->toolbarItem('cancel', CASTOR_SITEPAGE_URL_ADMIN.'&task=list_taxrates', '');
		$jrtb .= $jrtbar->toolbarItem('save', '', '', true, 'save_taxrate');

		if ($id > 0) {
			$jrtb .= $jrtbar->toolbarItem('delete', CASTOR_SITEPAGE_URL_ADMIN.'&task=delete_taxrate'.'&no_html=1&id='.$id, '');
		} else {
