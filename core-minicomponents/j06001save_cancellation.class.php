...
class j06001save_cancellation
{
	...
	public function __construct($componentArgs)
	{
		...
		if (isset($componentArgs['contract_uid'])) {
			...
			$reason = castorGetParam($_POST, 'reason', '');
		...
		if ($contract_uid != 0) {
			...
			if ($cancellationSuccessful === true) {
				...
				$query = "SELECT guest_uid,tag FROM #__castor_contracts WHERE contract_uid = '".(int) $contract_uid."' AND property_uid = '".(int) $defaultProperty."'";
				...
				if (!isset($componentArgs['contract_uid'])) {
					castorRedirect(castorURL(CASTOR_SITEPAGE_URL.'&task=list_bookings'), $saveMessage);
				}
			} else {
