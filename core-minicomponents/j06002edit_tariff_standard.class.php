...
class j06002edit_tariff_standard
{
	function __construct()
	{
		...
		$room_types_with_no_tariff = $jrportal_rates->get_unpopulated_room_type_ids($defaultProperty);
		// Here we are going to filter out room types that already have a tariff
		...
		if (isset($jrportal_rates->rates[$tarifftype_id]) && !empty($jrportal_rates->rates[$tarifftype_id])) {
			...
			$output['EXTRA_GUESTS_PRICE'] = $jrportal_rates->rates_defaults['extra_guests_price'];
		...
		$output['MAXDAYS_DROPDOWN'] = castorHTML::integerSelectList(0, $jrportal_rates->rates_defaults['maxdays'], 1, 'maxdays', '', $output['MAXDAYS']);
		$output['7DAY_MODIFIER_DROPDOWN'] = castorHTML::integerSelectList(0, 100, 1, 'modifier_7_days', '', $modifier_7_days);
		$output['30DAY_MODIFIER_DROPDOWN'] = castorHTML::integerSelectList(0, 100, 1, 'modifier_30_days', '', $modifier_30_days);
		...
		$output['PERCENTAGE_YESNO'] = castorHTML::selectList($yesno, 'modifier_is_percentage', '', 'value', 'text', $modifier_is_percentage);
		...
		foreach ($datesInyearsArray as $ykey => $y) {
			foreach ($y as $mkey => $m) {
				...
				foreach ($m as $dkey => $d) {
					...
					if ($dkey <= 15) {
						$days1.='<td class="setdate"> <span style="display:none" data-date="'.$d['date'].'"></span><div class="btn btn-sm btn-default">'.$d['dom'].'</div></td>';
							$inputs1.='
							<td>
						...
						$days2.='<td class="setdate"> <span style="display:none" data-date="'.$d['date'].'"></span><div class="btn btn-sm btn-default">'.$d['dom'].'</div></td>';
							$inputs2.='
							<td>
								<input type="number" class="'.$d['class'].'_rates input-mini form-control " style="" id="tariffinput_'.$d['epoch'].'" name="tariffinput['.$d['epoch'].']" value="'.$d['value'].'" />
								<input type="number" class="'.$d['class'].'_mindays input-mini form-control " style="color:red;" id="mindaysinput_'.$d['epoch'].'" name="mindaysinput['.$d['epoch'].']" value="'.$d['mindays'].'" />
		...
		$output['ROOMTYPE_HIDDEN'] ='';
		if ($output['TARIFFTYPEID'] > 0) {
			$output['ROOMTYPEDROPDOWN'] = $basic_property_details->this_property_room_classes[$roomclassid]["abbv"];
			$output['ROOMTYPE_HIDDEN'] = '<input type="hidden" name = "room_type_id" value="'.$roomclassid.'" />';
		}
		...
		$output['CASTOR_COM_A_DAILY_EXTRA_GUEST_PRICE']=jr_gettext('CASTOR_COM_A_DAILY_EXTRA_GUEST_PRICE', 'CASTOR_COM_A_DAILY_EXTRA_GUEST_PRICE', false);

		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT', 'CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT', false);
		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT_DESC']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT_DESC', 'CASTOR_COM_A_TARIFFMODE_STANDARD_PER_PERSON_PER_NIGHT_DESC', false);
		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER', 'CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER', false);
		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER_DESC']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER_DESC', 'CASTOR_COM_A_TARIFFMODE_STANDARD_7DAY_MODIFIER_DESC', false);
		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER', 'CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER', false);
		$output['CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER_DESC']=jr_gettext('CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER_DESC', 'CASTOR_COM_A_TARIFFMODE_STANDARD_30DAY_MODIFIER_DESC', false);
		$output['_CASTOR_VARIANCES_ISPERCENTAGE']=jr_gettext('_CASTOR_VARIANCES_ISPERCENTAGE', '_CASTOR_VARIANCES_ISPERCENTAGE', false);
		...
		$jrtb .= $jrtbar->toolbarItem('cancel', castorURL(CASTOR_SITEPAGE_URL."&task=list_tariffs_standard"), jr_gettext('_CASTOR_COM_A_CANCEL', '_CASTOR_COM_A_CANCEL', false));
		$jrtb .= $jrtbar->toolbarItem('save', castorURL(CASTOR_SITEPAGE_URL."&task=save_tariff_standard"), jr_gettext('_CASTOR_COM_MR_SAVE', '_CASTOR_COM_MR_SAVE', false), true, 'save_tariff_standard');
		...
		$tmpl->readTemplatesFromInput('edit_standard_tariff.html');
