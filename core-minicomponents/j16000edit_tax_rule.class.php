...
class j16000edit_tax_rule
{
	...
	public function __construct()
	{
		...
		if ($id > 0) {
			$query = 'SELECT `tax_rate_id`,`country_id`,`region_id` FROM #__castor_tax_rules WHERE id='.$id.' LIMIT 1';
		...
		$jrtb .= $jrtbar->toolbarItem('cancel', CASTOR_SITEPAGE_URL_ADMIN.'&task=list_tax_rules', '');
		$jrtb .= $jrtbar->toolbarItem('save', '', '', true, 'save_tax_rule');
		$jrtb .= $jrtbar->toolbarItem('delete', CASTOR_SITEPAGE_URL_ADMIN.'&task=delete_tax_rule'.'&no_html=1&id='.$id, '');
