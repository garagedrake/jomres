...
class j06002charts
{
	...
	public function __construct($componentArgs)
	{
		...
		$output['CHARTS_DROPDOWN'] = castorHTML::selectList($options, 'jr_chart', '', 'value', 'text', get_showtime('jr_chart'), false);
		...
		$output['CHART_TYPES_DROPDOWN'] = castorHTML::selectList($options, 'jr_chart_type', '', 'value', 'text', $chart_type, false);

		//get selected jr chart
		if (get_showtime('jr_chart') == '0') {
			$output['CHART'] = '';
		} else {
			if ($MiniComponents->eventSpecificlyExistsCheck('06002', get_showtime('jr_chart'))) {
