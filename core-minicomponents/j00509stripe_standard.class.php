...
class j00509stripe_standard {
	function __construct($componentArgs)
		...
		$query="SELECT value FROM #__castor_pluginsettings WHERE prid = '".(int)$defaultProperty."' AND plugin = '$plugin' AND setting = 'active' AND value = '1'";
		$activeList =doSelectSql($query);
		if (!empty($activeList))
			$active=jr_gettext('_CASTOR_COM_MR_YES','_CASTOR_COM_MR_YES',false);
		else
			$active=jr_gettext('_CASTOR_COM_MR_NO','_CASTOR_COM_MR_NO',false);
	...
	function touch_template_language()
		{
		$plugin="stripe";
		echo jr_gettext('_CASTOR_CUSTOMTEXT_GATEWAYNAME'.$plugin,ucwords($plugin));
		}
