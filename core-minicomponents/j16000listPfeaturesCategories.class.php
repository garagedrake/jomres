...
class j16000listPfeaturesCategories
{
	...
	public function __construct()
	{
		...
		$output['PAGETITLE'] = jr_gettext('_CASTOR_PROPERTYFEATURES_HCATEGORIES', '_CASTOR_PROPERTYFEATURES_HCATEGORIES', false);
		...
		foreach ($castor_property_features_categories->property_features_categories as $c) {
			...
			$toolbar->newToolbar();
			$toolbar->addItem('fa fa-pencil-square-o', 'btn btn-info', '', castorURL(CASTOR_SITEPAGE_URL_ADMIN.'&task=editPfeatureCategory&id='.$c['id']), jr_gettext('COMMON_EDIT', 'COMMON_EDIT', false));
			$toolbar->addSecondaryItem('fa fa-trash-o', '', '', castorURL(CASTOR_SITEPAGE_URL_ADMIN.'&task=deletePfeatureCategory&id='.$c['id']), jr_gettext('COMMON_DELETE', 'COMMON_DELETE', false));
