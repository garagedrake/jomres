...
class j03379room_types
{
	...
	public function __construct($componentArgs)
	{
		...
		if ($property_uid > 0 && $jrConfig[ 'frontend_room_type_editing_allowed' ] == "1") {
			...
			if (isset($keys[0])) {
				$first_index = $keys[0];
				$preview_link = CASTOR_SITEPAGE_URL_AJAX.'&task=show_property_room_type&property_uid='.$property_uid."&room_class_uid=".$first_index;
				...
				if ($mrConfig[ 'singleRoomProperty' ] != '1' && $mrConfig['is_real_estate_listing'] != '1' && !empty($current_property_details->room_types)) {
					$this->ret_vals = array(
										...
										'name' => jr_gettext('_CASTOR_COM_MR_VRCT_TAB_ROOMTYPES', '_CASTOR_COM_MR_VRCT_TAB_ROOMTYPES', false),
