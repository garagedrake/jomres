...
// ################################################################
	...
	class j06000media_centre_resources_ajax_existing_images
	{
		...
		public function __construct($componentArgs)
		{
			...
			if (!castor_cmsspecific_areweinadminarea()) {
				$delete_url = CASTOR_SITEPAGE_URL_AJAX.'&task=media_centre_handler&delete=1&resource_type='.$resource_type.'&resource_id='.$resource_id.'&file=';
			} else {
				$delete_url = CASTOR_SITEPAGE_URL_ADMIN_AJAX.'&task=media_centre_handler&delete=1&resource_type='.$resource_type.'&resource_id='.$resource_id.'&file=';
			}
			...
			if (!empty($images)) {
				foreach ($images as $image) {
					if (isset($image['small'])) {
						...
						$image_small_path = str_replace(CASTOR_IMAGELOCATION_RELPATH, '', $image['small']);
						...
						if (file_exists(CASTOR_IMAGELOCATION_ABSPATH.$image_small_path)) {
				...
				echo ' <img src="'.CASTOR_IMAGES_RELPATH.'noimage.svg'.'" width="100px" height="200px" />';
			}
		}
