...
/*		if (!$this->key_valid) {
			echo '<span class="alert alert-danger">'.jr_gettext('_CASTOR_SUPPORTKEY_DESC_INVALID', '_CASTOR_SUPPORTKEY_DESC_INVALID', false).'</span>';
			return;
		}*/

		if ((!file_exists(CASTOR_COREPLUGINS_ABSPATH.'plugin_manager'.JRDS.'plugin_info.php'))) { // We will need to install the plugin manager, plugin force a registry rebuild, then redirect to this page again.
			if (!isset($_REQUEST['install'])) {
				...
				if (substr(CASTORCONFIG_ABSOLUTE_PATH, -1) != '/') {
					$absolute_path = CASTORCONFIG_ABSOLUTE_PATH."/";
				} else {
					$absolute_path = CASTORCONFIG_ABSOLUTE_PATH."/";
				}

				$updateDirPath = $absolute_path.CASTOR_ROOT_DIRECTORY.JRDS.'updates'.JRDS.'plugin_manager'.JRDS;
				$newfilename = $updateDirPath.'plugin_manager.vnw';
				$destinationPath = CASTOR_COREPLUGINS_ABSPATH.'plugin_manager';
				...
				$query_string = 'index.php?r=gp&cms='._CASTOR_DETECTED_CMS.'&vnw=1&plugin=plugin_manager&castorver='. $jrConfig[ 'version' ].'&key='.$key_validation->key_hash.'&php_version='.$php_version;
				...
				try {
					...
					$castor_user_feedback->construct_message(array('message'=>"Could not download plugin manager", 'css_class'=>'alert-danger alert-error'));
				}
				...
				if ($res === true) {
					...
					$result = dirmv($updateDirPath.'unpacked', CASTOR_COREPLUGINS_ABSPATH.'plugin_manager', true, $funcloc = JRDS);
				}
				$zip->close();
				...
				castorRedirect(CASTOR_SITEPAGE_URL_ADMIN.'&task=showplugins');
			}
		}
	}
...
function createPath($path)
{
	...
	$prev_path = substr($path, 0, strrpos($path, JRDS, -2) + 1);
