...
class patTemplate_Compiler extends patTemplate
	...
	function displayParsedTemplate( $name = null )
		{
		if ( is_null( $name ) ) $name = $this->_root;
	...
	function _addToCode( $line, $indent = 0, $comment = null )
		...
		if ( !is_null( $comment ) )
			{
			fputs( $this->_fp, "\n" );
			if ( $indent > 0 ) fputs( $this->_fp, str_repeat( "\t", $indent ) );
	...
	function _getVar( $template, $varname )
		{
		if ( isset( $this->_templates[ $template ][ 'vars' ][ $varname ] ) ) return $this->_templates[ $template ][ 'vars' ][ $varname ];

		if ( isset( $this->_globals[ $this->_startTag . $varname . $this->_endTag ] ) ) return $this->_globals[ $this->_startTag . $varname . $this->_endTag ];
