...
class patTemplate_Reader_IT extends patTemplate_Reader
	...
	function parseString( $string )
		...
		if ( !isset( $this->_defaultAtts[ 'autoload' ] ) ) $this->_defaultAtts[ 'autoload' ] = 'on';
		 ...
		 */
		if ( $tokens[ 0 ] != '' ) $this->_characterData( $tokens[ 0 ] );
	...
	function readTemplates( $input )
		...
		if ( patErrorManager::isError( $fullPath ) ) return $fullPath;
		$content = $this->_getFileContents( $fullPath );
		if ( patErrorManager::isError( $content ) ) return $content;
