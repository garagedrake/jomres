...
class patTemplate_Dump_Html extends patTemplate_Dump
	...
	function dumpTemplates( $templates, $vars )
		...
		foreach ( $templates as $name => $tmpl )
			...
			if ( !empty( $tmpl[ 'dependencies' ] ) )
				{
				$dependencies = array ();
				foreach ( $tmpl[ 'dependencies' ] as $dependency ) array_push( $dependencies, '<a href="javascript:jump( \'' . $dependency . '\' );" class="patTemplate">' . $dependency . '</a>' );
	...
	function _displayVariables( $tmpl, $tmplName, $setVars, $vars )
		...
		foreach ( $setVars as $var )
			{
			if ( isset( $vars[ $tmplName ][ $var ] ) ) $value = $vars[ $tmplName ][ $var ];
			else
	...
	function _displayConditions( $tmpl, $tmplName, $vars )
		...
		foreach ( $tmpl[ 'subtemplates' ] as $cond => $spec )
			...
			if ( !empty( $spec[ 'dependencies' ] ) )
				{
				$dependencies = array ();
				foreach ( $spec[ 'dependencies' ] as $dependency ) array_push( $dependencies, '<a href="javascript:jump( \'' . $dependency . '\' );" class="patTemplate">' . $dependency . '</a>' );
	...
	function _displayAttributes( $tmpl, $tmplName = null )
		...
		{
			case 'modulo':
				if ( $tmpl[ 'attributes' ][ 'modulo' ] == 2 ) $type = 'OddEven';
				else
				$type = 'modulo (' . $tmpl[ 'attributes' ][ 'modulo' ] . ')';
			case 'condition':
				if ( !isset( $type ) ) $type = 'condition';
				...
				if ( isset( $tmpl[ 'attributes' ][ 'conditionvar' ] ) )
					{
					if ( isset( $tmpl[ 'attributes' ][ 'conditiontmpl' ] ) ) $this->_displayLine( 'ConditionVar', $tmpl[ 'attributes' ][ 'conditiontmpl' ] . '.' . $tmpl[ 'attributes' ][ 'conditionvar' ] );
					elseif ( isset( $tmpl[ 'attributes' ][ 'useglobals' ] ) && $tmpl[ 'attributes' ][ 'useglobals' ] == 'yes' ) $this->_displayLine( 'ConditionVar', '__globals.' . $tmpl[ 'attributes' ][ 'conditionvar' ] );
					else
		 ...
		 * external source
		 */
		if ( isset( $tmpl[ 'attributes' ][ 'src' ] ) ) $this->_displayLine( 'External Src', $tmpl[ 'attributes' ][ 'src' ] );
		/**
		 * varscope
		 */
	...
	function _getClassName( $class )
		{
		if ( !$this->_useBorders ) return $class;
