...
/**
	** Title | Report server url to Castor and number of published properties
	** Description | Intended for use by the superserver that will produce a list of properties with Castor installed, and the number of properties they offer
	** Plugin | core
	...
	** Success Response |
	** Error Response |
	** Sample call |castor/api/core/report
	** Notes |
*/
...
Flight::route('GET /core/report', function () {
	...
	$data[] = array (
		"api_url"				=> get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/api/',
		"castor_url" 			=> CASTOR_SITEPAGE_URL_NOSEF,
		"property_count"		=> count($castor_properties->all_property_uids['all_propertys'])
	);
...
/**
	...
	** Success Response |
	** Error Response |
	** Sample call |castor/api/core/get_properties
	** Notes |
*/
Flight::route('GET /core/get_properties', function () {
	...
	$current_property_details->gather_data_multi($castor_properties->all_property_uids['all_published_propertys']);
		
	$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
	$castor_media_centre_images->get_images_multi($castor_properties->all_property_uids['all_published_propertys'], array('property'));
	...
	if (count($current_property_details->multi_query_result) > 0) {
		foreach ($current_property_details->multi_query_result as $property) {
			if ($property['published'] == 1) {
				$castor_media_centre_images->get_images($property['propertys_uid'], array('property'));
				if (isset($castor_media_centre_images->images['property'][0][0]['large'])) {
					$thumbnail = $castor_media_centre_images->images['property'][0][0]['small'];
	...
	Flight::route('GET /core/endpoints', function()
	{
		...
		$core = get_directory_contents(CASTOR_LIBRARIES_ABSPATH.'castor'.JRDS.'api');
		$core_plugins = get_directory_contents(CASTOR_COREPLUGINS_ABSPATH);
		$remote_plugins = get_directory_contents(CASTOR_REMOTEPLUGINS_ABSPATH);

		$endpoint_plugin_directories = array();

		foreach ($core as $plugin_dir){
			if (strpos($plugin_dir , 'api_feature_' ) !== false ) {
				$endpoint_plugin_directories[] = CASTOR_COREPLUGINS_ABSPATH.$plugin_dir;
			}
		}
		...
		if ($remote_plugins != false) {
			foreach ($remote_plugins as $plugin_dir){
				if (strpos($plugin_dir , 'api_feature_' ) !== false ) {
					$endpoint_plugin_directories[] = CASTOR_REMOTEPLUGINS_ABSPATH.$plugin_dir;
				}
			}
		}
