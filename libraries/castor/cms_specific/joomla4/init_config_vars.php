...
require_once CASTORCONFIG_ABSOLUTE_PATH.JRDS.'configuration.php';
$CONFIG = new JConfig();
...
if (!defined('AUTO_UPGRADE')) {
	...
	if (class_exists('JURI')) {
		$castorConfig_live_site = @JURI::root();
	} else {
		...
		$castorConfig_live_site = 'http://'.implode('/', $_URI);
	}
} else {
	$detect_os = strtoupper($_SERVER[ 'SERVER_SOFTWARE' ]); // converted to uppercase
	$IIS = strpos($detect_os, 'IIS');
...
if ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || $_SERVER['SERVER_PORT'] == 443) {
	$castorConfig_live_site = str_replace('http://', 'https://', $castorConfig_live_site);
}

$castorConfig_live_site = str_replace('/administrator/', '', $castorConfig_live_site);
$castorConfig_live_site = str_replace('/administrator', '', $castorConfig_live_site);
if (substr($castorConfig_live_site, -1) == '/') {
	$castorConfig_live_site = substr($castorConfig_live_site, 0, -1);
}
...
if (!defined('AUTO_UPGRADE')) {
	$castorConfig_lang = JFactory::getLanguage()->getTag();
	
	//get lang short code
	$languages = JLanguageHelper::getLanguages('lang_code');
	
	if (isset($languages[$castorConfig_lang])) {
		$castorConfig_lang_shortcode = $languages[$castorConfig_lang]->sef;
	}
}
