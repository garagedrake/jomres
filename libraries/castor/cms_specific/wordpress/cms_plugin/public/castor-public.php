...
class Castor_Public
{
	 ...
	 * @since	9.9.19
	 */
	public function frontend_trigger_castor()
	{
		...
		if ($wp_castor->get_content() == '' && is_object($post)) {
			// Check if we are on a [castor:xx-XX] post
			// Shortcode [castor:xx-XX] xx-XX is the language code
			if (strstr(strtolower($post->post_content), '[castor:'.strtolower($language).']')) {
				ob_start();
				...
				if (isset($_REQUEST[ 'tmpl' ]) && $_REQUEST[ 'tmpl' ] == 'castor') {
					add_filter('show_admin_bar', '__return_false');
				}

				trigger_castor();

				add_action('wp_enqueue_scripts', array($wp_castor, 'add_castor_js_css'), 9999);
	...
	/**
	 * Wraps Castor output in the asamodule_search_results div.
	 *
	 * @since	9.9.19
	 */
	public function asamodule_search_results($content)
	{
		...
		if (is_object($post) && strstr(strtolower($post->post_content), '[castor:' . strtolower($language) . ']')) {
			if ($wp_castor->get_content() != '') {
				$content = '<div id="asamodule_search_results" style="max-width:100%">' . $wp_castor->get_content() . '</div>';
			} else {
	...
	/**
	 * Sets Castor pages meta title.
	 *
	 * @since	9.9.19
	 */
	public function set_castor_meta_title($title, $id = null)
	{
		
		$wp_castor = WP_Castor::getInstance();
		
		$meta_title = $wp_castor->get_meta_title();
	 ...
	 * @since	9.9.19
	 */
	public function castor_fullscreen_view($template)
	{
		
		$template = ABSPATH . CASTOR_ROOT_DIRECTORY . '/libraries/fullscreen_view/wp-fullscreen.php';
