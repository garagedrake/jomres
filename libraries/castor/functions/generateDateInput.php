...
// ################################################################
	...
	if (!function_exists('generateDateInput')) {
		function generateDateInput($fieldName, $dateValue = '', $myID = false, $siteConfig = false, $historic = false , $options = [] )
		{
			...
			$uniqueID = generateCastorRandomString(15);
			...
			$placeholder = jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL', '_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL', false);
			...
			if ($fieldName == 'arrivalDate' || $fieldName == 'asc_arrivalDate' || $fieldName == 'start') {
				...
				$placeholder = jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE', '_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE', false);
	...
	castorJquery(function() {
		...
		';
			...
			if ($fieldName == 'arrivalDate' || $fieldName == 'asc_arrivalDate' || $fieldName == 'start') {
				...
				var nextDayDate = castorJquery("#' .$uniqueID.'").datepicker(\'getDate\', \'+1d\');
				nextDayDate.setDate(nextDayDate.getDate() + 1);
				castorJquery("#' .get_showtime('departure_date_unique_id').'").datepicker(\'setDate\', nextDayDate);
				castorJquery("#' .get_showtime('departure_date_unique_id').'").datepicker(\'option\', {minDate: castorJquery(this).datepicker(\'getDate\')});
				}';
				$output .= ', onClose: function() {
				castorJquery("#' .get_showtime('departure_date_unique_id').'").datepicker(\'show\');
				}';
			}
