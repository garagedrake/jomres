 ...
 *
 */
	if (!function_exists('castor_decode')) {
		function castor_decode($string)
		{
			...
			/* 	Left in-situ for now, as xss clean blocks xss threats, however users may want to refer to and/or use our previous filtering.
				...
				$string = str_replace( "&#39;", "&#96;", $string );
				$string = str_replace( "&#60;x&#62;", "", $string ); // Older versions of Castor used to add <x> to strings to make them safe. We don't need that any more, however if we don't add this line then some older installations may show <x> in, for example, the property details.
				$string = str_replace( "&#60;", "<", $string );
				$string = str_replace( "&#62;", ">", $string );
				$string = str_replace( "&lt;", "<", $string );
				$string = str_replace( "&gt;", ">", $string ); */
