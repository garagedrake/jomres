 ...
 */
	if (!function_exists('output_price')) {
		function output_price($value, $currencycode = '', $do_conversion = true, $zeroOK = true, $just_code = false)
		{
			...
			if ($castor_currency_conversion->this_code_can_be_converted($currencycode) && $currencycode != $foreign && $do_conversion && $foreign != '' && $price > 0.00) {
				$base = $currencycode;
				$converted_price = $castor_currency_conversion->convert_sum($price, $base, $foreign);
				...
				$original_price_template = simple_template_output(CASTOR_TEMPLATEPATH_FRONTEND, 'output_price_native_price.html', $price);
				...
				if ($price > 0.00 || $zeroOK) {
					...
					if ($mrConfig['poa_price'] != '') {
						...
						$price = ' '.$symbols[ 'pre' ].jr_gettext('_CASTOR_PRICE_ON_APPLICATION', '_CASTOR_PRICE_ON_APPLICATION', false, false).$symbols[ 'post' ];
					}
				}
			}
