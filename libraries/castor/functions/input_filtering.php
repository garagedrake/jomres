...
function RemoveXSS($val)
{
	 ...
	 * @package Castor\Core\Functions
	 *
	 * Filters all inputs passed from Requests into Castor
	 */
function castorGetParam($request, $element, $def = null) // variable type not used, we'll cast the variable type depending on the default ($def) that's passed to the function
{
	...
	if ($enable_input_cache) {
		$purified_inputs_cache = castor_singleton_abstract::getInstance('purified_inputs_cache');
	...
	if (!castor_cmsspecific_areweinadminarea() && this_cms_is_joomla() && get_showtime('sef') == '1' && !AJAXCALL && !defined('AUTO_UPGRADE') && in_array($element, $sef_vars) && !array_key_exists($element, $request)) {
		if ( _CASTOR_DETECTED_CMS == 'joomla4' || _CASTOR_DETECTED_CMS == 'joomla5') {
		...
		default: // treat everything else as a string.
			...
			if (in_array($element, $allowed_inputs)) {
				if ($jrConfig[ 'input_filtering' ] != 'weak') {
					$clean = castor_purify_html($dirty);
				...
				$clean = castor_sanitise_string($dirty);
	 ...
	 *
	 */
function castor_sanitise_string($dirty)
{
	...
	$html_purifier = castor_singleton_abstract::getInstance('castor_input_filter_singleton');
	$dirty = castor_remove_HTML($dirty); // Strip out any html
	 ...
	 *
	 */
function castor_purify_html($dirty)
{
...
// Like many other functions in Castor, it has been renamed to castor_ becomes it's not unusual for other software developers to use the same functions in their libraries. Renaming the function thus prevents php from throwing duplicate function name errors
...
function castor_remove_HTML($s, $keep = 'p|br', $expand = 'script|style|noframes|select|option')
{
	//prep the string
	$s = ' '.$s;
...
function castor_code2utf($num)
{
	 ...
	 */
function castor_get_var_type($variable)
{
	 ...
	 *
	 *          If it is, a naughty person might be trying to hack Castor
	 */
function castor_parseRequest() //
{
