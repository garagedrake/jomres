...
*
* Uses the castor_countries singleton to get country names
*
...
*/
	if (!function_exists('getSimpleCountry')) {
		function getSimpleCountry($selectedCountry = '')
		{
			...
			if (isset($castor_countries->used_countries[$selectedCountry]['countryname'])) {
				return $castor_countries->used_countries[$selectedCountry]['countryname'];
			} else { //this shouldn`t usually happen
				$castor_countries->get_all_countries();

				if (isset($castor_countries->countries[$selectedCountry]['countryname'])) {
					return $castor_countries->countries[$selectedCountry]['countryname'];
				}
			}
	...
	if (!function_exists('configCountries')) {
		function configCountries()
		{
			...
			foreach ($castor_countries->countries as $country) {
				$options[] = castorHTML::makeOption($country['countrycode'], $country['countryname']);
			}

			$countryDropdown = castorHTML::selectList($options, 'cfg_defaultcountry', '', 'value', 'text', $selectedCountry);
...
*
* Used in numerous places, pass it the country code and an optional input name for the form element. The castor_countries singleton creates a list of countries (including translation of the country names ).
*
*/
	if (!function_exists('createSimpleCountriesDropdown')) {
		function createSimpleCountriesDropdown($selectedCountry = '', $input_name = 'guest_country')
		{
			...
			$countryDropdown = castorHTML::selectList($options, $input_name, '', 'value', 'text', $selectedCountry);
...
*/
	if (!function_exists('limitCountriesDropdown')) {
		function limitCountriesDropdown()
		{
			...
			$countryDropdown = castorHTML::selectList($options, 'cfg_limit_property_country_country', ' ', 'value', 'text', $jrConfig[ 'limit_property_country_country' ]);
...
*/
	if (!function_exists('setupRegions')) {
		function setupRegions($countryCode = 'GB', $currentRegion = 'Pembrokeshire', $firstBlank = false, $input_name = 'region')
		{
			...
			$regionArray = $castor_regions->get_country_regions($countryCode);
			...
			if (!is_numeric($currentRegion)) { // This allows us to transition from using region names in the dropdown's value field, to region ids.
				...
				$currentRegion = castor_decode($currentRegion);
			}

			if (!empty($regionArray)) {
				natcasesort($regionArray);
				if ($firstBlank) {
					$regions[ ] = castorHTML::makeOption('', '');
				}

				foreach ($regionArray as $k => $v) {
					$regions[ ] = castorHTML::makeOption($k, $v);
				}
				$regionDropdown = castorHTML::selectList($regions, $input_name, '', 'value', 'text', $currentRegion);
			}
...
*
* Used by the installer when creating a new installation of Castor.
*
*/
...
*
* Not currently used. Was used when building dropdowns but has now been superceeded by the castor_countries singleton
*
* @depreciated
*
*/
 ...
 * @package Castor\Core\Functions
*
* Replaces the older countryCodes array, which is now only used for importing into the #__castor_countries table
*
*/
...
function old_countryCodesArray($translate = true)
{
	...
	if ($translate) {
		...
		foreach ($countryCodes as $key => $val) {
			$new_arr[ $key ] = jr_gettext('_CASTOR_CUSTOMTEXT_COUNTRYNAMES_'.$key, $val, false, false);
		}
...
function import_countries()
{
	...
	if (empty($castor_countries->countries)) {
		$query = '
			INSERT INTO #__castor_countries 
		...
		$castor_countries->countries = false;
		$castor_countries->get_all_countries();
