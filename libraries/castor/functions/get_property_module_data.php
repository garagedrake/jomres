...
// Intended as a utility function used by Castor modules to display information about a property in modules
	/**
	 *
	 * @package Castor\Core\Functions
	 *
	 */
	if (!function_exists('get_property_module_data')) {
		function get_property_module_data($property_uid_array, $alt_template_path = '', $alt_template_name = '', $vertical = false)
		{
			...
			$castor_property_list_prices->gather_lowest_prices_multi($property_uid_array);

			$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
			$castor_media_centre_images->get_images_multi($property_uid_array, array('property'));
			...
			if ($alt_template_name == '' && isset($_REQUEST[$template_file_name]) && $_REQUEST[$template_file_name] != '') {
				$temp_template = castorGetParam($_REQUEST, $template_file_name, '');
			...
			foreach ($property_uid_array as $property_uid) {
				if ($property_uid > 0) {
					...
					if ($cache_feature_enabled && file_exists($cache_file) && $jrConfig[ 'development_production' ] != 'development' ) {
						...
						$property_data[ 'RANDOM_IDENTIFIER' ] = generateCastorRandomString(10);
						...
						$property_data['_CASTOR_REVIEWS'] = jr_gettext('_CASTOR_REVIEWS', '_CASTOR_REVIEWS', false);
						$property_data['_CASTOR_REVIEWS_AVERAGE_RATING'] = jr_gettext('_CASTOR_REVIEWS_AVERAGE_RATING', '_CASTOR_REVIEWS_AVERAGE_RATING', false);
						$property_data['_CASTOR_REVIEWS_TOTAL_VOTES'] = jr_gettext('_CASTOR_REVIEWS_TOTAL_VOTES', '_CASTOR_REVIEWS_TOTAL_VOTES', false);
						$property_data['_CASTOR_REVIEWS'] = jr_gettext('_CASTOR_REVIEWS', '_CASTOR_REVIEWS', false);
						$property_data['COLON'] = ' : ';
						...
						$castor_ribbon_generator = new castor_ribbon_generator($property_uid);
						...
						if ($property_data['featured'] == true) {
							$castor_ribbon_generator->set_review_score(10, $featured);
						}
						$property_data['RIBBON'] = $castor_ribbon_generator->generate_html();
						...
						$property_data[ 'THUMBNAIL' ] = $castor_media_centre_images->images ['property'][0][0]['small'];

						$property_data[ 'IMAGELARGE' ] = $castor_media_centre_images->images ['property'][0][0]['large'];
						$property_data[ 'IMAGEMEDIUM' ] = $castor_media_centre_images->images ['property'][0][0]['medium'];
						$property_data[ 'IMAGETHUMB' ] = $castor_media_centre_images->images ['property'][0][0]['small'];
						...
						$property_data[ 'PROPERTY_DESCRIPTION' ] = ($castor_markdown->get_markdown($current_property_details->property_description));

						$property_data[ 'PRICE_PRE_TEXT' ] = $castor_property_list_prices->lowest_prices[$property_uid][ 'PRE_TEXT' ];
						$property_data[ 'PRICE_PRICE' ] = $castor_property_list_prices->lowest_prices[$property_uid][ 'PRICE' ];
						$property_data[ 'PRICE_POST_TEXT' ] = $castor_property_list_prices->lowest_prices[$property_uid][ 'POST_TEXT' ];

						$property_data[ 'PROPERTY_UID' ] = $property_uid;

						$property_data[ 'LIVE_SITE' ] = get_showtime('live_site');
						$property_data[ 'MOREINFORMATION' ] = jr_gettext('_CASTOR_COM_A_CLICKFORMOREINFORMATION', '_CASTOR_COM_A_CLICKFORMOREINFORMATION', $editable = false, true);
						$property_data[ 'QUICKINFORMATION' ] = jr_gettext('_CASTOR_QUICK_INFO', '_CASTOR_QUICK_INFO', $editable = false, true);
						...
						if ($mrConfig[ 'singleRoomProperty' ] == '1') {
							if ($mrConfig[ 'requireApproval' ] == '1') {
								...
								$property_data[ 'BOOKTHIS_TEXT' ] = jr_gettext('_CASTOR_FRONT_MR_MENU_BOOKTHISPROPERTY', '_CASTOR_FRONT_MR_MENU_BOOKTHISPROPERTY', false);
							}
						} else {
							if ($mrConfig[ 'requireApproval' ] == '1') {
								$property_data[ 'BOOKTHIS_TEXT' ] = jr_gettext('_BOOKING_CALCQUOTE', '_BOOKING_CALCQUOTE', false);
							} else {
								$property_data[ 'BOOKTHIS_TEXT' ] = jr_gettext('_CASTOR_FRONT_MR_MENU_BOOKAROOM', '_CASTOR_FRONT_MR_MENU_BOOKAROOM', false);
							}
						}
						...
						$property_data[ 'TOWN_SEARCH_LINK' ] = castorURL(CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&town='.castor_decode($current_property_details->multi_query_result[ $property_uid ][ 'property_town' ]));
						$property_data[ 'REGION_SEARCH_LINK' ] = castorURL(CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&region='.$current_property_details->multi_query_result[ $property_uid ][ 'property_region_id' ]);
						$property_data[ 'COUNTRY_SEARCH_LINK' ] = castorURL(CASTOR_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&country='.castor_decode($current_property_details->multi_query_result[ $property_uid ][ 'property_country_code' ]));


						$property_data[ 'SUPERIOR' ] = '';
						if ($property_data[ 'superior' ] == 1) {
							$property_data[ 'SUPERIOR' ] = '<img src="'.CASTOR_IMAGES_RELPATH.'superior.png" alt="superior" border="0" />';
						}
						if ($property_data['cat_id'] > 0) {
							$castor_property_categories->get_property_category($property_data['cat_id']);
							$property_data["PROPERTY_CATEGORY"]		= $castor_property_categories->title;
						} else {
