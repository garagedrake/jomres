...
// ################################################################
	...
	if ($selectedProperty > 0 &&
		...
		castorRedirect(get_booking_url($selectedProperty), '');
	}
	if (!defined('CASTOR_API_CMS_ROOT')) {
		$selectedProperty = $property_uid;
	}

	$remus = castorGetParam($_REQUEST, 'remus', '');
	...
	if (!isset($_REQUEST[ 'arrivalDate' ])) {
		...
		if (!isset($_REQUEST[ 'pdetails_cal' ])) {
			$thisdate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'arrivalDate', ''));
		} else {
	...
	$query = "SELECT propertys_uid FROM #__castor_propertys WHERE propertys_uid = '".(int) $selectedProperty."'";
...
// End run here.
	...
	function dobooking($selectedProperty, $thisdate, $remus)
	{
		...
		if ($thisJRUser->id > 0) {
			...
			if ($previous_guest_uid) {
				...
				if ($jrportal_guests->blacklisted == 1) {
					castorRedirect(castorURL(CASTOR_SITEPAGE_URL.'&task=contactowner&amp;selectedProperty='.$selectedProperty.'&amp;arrivalDate='.$thisdate));
				}
			}
		}
		...
		if (get_showtime('include_room_booking_functionality')) {
			$query = 'SELECT `rates_uid` FROM `#__castor_rates` where property_uid = '.(int) $selectedProperty.'';
			$result = doSelectSql($query);
			if (empty($result)) {
				castorRedirect(castorURL(CASTOR_SITEPAGE_URL.'&task=contactowner&amp;selectedProperty='.$selectedProperty.'&amp;arrivalDate='.$thisdate));
			}
		...
		$output[ 'UPDATEADDRESSBUTTON' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_UPDATEADDRESSBUTTON', '_CASTOR_BOOKINGFORM_UPDATEADDRESSBUTTON', false, false));
		/*	if ($mrConfig[ 'singleRoomProperty' ] == '1') {
				$output[ 'BLOCKUI_RECHECKINGROOMAVAILABILITY' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_RECHECKINGROOMAVIALABILITY_SRP', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_RECHECKINGROOMAVIALABILITY_SRP', false, false));
			} else {
				$output[ 'BLOCKUI_RECHECKINGROOMAVAILABILITY' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_RECHECKINGROOMAVIALABILITY', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_RECHECKINGROOMAVIALABILITY', false, false));
			}*/

		$output[ 'BOOKING_FORM_CALENDAR' ] = get_showtime('booking_form_calendar');
		$output[ '_CASTOR_COM_A_AVLCAL' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_COM_A_AVLCAL', '_CASTOR_COM_A_AVLCAL', false, false));
		$output[ '_CASTOR_FRONT_MR_SUBMITBUTTON_CHECKAVAILABILITY' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_FRONT_MR_SUBMITBUTTON_CHECKAVAILABILITY', '_CASTOR_FRONT_MR_SUBMITBUTTON_CHECKAVAILABILITY', false, false));
		...
		if ($mrConfig[ 'showAvailabilityCalendar' ] == '1') {
			$calendar_modal = array( 0 =>
				array (
					...
					'_CASTOR_COM_A_AVLCAL' => $output[ '_CASTOR_COM_A_AVLCAL' ],
					'_CASTOR_FRONT_MR_SUBMITBUTTON_CHECKAVAILABILITY' => $output[ '_CASTOR_FRONT_MR_SUBMITBUTTON_CHECKAVAILABILITY' ]
				)
			);
		}

		/*	$output[ 'BLOCKUI_CHANGINGEXTRA' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_CHANGINGEXTRA', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_CHANGINGEXTRA', false, false));
			$output[ 'BLOCKUI_CHANGINGROOMSELECTION' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_CHANGINGROOMSELECTION', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_CHANGINGROOMSELECTION', false, false));
			$output[ 'BLOCKUI_UPDATINGADDRESS' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_UPDATINGADDRESS', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_UPDATINGADDRESS', false, false));
			$output[ 'BLOCKUI_ADDRESSINPUTERROR' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_ADDRESSINPUTERROR', '_CASTOR_BOOKINGFORM_BLOCKUIMESSAGES_ADDRESSINPUTERROR', false, false));*/
		$output[ 'NOROOMSSELECTEDYETMESSAGE' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_NOROOMSSELECTEDYET', '_CASTOR_BOOKINGFORM_NOROOMSSELECTEDYET', false, false));
		...
		$output[ 'SUBMITURL' ] = castorURL(CASTOR_SITEPAGE_URL.'&task=confirmbooking');
		...
		if ($bkg->use_coupons) {
			...
			$coupon_output[ 'COUPON_BUTTON' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_COUPON_APPLYBUTTON', '_CASTOR_AJAXFORM_COUPON_APPLYBUTTON', false, false));
			$coupon_output[ 'COUPON_INSTRUCTIONS' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_COUPON_INSTRUCTIONS', '_CASTOR_AJAXFORM_COUPON_INSTRUCTIONS'));
			$coupon_output_totals[ 'COUPON_DISCOUNT_VALUE' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_COUPON_DISCOUNTVALUE', '_CASTOR_AJAXFORM_COUPON_DISCOUNTVALUE'));

			$coupon = castorGetParam($_REQUEST, 'coupon', '');
			...
			if ($amend_contract && isset($_REQUEST[ 'contractuid' ]) && $thisJRUser->userIsManager && in_array((int) $selectedProperty, $thisJRUser->authorisedProperties)) {
				$query = 'SELECT coupon_id FROM #__castor_contracts WHERE contract_uid = '.(int) $_REQUEST[ 'contractuid' ].'';
				$coupon_id = doSelectSql($query, 1);
				if ($coupon_id > 0) {
					$query = 'SELECT `coupon_code` FROM #__castor_coupons WHERE coupon_id = '.(int) $coupon_id.' AND property_uid = '.(int) $selectedProperty.'';
		...
		if ($bkg->cfg_singleRoomProperty != '1') {
			$output[ 'SELECTROOMMESSAGE' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_SELECT_A_ROOM', '_CASTOR_BOOKINGFORM_MONITORING_SELECT_A_ROOM', false, false));
		} else {
			$output[ 'SELECTROOMMESSAGE' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_QUICKRES_STEP4_TITLE', '_CASTOR_COM_MR_QUICKRES_STEP4_TITLE', false, false));
		}
		...
		if ($bkg->guest_specific_discount > 0) {
			$output[ 'PERSONAL_DISCOUNT' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_PERSONAL_DISCOUNT', '_CASTOR_PERSONAL_DISCOUNT'));
		}
		...
		if ($bkg->arrivalDate != '' && !$overrideSessionArrivalDate) {
			...
			if (!isset($_REQUEST[ 'arrivalDate' ])) {
				...
				$departureDate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'departureDate', ''));
			}
		...
		$output[ 'EARLIESTPOSSIBLEARRIVALDATE' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EARLIESTPOSSIBLEARRIVALDATE', '_CASTOR_AJAXFORM_EARLIESTPOSSIBLEARRIVALDATE', false)).$bkg->JSCalmakeInputDates($arrivalDate);
		...
		if ((!empty($extra_details) || !empty($third_party_extras)) && $mrConfig[ 'showExtras' ] == '1') {
			$output[ 'EXTRAS_INFO' ] = '<img border="0" style="vertical-align:top;" src="'.CASTOR_IMAGES_RELPATH.'info.png" />';
			$output[ 'AJAXFORM_EXTRAS' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS', '_CASTOR_AJAXFORM_EXTRAS'));
			$output[ 'AJAXFORM_EXTRAS_DESC' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS_DESC', '_CASTOR_AJAXFORM_EXTRAS_DESC', false));
			$output[ 'EXTRAS_TOTAL' ] = $bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS_TOTAL', '_CASTOR_AJAXFORM_EXTRAS_TOTAL'));
		...
		if ($mrConfig[ 'singleRoomProperty' ] == '0') {
			if ($mrConfig[ 'booking_form_rooms_list_style' ] == '1') {
				$rm = '<div><strong>'.$bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_SELECTEDROOMS', '_CASTOR_AJAXFORM_SELECTEDROOMS')).'</strong></div>';
				if ($bkg->numberOfCurrentlySelectedRooms() > 0) {
					$rm .= $bkg->listCurrentlySelectedRooms();
				} else {
					$rm .= '<div class="ui-state-error">'.$bkg->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_NOROOMSSELECTEDYET', '_CASTOR_BOOKINGFORM_NOROOMSSELECTEDYET')).'</div>';
				}
				$output[ 'SELECTEDROOM' ] = $rm;

				$rm = '<div><strong>'.$bkg->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLEROOMS', '_CASTOR_AJAXFORM_AVAILABLEROOMS')).'</strong></div>';
				//$rm.="<br>";
		...
		if (get_showtime('include_room_booking_functionality')) {
			if ($mrConfig[ 'singleRoomProperty' ] == '1' && (isset($thisdate) && !empty($thisdate))) {
				...
				$tmpsrptrigger = "castorJquery.get(ajaxurl+'&task=handlereq&property_uid_check=".(int) $selectedProperty."',{ field : 'arrivalDate', value: '".$bkg->JSCalmakeInputDates($bkg->arrivalDate)."', field : 'departureDate', value: '".$bkg->JSCalmakeInputDates($bkg->departureDate)."' },function(data){showRoomsList(data); show_log('');});";
				$load[ 'ONLOAD' ] = $tmpsrptrigger;
				$toload[ ] = $load;
			} else {
				$load[ 'ONLOAD' ] = "castorJquery.get(ajaxurl+'&task=handlereq&firstrun=1&property_uid_check=".(int)$selectedProperty."&field=arrivalDate&value=".$bkg->JSCalmakeInputDates($bkg->arrivalDate)."',{},function(data)
			{ 
			castorJquery.get(ajaxurl+'&task=handlereq&property_uid_check=".(int)$selectedProperty."&initialise_form=1&field=departureDate&value=".$bkg->JSCalmakeInputDates($bkg->departureDate)."' , {} , function(){}); 
		...
		$tmpsrptrigger = "castorJquery.get(ajaxurl+'&task=handlereq&initialise_form=1&property_uid_check=".(int) $selectedProperty."',{ field : 'standard_guests', value: '2' },function(data){showRoomsList(data); show_log('');});";
		$load[ 'ONLOAD' ] = $tmpsrptrigger;
		$toload[ ] = $load;

		$output[ 'CASTOR_SITEPAGE_URL' ] = castorValidateUrl(CASTOR_SITEPAGE_URL);

		if (get_showtime('include_room_booking_functionality')) {
			$output[ 'CASTOR_ROOMSLISTENABLED' ] = 'true';
		} else {
			$output[ 'CASTOR_ROOMSLISTENABLED' ] = 'false';
		}

		jr_import('castor_custom_field_handler');
		$custom_fields = new castor_custom_field_handler();
		...
		if (get_showtime('task') == 'dobooking' && $jrConfig[ 'booking_form_modal_popup' ] == '1' && !isset($_REQUEST[ 'is_wrapped' ]) && !isset($_REQUEST[ 'tmpl' ]) && !using_bootstrap()) {
			$output[ 'MODAL' ] = 'castorJquery( "#booking_form" ).dialog({width:1000,modal:true});';
			$output[ 'HIDDENSTYLE' ] = 'style="display:none;"';

			$output[ 'OPENBOOKINGOFRM_BUTTON' ] = '<a href="javascript:open_booking_form();" class="fg-button ui-state-default ui-corner-all">'.$output[ '_CASTOR_AJAXFORM_BUTTON_OPEN_BOOKINGFORM' ].'</a>';
			$output[ 'BUTTON_BACK_TO_PROPERTY_DETAILS' ] = '<a href="'.get_property_details_url($selectedProperty).'" class="fg-button ui-state-default ui-corner-all">'.$output[ '_CASTOR_AJAXFORM_BUTTON_BACK_TO_PROPERTY_DETAILS' ].'</a>';
		}
		...
		$output[ 'EMAIL_ALREADY_INUSE' ] = jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_EMAIL_ALREADY_IN_USE', '_CASTOR_BOOKINGFORM_MONITORING_EMAIL_ALREADY_IN_USE', false, false)." <button type='button' id='login_modal_trigger' class='btn btn-primary'>".jr_gettext('_CASTOR_CUSTOMCODE_CASTORMAINMENU_LOGIN', '_CASTOR_CUSTOMCODE_CASTORMAINMENU_LOGIN', false, false)."</button>";
		echo "<script>
        castorJquery(function(){
            castorJquery('[name=castor_loginform_email]').val( castorJquery('#eemail').val() );  
            castorJquery('body').on('click','#login_modal_trigger',function(){
                castorJquery('#loginModal').modal('show');
            });
        });
     </script>";
		...
		if ($bkg->room_feature_filtering_enabled) {
			...
			$roomfeaturesHeader[ ] = array('_CASTOR_BOOKINGORM_ROOMFEATURE_FILTER' => $output[ '_CASTOR_BOOKINGORM_ROOMFEATURE_FILTER' ]);
		...
		if (!isset($_COOKIE['castor_gdpr_consent_form_processed']) || $_COOKIE['castor_gdpr_consent_form_processed'] == "0") {
			if ($jrConfig[ 'enable_gdpr_compliant_fucntionality' ] == "1" && !isset($_REQUEST['skip_consent_form'])) {
				...
				$consent_output['_CASTOR_GDPR_CONSENT_TRIGGER_FORM'] = jr_gettext('_CASTOR_GDPR_CONSENT_TRIGGER_FORM', '_CASTOR_GDPR_CONSENT_TRIGGER_FORM', false);
				...
				if (castor_bootstrap_version() != "5") {
					echo "<script>window.onload = function() {castorJquery('#consentForm').modal('show');}</script>";
				} else {
		...
		/*	if ($jrConfig[ 'enable_gdpr_compliant_fucntionality' ] == "1") {
				...
				if ( !isset($_COOKIE['castor_gdpr_consent_form_processed']) || !$_COOKIE['castor_gdpr_consent_form_processed'] == "1" ) {
					echo '<script>
					castorJquery(document).ready(function () {
						castorJquery(\'#booking_form\').block({ message: null });
					});
					</script>
				';
				}
		...
		$login_form = $MiniComponents->specificEvent('06000', 'show_login_form', array ('output_now' => false , 'login_reason' => jr_gettext('_CASTOR_LOGIN_REASON_EMAIL_ALREADY_USED', '_CASTOR_LOGIN_REASON_EMAIL_ALREADY_USED', false)  ));
