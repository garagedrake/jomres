...
// ################################################################
	...
	/*
	 * This script allows you to have customised versions of Castor functions in your own 'custom_functions.php' file
	 *
	 * custom_functions.php file(s) are included before the Castor functions.php script, and before the CMS specific functions script.
	 *
	 * Priorities :
	 * First it will attempt to find custom_functions.php in the theme's/template's  html/com_castor/custom_code directory.
	 * Next in castor/core-plugins
	 * Finally in castor/remote_plugins
	 *
	 * All Castor functions that users are likely to want to override are wrapped in an if (!function_exists('xxx')) check, so you can safely have your custom versions of functions without needing to modify the Core.
	 *
...
if (!defined('CASTOR_OVERRIDE_PATH')) {
	...
	$files = load_custom_functions_scanAllDir(CASTOR_REMOTEPLUGINS_ABSPATH);
	if (!empty($files)){
		foreach ($files as $filename) {
			$bang = explode(DIRECTORY_SEPARATOR , $filename);
			if (end($bang) === 'custom_functions.php') {
    ...
    $files = load_custom_functions_scanAllDir(CASTOR_COREPLUGINS_ABSPATH);
    if (!empty($files)){
        foreach ($files as $filename) {
            $bang = explode(DIRECTORY_SEPARATOR , $filename);
            if (end($bang) === 'custom_functions.php') {
