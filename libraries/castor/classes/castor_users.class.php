...
class castor_users
{
	...
	function get_users()
	{
		...
		if (!is_array($this->all_cms_users)) {
			$this->all_cms_users = castor_cmsspecific_getCMSUsers();
		}
		
		$query	= "SELECT `manager_uid`, `userid`, `access_level`, `currentproperty`, `apikey`, `suspended`, `last_active` FROM #__castor_managers";
		...
		if (!empty($this->users)) {
			$query = "SELECT `manager_id`, `property_uid` FROM #__castor_managers_propertys_xref WHERE `manager_id` IN (" . castor_implode(array_keys($this->users)) . ")";
			$result = doSelectSql($query);
	...
	function get_user($cms_user_id = 0)
	{
		...
		if (!isset($this->users[$cms_user_id])) {
			...
			$query	= "SELECT `manager_uid`, `userid`, `access_level`, `currentproperty`, `apikey`, `suspended`, `last_active` FROM #__castor_managers WHERE `userid` = " . (int)$cms_user_id;
			$result = doSelectSql($query);
			...
			if ($result) {
				...
				$query = "SELECT `manager_id`, `property_uid` FROM #__castor_managers_propertys_xref WHERE `manager_id` = " . (int)$cms_user_id;
				$result = doSelectSql($query);
	...
	function commit_new_user()
	{
		...
		//assign the castor user role to this new user
		$query = "INSERT INTO #__castor_managers 
						(
						`userid`,
						`access_level`,
						`currentproperty`,
	...
	function commit_update_user()
	{
		...
		$query = "UPDATE #__castor_managers SET `access_level` = " . (int)$this->access_level . " WHERE `userid` = " . (int)$this->cms_user_id;
	...
	function delete_user($cms_user_id = 0)
	{
		...
		$query = "DELETE FROM #__castor_managers WHERE `userid` = " . $cms_user_id;
	...
	function generate_user_api_key()
	{
		...
		$query  = "UPDATE #__castor_managers SET `apikey`= '" . $apikey . "' WHERE `userid` = " . $this->cms_user_id;
	...
	public function getManagerIdsForProperty($property_uid = 0, $notIncludingSuperManagers = false)
	{
		...
		$query = 'SELECT a.id, a.manager_id FROM #__castor_managers_propertys_xref a, #__castor_managers b WHERE a.property_uid = '.(int) $property_uid.' ';
