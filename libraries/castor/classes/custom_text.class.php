...
class custom_text
{
	...
	public function __construct()
	{
		...
		$this->all_languages  = $castor_language->define_langfile_to_languages_array();
	...
	public function gather_data($property_uids = array())
	{
		...
		if (!empty($property_uids)) {
			$query = 'SELECT `constant` AS language_constant, `customtext`, `property_uid`, `language_context` FROM #__castor_custom_text WHERE `property_uid` IN ('.castor_implode($property_uids).") AND `language` = '".$this->lang."'";
	...
	function updateCustomText($theConstant, $theValue, $audit = true, $property_uid = null, $language_context = '0', $target_language = '')
	{
		...
		if (strlen($testStr) == 0 &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_DESCRIPTION_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_CHECKINTIMES_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_AREAACTIVITIES_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_DIRECTIONS_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_AIRPORTS_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_OTHERTRANSPORT_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_ROOMTYPE_DISCLAIMERS_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_PROPERTY_METATITLE_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_PROPERTY_METADESCRIPTION_'.$property_uid &&
			$theConstant != '_CASTOR_CUSTOMTEXT_PROPERTY_METAKEYWORDS_'.$property_uid
			) {
			return false;
		}
		...
		$query = "SELECT `customtext` FROM #__castor_custom_text 
		...
		if (strlen($theValue) == 0 || $theValue == "") {
			$query = "DELETE FROM #__castor_custom_text 
							WHERE `constant` = '".$theConstant."' 
							AND `property_uid` = ".(int) $property_uid." 
							AND `language` = '".$target_language."' 
							AND `language_context` = '".$language_context."'";
			...
			if (empty($result)) {
				...
				$query = "UPDATE #__castor_custom_text 
		...
		$audit_msg = '';
		if ($audit) {
			$audit_msg = jr_gettext('_CASTOR_MR_AUDIT_UPDATECUSTOMTEXT', '_CASTOR_MR_AUDIT_UPDATECUSTOMTEXT', false);
		}
