...
class castor_media_centre_images_dbimport
{

	protected $propertys_uids;
	
	protected $castor_media_centre_images;
	...
	public function __construct($propertys_uids = array(), $import_site_images = false)
	{
		...
		$this->castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
	}
	...
	public function run()
	{
		...
		if ($this->import_site_images) {
			...
			$this->castor_media_centre_images->site_images = array();
		}
	...
	private function get_site_images()
	{
		...
		foreach ($resource_types as $k => $v) {
			$this->castor_media_centre_images->get_site_images($k);
		}
	...
	private function get_property_images($p_uids = array())
	{
		...
		$this->castor_media_centre_images->get_images_multi($p_uids);
	...
	private function import_site_images()
	{
		...
		foreach ($resource_types as $k => $v) {
			if (isset($this->castor_media_centre_images->site_images[$k]) && !empty($this->castor_media_centre_images->site_images[$k])) {
				...
				if ($resource_types[$k]['resource_id_required']) {
					foreach ($this->castor_media_centre_images->site_images[$k] as $resource_id => $resource) {
					...
					foreach ($this->castor_media_centre_images->site_images[$k] as $filenames => $f) {
				...
				//clear memory
				unset($this->castor_media_centre_images->site_images[$k]);
			}
		}
	...
	private function import_property_images($property_uids = array())
	{
		...
		foreach ($property_uids as $property_uid) {
			if (isset($this->castor_media_centre_images->multi_query_images[$property_uid])) {
				foreach ($this->castor_media_centre_images->multi_query_images[$property_uid] as $resource_type => $resource_ids) {
				...
				unset($this->castor_media_centre_images->multi_query_images[$property_uid]);
			}
		}
