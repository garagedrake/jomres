...
class webhooks
{
	...
	{
		$query = 'SELECT `id`, `manager_id` , `settings` , `enabled` FROM #__castor_webhooks_integrations WHERE manager_id = '.(int)$this->manager_id.' OR manager_id = 0';
	...
	public function commit_integration($id = 0)
	{
		if ($id == 0) {
			...
			$query = "INSERT INTO #__castor_webhooks_integrations (`manager_id`, `settings` , `enabled`) VALUES (".$this->manager_id." , '".serialize($this->webhooks[$id]["settings"])."' , ".$this->webhooks[$id]["enabled"]."  )";
			...
			$query = "UPDATE #__castor_webhooks_integrations SET 
	...
	public function delete_integration($id)
	{
		...
		$query = "DELETE FROM #__castor_webhooks_integrations WHERE id = ".(int)$id." LIMIT 1";
		$result = doInsertSql($query, false);
		if ($result == true) {
			return true;
		} else {
