...
class castor_occupancy_levels
{
	...
	public function save_occupancy_levels($room_type_id = 0)
	{
		...
		$query = "SELECT uid FROM #__castor_settings WHERE property_uid = '".(int) $this->property_uid."' and akey = 'occupancy_levels' ";
		$result = doSelectSql($query);
		if (empty($result)) {
			$query = "INSERT INTO #__castor_settings (`property_uid`,`akey`,`value`) VALUES (".(int) $this->property_uid." , 'occupancy_levels' , '".$policies."')";
		} else {
			$query = "UPDATE #__castor_settings SET `value`='".$policies."' WHERE `property_uid` = ".(int)  $this->property_uid." and `akey` = 'occupancy_levels' ";
		}
		doInsertSql($query);

		// Now to update rooms of this type with the updated occupancy levels

		$query = "UPDATE #__castor_rooms SET 
		...
		$castor_calculate_accommodates_value = new castor_calculate_accommodates_value($this->property_uid);
	...
	{
		$query = "SELECT property_uid ,value FROM #__castor_settings WHERE akey = 'accommodates_adults'";
		...
		$query = "SELECT property_uid ,value FROM #__castor_settings WHERE akey = 'accommodates_children'";
	...
	{
		$query = "SELECT property_uid ,value FROM #__castor_settings WHERE akey = 'occupancy_levels'";
