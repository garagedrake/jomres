...
// When sending invoice payments to gateways, some gateways will not let you send more than one return element in the url when passing back to ( for example ) Castor.
...
class jrportal_payment_reference
{
	...
	{
		$query ="SELECT id FROM #__castor_invoice_payment_ref WHERE invoice_id =".$this->invoice_id." LIMIT 1";
		$payment_reference = doSelectSql($query, 1);
		if (!$payment_reference) {
			$query = "INSERT INTO #__castor_invoice_payment_ref ( `invoice_id` , `gateway` ) VALUES ( '".$this->invoice_id."' , '".$this->gateway."' ) ";
		} else {
			$query ="UPDATE #__castor_invoice_payment_ref SET gateway = '".$this->gateway."' WHERE id=".(int)$payment_reference;
		}
		try {
	...
	public function get_invoice_details_for_reference($id)
	{
		...
		$query = 'SELECT `invoice_id` FROM #__castor_invoice_payment_ref  WHERE id = '.(int) $id.' LIMIT 1';
	...
	public function get_invoice_data($invoice_id)
	{
		...
		$query = 'SELECT `id`,`invoice_id`,`gateway` FROM #__castor_invoice_payment_ref  WHERE invoice_id = '.(int) $invoice_id.' LIMIT 1';
		...
		if ((int) $invoice->contract_id > 0) {
			$query = 'SELECT tag FROM #__castor_contracts WHERE contract_uid = '.(int) $invoice->contract_id;
		...
		if ((int) $invoice->subscription_id > 0 || (int) $invoice->is_commission > 0) {
			$query = "SELECT setting,value FROM #__castor_pluginsettings WHERE prid = 0 AND plugin = '".$payment_details['gateway']."' ";
	...
	public function mark_payment_reference_paid()
	{
		if (isset($this->payment_reference)) {
			$query = 'UPDATE #__castor_invoice_payment_ref SET `paid` = 1 WHERE id = '.(int) $this->payment_reference;
			try {
				doInsertSql($query);
			} catch (Exception $e) {
				output_fatal_error($e);
