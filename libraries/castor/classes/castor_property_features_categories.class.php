...
class castor_property_features_categories
{
	...
	function get_all_property_features_categories()
	{
		...
		foreach ($result as $r) {
			...
			$this->property_features_categories[(int)$r->id]['title'] 	= jr_gettext('_CASTOR_PROPERTY_FEATURES_CATEGORY'.(int)$r->id, stripslashes($r->title), false);
		}
	...
	function get_property_features_category($id = 0)
	{
		...
		$query = "SELECT `id`,`title` FROM #__castor_hotel_features_categories WHERE `id` = " . (int)$id;
		$result = doSelectSql($query);
		...
		foreach ($result as $r) {
			$this->id 		= (int)$r->id;
			$this->title 	= jr_gettext('_CASTOR_PROPERTY_FEATURES_CATEGORY'.(int)$r->id, stripslashes($r->title), false);
		}
	...
	function commit_new_property_features_category()
	{
		...
		$query = "INSERT INTO #__castor_hotel_features_categories (`title`) VALUES ('" . $this->title . "')";
	...
	function commit_update_property_features_category()
	{
		...
		$query = "UPDATE #__castor_hotel_features_categories SET `title`= '" . $this->title . "' WHERE `id` = " . (int)$this->id;
	...
	function delete_property_features_category()
	{
		...
		$query = "SELECT `cat_id` FROM #__castor_hotel_features WHERE `cat_id` = " . (int)$this->id;
		$result = doSelectSql($query);
		...
		$query = "DELETE FROM #__castor_hotel_features_categories WHERE `id` = " . (int)$this->id;
