...
class booking_engine_adults_dropdown
{
	...
	public function build_adults_dropdown($bkg)
	{
		...
		$guests_dropdown = castorHTML::integerSelectList(1,  $bkg->absolute_max_guests_by_tariffs, 1, 'standard_guests', ' autocomplete="off" onchange="getResponse_standardguests();"', 2, '%02d', $use_bootstrap_radios = false);

		$standard_guests[] = array (
			"GUESTS_DROPDOWN" => $guests_dropdown ,
			'TEXT' => jr_gettext('CASTOR_GUEST_BOOKING_FORM_LABEL', 'CASTOR_GUEST_BOOKING_FORM_LABEL')  ,
			'INFO' => jr_gettext('CASTOR_GUEST_BOOKING_FORM_LABELINFO', 'CASTOR_GUEST_BOOKING_FORM_LABELINFO')
		);
