...
// ################################################################
	...
	/**
	 * The true heart of Castor. Finds all core and plugin minicomponents for Castor, enables calling of specific minicomponents and can return minicomponent stored variables.
	 */
	...
	class mcHandler
	{
			...
			foreach ($this->registeredClasses as $eventPoint => $ev) {
				foreach ($ev as $eventName => $eventDetails) {
					...
					if (file_exists($eventDetails[ 'filepath' ].$filename)) {
						...
						$eLiveSite = str_replace(CASTORCONFIG_ABSOLUTE_PATH, get_showtime('live_site').'/', $eventDetails[ 'filepath' ]);
		...
		public function triggerEvent($eventPoint, $eventArgs = null)
		{
			...
			if (!castor_cmsspecific_areweinadminarea()) {
				...
				if (isset($castor_access_control->controllable_patterns[$eventPoint])) {
					$check_access = true;
				}
			}
			...
			if (!empty($this->registeredClasses && isset($this->registeredClasses[$eventPoint]))) {
				foreach ($this->registeredClasses[$eventPoint] as $eventName => $eventDetails) {
					...
					if ($check_access) {
						if (!$castor_access_control->this_user_can($eventName)) {
					...
					if ($can_access && file_exists($eventDetails[ 'filepath' ].$filename)) {
						...
						$eLiveSite = str_replace(CASTORCONFIG_ABSOLUTE_PATH, get_showtime('live_site').'/', $ePointFilepath);
						$eLiveSite = str_replace(JRDS, '/', $eLiveSite);
						set_showtime('eLiveSite', $eLiveSite);
