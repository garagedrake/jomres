...
class jrportal_property_functions // Functions supplied as a class so that they can be extended easily without rewriting  the functions
{
	...
	public function getAllCastorProperties($limit = false, $limitstart = false)
	{
		$retResult = array();
		$caveat = '';
		if ($limit && $limitstart) {
		...
		FROM #__castor_propertys ' .$caveat;
		//echo $query;exit;
		...
		if (!empty($result)) {
			foreach ($result as $r) {
				if (is_numeric($r->property_region)) {
					...
					$property_region = jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$r->property_region, $castor_regions->get_region_name($r->property_region), true, false);
				} else {
					$property_region = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_REGION'.$r->property_region, $r->property_region, false);
				}
