...
class vat_number_validation
{
	...
	public function get_subject($type = '', $arguments = array())
	{
		...
		switch ($type) {
			case 'guest_registered_byguest_id':
					...
					$query = 'SELECT `enc_vat_number`,`vat_number_validated`,`enc_country`,`vat_number_validation_response` FROM #__castor_guests WHERE guests_uid = '.(int) $id.' AND property_uid = '.(int) $property_uid.' LIMIT 1';
					$result = doSelectSql($query, 2);
				if (!empty($result)) {
					$this->vat_number = $this->castor_encryption->decrypt($result[ 'enc_vat_number' ]);
					$this->vat_number_validated = $result[ 'vat_number_validated' ];
					$this->country = $this->castor_encryption->decrypt($result[ 'enc_country' ]);
			...
			case 'buyer_registered_byprofile_id':
					...
					$query = 'SELECT `enc_vat_number`,`vat_number_validated`,`enc_country`,`vat_number_validation_response` FROM #__castor_guest_profile WHERE cms_user_id = '.(int) $id.' LIMIT 1';
					$result = doSelectSql($query, 2);
				if (!empty($result)) {
					$this->vat_number = $this->castor_encryption->decrypt($result[ 'enc_vat_number' ]);
					$this->vat_number_validated = $result[ 'vat_number_validated' ];
	...
	public function save_subject($type = '', $arguments = array())
	{
		...
		switch ($type) {
			case 'guest_registered_byguest_id':
					$id = $arguments['guest_id'];
					$property_uid = $arguments['property_uid'];
					$query = "UPDATE #__castor_guests SET `vat_number_validated`='".$validated."',`enc_vat_number`='".$this->castor_encryption->encrypt($this->validation_messages[ 'clean_vat_no' ])."',`vat_number_validation_response`='".$messages."' WHERE guests_uid = '".(int) $id."' AND `property_uid` = ".(int) $property_uid;
			...
			case 'guest_registered_byprofile_id':
					$id = $arguments['profile_id'];
					$query = "UPDATE #__castor_guests SET `enc_vat_number`='".$this->castor_encryption->encrypt($this->validation_messages[ 'clean_vat_no' ])."', `vat_number_validated`='".$validated."',`vat_number_validation_response`='".$messages."' WHERE `mos_userid`=".(int) $id;
					doInsertSql($query, '');
					$query = "UPDATE #__castor_guest_profile SET `enc_vat_number`='".$this->castor_encryption->encrypt($this->validation_messages[ 'clean_vat_no' ])."',`vat_number_validated`='".$validated."',`vat_number_validation_response`='".$messages."' WHERE cms_user_id = ".(int) $id;
					doInsertSql($query, '');
				break;

			case 'property':
					$property_uid = $arguments['property_uid'];
					$query = "SELECT uid FROM #__castor_settings WHERE property_uid = '".(int) $property_uid."' and akey = 'property_vat_number'";
					$result = doSelectSql($query);
				if (empty($result)) {
					$query = "INSERT INTO #__castor_settings (property_uid,akey,value) VALUES ('".(int) $property_uid."','property_vat_number','".$this->validation_messages[ 'clean_vat_no' ]."')";
				} else {
					$query = "UPDATE #__castor_settings SET `value`='".$this->validation_messages[ 'clean_vat_no' ]."' WHERE property_uid = '".(int) $property_uid."' and akey = 'property_vat_number'";
				}
					doInsertSql($query, '');
					$query = "SELECT uid FROM #__castor_settings WHERE property_uid = '".(int) $property_uid."' and akey = 'vat_number_validated'";
					$result = doSelectSql($query);
				if (empty($result)) {
					$query = "INSERT INTO #__castor_settings (property_uid,akey,value) VALUES ('".(int) $property_uid."','vat_number_validated','".$validated."')";
				} else {
					$query = "UPDATE #__castor_settings SET `value`='".$validated."' WHERE property_uid = '".(int) $property_uid."' and akey = 'vat_number_validated'";
				}
					doInsertSql($query, '');
				break;
	...
	public function vies_check($vat_number)
	{
		...
		if ($vat_number != '') {
			...
			if (strlen($countryCode) != 2 || is_numeric(substr($countryCode, 0, 1)) || is_numeric(substr($countryCode, 1, 2))) {
				$this->validation_messages = array('result' => false, 'message' => jr_gettext('_CASTOR_VIES_VATCHECK_INCORRECT_SYNTAX', '_CASTOR_VIES_VATCHECK_INCORRECT_SYNTAX', false), 'clean_vat_no' => $countryCode.$vatNumber);
			...
			if ($result[ 'valid' ] != 'true') {
				$this->validation_messages = array('result' => false, 'message' => jr_gettext('_CASTOR_VIES_VATCHECK_INCORRECT_COULDNOTVALIDATE', '_CASTOR_VIES_VATCHECK_INCORRECT_COULDNOTVALIDATE', false), 'response_content' => $results, 'clean_vat_no' => $countryCode.$vatNumber);
				$this->vat_number_validated = '0';

				return false;
			} else {
				$this->validation_messages = array('result' => true, 'message' => jr_gettext('_CASTOR_VIES_VATCHECK_INCORRECT_VALIDATED', '_CASTOR_VIES_VATCHECK_INCORRECT_VALIDATED', false), 'response_content' => $results, 'clean_vat_no' => $countryCode.$vatNumber);
				$this->vat_number_validated = '1';
	...
	// The purpose of this method is for it to be passed a country code that's been pulled from a VAT number, and to convert it to a country code that Castor will recognise. Countries like Greece use EL instead of Castor' GR, so we need to pass EL, cycle through the xref array until we find that code, then return the key, which would be GR
	public function get_adjusted_country_code($country_code)
	{
		$xref_array = $this->get_euro_country_xref();
		foreach ($xref_array as $castor_country_code => $vat_number_country_code) {
			if ($country_code == $vat_number_country_code) {
				return $castor_country_code;
			}
		}
	}
