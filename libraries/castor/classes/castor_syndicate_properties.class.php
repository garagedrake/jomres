...
class castor_syndicate_properties
{
	...
	public function get_all_properties()
	{
		...
		$query = "SELECT id , `syndication_domain_id` , `view_property_url` , `booking_form_url` , `propertys_uid` , `name` , `multi_room_property` , `lat` , `long` , `metadescription` , `thumbnail_location` , `date_added` , `last_checked` , `approved` , `unapproval_reason` FROM #__castor_syndication_properties";
		$result = doSelectSql($query);
		if (!empty($result)) {
			foreach ($result as $r) {
	...
	public function get_random_properties($limit = 5, $multi_room_property = 1)
	{
		...
		/*if ( !empty($this->base_lat_long ) && $this->base_property_id > 0 )  {
			...
			if ( $this->base_property_id > 0 ) {
				$query = "SELECT distance , syndication_id FROM #__castor_syndication_distances WHERE property_id = ".(int)$this->base_property_id. " ORDER BY distance ASC LIMIT 200" ;
			...
			$query = "INSERT INTO #__castor_syndication_distances  ( `property_id` , `syndication_id` , `distance` ) VALUES ".$row_str;
			doInsertSql($query);
			}
		}*/
	...
	public function report_properties_display($properties)
	{
		...
		$response = $client->request('POST', "https://app.castor.net/castor/api/syndication/report/", [
			'form_params' => [
