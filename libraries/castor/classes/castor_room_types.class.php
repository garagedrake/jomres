...
class castor_room_types
{
	...
	public function get_all_room_types()
	{
		...
		$query = 'SELECT `room_classes_uid`, `room_class_abbv`, `room_class_full_desc`, `image` , `property_uid` FROM #__castor_room_classes ORDER BY `room_class_abbv` ';
	...
	public function get_room_type($room_classes_uid = 0)
	{
		...
		$query = 'SELECT `room_classes_uid`, `room_class_abbv`, `room_class_full_desc`, `image` , `property_uid` FROM #__castor_room_classes WHERE `room_classes_uid` = '.(int) $room_classes_uid;
		$result = doSelectSql($query);
	...
	public function save_room_type()
	{
		if ($this->room_type['room_classes_uid'] > 0) {
			$query = "UPDATE #__castor_room_classes
						SET
			...
			$query = "INSERT INTO #__castor_room_classes
								(
								`room_class_abbv` ,
		...
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPES_ABBV'.(int) $roomtype_id, $this->room_type['room_class_abbv'], true, $this->room_type['property_uid']);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPES_DESC'.(int) $roomtype_id, $this->room_type['room_class_full_desc'], true, $this->room_type['property_uid']);
	...
	public function update_roomtype_propertytype_xref_table($roomtype_id = 0, $ptype_xref = array())
	{
		...
		$query = 'DELETE FROM #__castor_roomtypes_propertytypes_xref WHERE `roomtype_id` = '.(int) $roomtype_id;
		if (!doInsertSql($query, false)) {
		...
		if (!empty($ptype_xref)) {
			foreach ($ptype_xref[0] as $ptype) {
				$query = 'INSERT INTO #__castor_roomtypes_propertytypes_xref (`roomtype_id`,`propertytype_id`) VALUES ('.(int) $roomtype_id.','.(int) $ptype.')';
	...
	public function delete_room_type($ids = array())
	{
		...
		foreach ($ids as $id) {
			$query = 'SELECT `room_classes_uid` FROM #__castor_rooms WHERE room_classes_uid = '.(int) $id;
			$result = doSelectSql($query);
			if (!empty($result)) {
				$success = false;
			} else {
				$query = "DELETE FROM #__castor_room_classes WHERE `room_classes_uid` = '".(int) $id."'";
				if (!doInsertSql($query, false)) {
					$success = false;
				}

				$query = "DELETE FROM #__castor_roomtypes_propertytypes_xref WHERE `roomtype_id` = '".(int) $id."'";
				if (!doInsertSql($query, false)) {
					$success = false;
				}
			}
	...
	public function get_xrefs($id = 0)
	{
		...
		$query = "SELECT `roomtype_id`, `propertytype_id` FROM #__castor_roomtypes_propertytypes_xref $clause ";
		$result = doSelectSql($query);
	...
	function get_all_room_type_images()
	{
		...
		$castor_media_centre_images->get_site_images('rmtypes');

		foreach ($castor_media_centre_images->site_images['rmtypes'] as $image) {
			$r = array();
