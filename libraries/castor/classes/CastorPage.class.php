...
################################
*/
// Adapted by Vince Wool for Joomla/Castor.

/**
 * Creates the Castor paging object.
...
class CastorPage
{
	...
	public function get_first_page_nav($user_link = '', $link_para = '')
	{
		...
		if (!$this->is_first_page() && $this->show_first_last) {
			$txt .= ' <a href="'.castorURL($this->page_name.'&plistpage=0'.$this->qry_str).$this->url_ptype_string.'">'.$user_link.'</a> ';
	...
	public function get_last_page_nav($user_link = '', $link_para = '')
	{
		...
		if (!$this->is_last_page() && $this->show_first_last) {
			$txt .= ' <a href="'.castorURL($this->page_name.'&plistpage='.($this->total_page - 1).$this->qry_str).$this->url_ptype_string.'">'.$user_link.'</a> ';
		} elseif ($this->show_first_last && $this->show_disabled_links) {
			$txt .= '&nbsp;'.$user_link;
		}
	...
	public function get_next_page_nav($user_link = '', $link_para = '')
	{
		...
		if (!$this->is_last_page() && $this->show_prev_next) {
			$txt .= ' <a href="'.castorURL($this->page_name.'&plistpage='.($this->page + 1).$this->qry_str).$this->url_ptype_string.'">'.$user_link.'</a> ';
	...
	public function get_scroll_prev_page_nav($user_link = '', $link_para = '')
	{
		...
		if ($this->page > $this->scroll_page && $this->show_scroll_prev_next) {
			$txt .= ' <a href="'.castorURL($this->page_name.'&plistpage='.($this->page - $this->scroll_page).$this->qry_str).$this->url_ptype_string.'">'.$user_link.'</a> ';
	...
	public function get_number_page_nav($link_para = '')
	{
		...
		for (; $i < $j + $scroll_page && $i < $this->total_records; ++$i) {
			if ($i == $this->page) {
				...
				$txt .= ' <a href="'.castorURL($this->page_name.'&plistpage='.$i.$this->qry_str).$this->url_ptype_string.'">'.($i + 1).'</a>&nbsp;';
			}
		}
	...
	{
		$this->page = intval(castorGetParam($_REQUEST, 'plistpage', 0));
	...
	public function getLimitBox()
	{
		...
		$limits = array();
		for ($i = 5; $i <= 30; $i += 5) {
			$limits[ ] = castorHTML::makeOption("$i");
		}
		$limits[ ] = castorHTML::makeOption('50');

		// build the html select list
		$link = $link."&amp;limit=' + this.options[selectedIndex].value + '&amp;page=".$this->page;
		$link = castorURL($link);

		return castorHTML::selectList($limits, 'limit', ' onchange="document.location.href=\''.$link.'\';"', 'value', 'text', $this->limit);
	}
}
