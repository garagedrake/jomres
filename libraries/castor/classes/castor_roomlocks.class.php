...
class castor_roomlocks
{
	...
	public function lock_room($room_uid, $date_range_string)
	{
		...
		if (!isset($dates_array[ $castorsession ])) {
			$dates_array[ $castorsession ] = array();
		}
		$dateRangeArray = explode(',', $date_range_string);
		foreach ($dateRangeArray as $date) {
			$dates_array[ $castorsession ][ $date ][ $room_uid ] = strtotime('now');
		}
		$this->save_lockfile_data($dates_array);
	}
	...
	public function unlock_room($room_uid, $date_range_string)
	{
		...
		$dateRangeArray = explode(',', $date_range_string);
		foreach ($dateRangeArray as $date) {
			unset($dates_array[ $castorsession ][ $date ][ $room_uid ]);
		}
		$this->save_lockfile_data($dates_array);
	}
	...
	public function unlock_all_rooms_for_session()
	{
		...
		$castorsession = get_showtime('castorsession');
		if (isset($dates_array[ $castorsession ])) {
			unset($dates_array[ $castorsession ]);
		}
