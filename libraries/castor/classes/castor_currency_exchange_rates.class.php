...
class castor_currency_exchange_rates
{
	...
	public function __construct()
	{
		...
		$this->exchange_rate_classfile = CASTOR_TEMP_ABSPATH.'exchangerates_'.$this->base_code.'.php';
	...
	public function update_exchange_rates()
	{
		...
		// We can safely assume that there's a GBP exchange rate (at least, for the forseeable future. If there isn't, then the world has probably been invaded by aliens and exchange rates in Castor is the least of our problems. Rule Britannia and all that. )
		$GBP_rate = $json->GBP;

		foreach ($currency_codes->codes as $k => $v) {
			if (isset($json->$k) && $k != 'GBP') {
				// Cross exchange rate calculation. As the base exchange rate in Castor is GBP we have to convert the 3rd change rate to GBP before saving it. The Base rate of Open Exchange Rates is USD
...
'
	...
	public function get_openexchangerates_rates()
	{
		...
		try {
			...
			$castor_user_feedback->construct_message(array('message'=>'Could not get currency exchange rates', 'css_class'=>'alert-danger alert-error'));
		}
