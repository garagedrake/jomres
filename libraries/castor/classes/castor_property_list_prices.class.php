...
class castor_property_list_prices
{
        ...
        if (!empty($property_uids)) {
            ...
            if (!empty($property_uids_to_query)) {
                ...
                if (isset($_REQUEST[ 'arrivalDate' ]) && $_REQUEST[ 'arrivalDate' ] != '') {
                    $this->arrivalDate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'arrivalDate', ''));
                ...
                if (isset($_REQUEST[ 'departureDate' ]) && $_REQUEST[ 'departureDate' ] != '') {
                    ...
                    $this->departureDate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'departureDate', ''));
                } elseif (!empty($tmpBookingHandler->tmpsearch_data)) {
                ...
                $query = 'SELECT property_uid, roomrateperday FROM #__castor_rates WHERE property_uid IN ('.castor_implode($property_uids).") AND roomrateperday > 0 $clause ";
                ...
                foreach ($property_uids_to_query as $property_uid) {
                        ...
                        if ($mrConfig[ 'is_real_estate_listing' ] == 0) {
                            if (isset($pricesFromArray[ $property_uid ])) {
                                ...
                                if ($mrConfig['poa_price'] != '') {
                                    ...
                                    $pre_text = '';
                                    $price = jr_gettext('_CASTOR_PRICE_ON_APPLICATION', '_CASTOR_PRICE_ON_APPLICATION', '', true, false);
                                    $post_text = '';
                                }
                            }
                        } else {
                    ...
                    if ($price == jr_gettext('_CASTOR_PRICE_ON_APPLICATION', '_CASTOR_PRICE_ON_APPLICATION', '', true, false)) {
                        $raw_price = -1;
