...
class jrportal_rates
{
	...
	public function get_rate()
	{
		...
		//get tarifftype name and description TODO: remove this, we already have this data in _castor_rates table..
		...
		//get the rates uids for this tarifftype_id TODO: remove this and move xref to _castor_rates table in tarifftype_id column..
		...
		$query = "SELECT
					...
					`property_uid` 
				FROM `#__castor_rates` 
				WHERE `rates_uid` IN (".castor_implode(array_keys($this->rates[$this->tarifftype_id])).") 
	...
	private function build_new_rates()
	{
		...
		foreach ($this->dates_rates as $epoch => $value) {
			...
			// Castor core will ensure that this index is set, but a remote system might not
	...
	private function insert_new_rates()
	{
		...
		foreach ($this->new_rates as $r) {
			...
			$query = "INSERT INTO #__castor_rates 
							(
							`rate_title`,
							`rate_description`,
							`validfrom`,
	...
	public function cleanup_existing_rate()
	{
		...
		$query = "DELETE FROM #__castor_rates WHERE `rates_uid` IN (".castor_implode(array_keys($this->rates[$this->tarifftype_id])).") ";
		...
		$query = "DELETE FROM #__jomcomp_tarifftype_rate_xref WHERE tariff_id IN (".castor_implode(array_keys($this->rates[$this->tarifftype_id])).") ";
	...
	public function delete_rate()
	{
		...
		$query = "DELETE FROM #__castor_custom_text WHERE `constant` = '_CASTOR_CUSTOMTEXT_TARIFF_TITLE_TARIFFTYPE_ID".(int)$this->tarifftype_id."' AND `property_uid` = ".(int)$this->property_uid;
	...
	public function get_tarifftype_id($rates_uid = 0)
	{
		...
		//get tarifftype id TODO: remove this, we already have this data in _castor_rates table..
	...
	private function create_tarifftype($rates_uid = 0)
	{
		...
		$query = "SELECT `rate_title`, `rate_description`, `roomclass_uid` FROM #__castor_rates WHERE `rates_uid` = ".(int)$rates_uid." AND `property_uid` = ".(int)$this->property_uid;
	...
	*/
	...
	private function update_rate_legacy()
	{
		...
		$query = "UPDATE #__castor_rates SET
	...
	public function get_unpopulated_room_type_ids($property_uid = 0)
	{
		...
		$query = "SELECT DISTINCT `roomclass_uid` FROM #__castor_rates WHERE property_uid = ".(int)$property_uid;
