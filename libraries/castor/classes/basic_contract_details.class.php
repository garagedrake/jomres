...
class basic_contract_details
{
	...
	public function gather_data($contract_uid = 0, $defaultProperty = 0)
	{
		...
		foreach ($contractData as $contract) {
			...
			//guest details
			$this->contract[$contract_uid]['guestdeets']['image'] = CASTOR_IMAGES_RELPATH.'noimage.svg';
			if (file_exists(CASTOR_IMAGELOCATION_ABSPATH.'userimages'.JRDS.'userimage_'.(int) $contract->mos_userid.'.jpg')) {
				$this->contract[$contract_uid]['guestdeets']['image'] = CASTOR_IMAGELOCATION_RELPATH.'userimages/userimage_'.(int) $contract->mos_userid.'_thumbnail.jpg';
			}

			$this->contract[$contract_uid]['guestdeets']['firstname'] = $this->castor_encryption->decrypt($contract->enc_firstname);
			$this->contract[$contract_uid]['guestdeets']['surname'] = $this->castor_encryption->decrypt($contract->enc_surname);
			$this->contract[$contract_uid]['guestdeets']['house'] = $this->castor_encryption->decrypt($contract->enc_house);
			$this->contract[$contract_uid]['guestdeets']['street'] = $this->castor_encryption->decrypt($contract->enc_street);
			$this->contract[$contract_uid]['guestdeets']['town'] = $this->castor_encryption->decrypt($contract->enc_town);
			if (is_numeric($this->castor_encryption->decrypt($contract->enc_county))) {
				$castor_regions = castor_singleton_abstract::getInstance('castor_regions');
				$this->contract[$contract_uid]['guestdeets']['county'] = jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$this->castor_encryption->decrypt($contract->enc_county), $castor_regions->get_region_name($this->castor_encryption->decrypt($contract->enc_county)), false);
			} else {
				$this->contract[$contract_uid]['guestdeets']['county'] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_REGION'.$this->castor_encryption->decrypt($contract->enc_county), $this->castor_encryption->decrypt($contract->enc_county), false);
			}
			$this->contract[$contract_uid]['guestdeets']['country'] = getSimpleCountry($this->castor_encryption->decrypt($contract->enc_country));
			$this->contract[$contract_uid]['guestdeets']['country_code'] = $this->castor_encryption->decrypt($contract->enc_country);

			$this->contract[$contract_uid]['guestdeets']['postcode'] = $this->castor_encryption->decrypt($contract->enc_postcode);
			$this->contract[$contract_uid]['guestdeets']['tel_landline'] = $this->castor_encryption->decrypt($contract->enc_tel_landline);
			$this->contract[$contract_uid]['guestdeets']['tel_mobile'] = $this->castor_encryption->decrypt($contract->enc_tel_mobile);
			$this->contract[$contract_uid]['guestdeets']['email'] = $this->castor_encryption->decrypt($contract->enc_email);
			$this->contract[$contract_uid]['guestdeets']['discount'] = $contract->discount;
			$this->contract[$contract_uid]['guestdeets']['vat_number'] = $this->castor_encryption->decrypt($contract->enc_vat_number);
		...
		$query = "SELECT  
					...
					FROM #__castor_room_bookings a 
					LEFT JOIN #__castor_rooms b ON a.room_uid = b.room_uid
					WHERE a.contract_uid = '" .(int) $contract_uid."' AND a.property_uid = '".(int) $defaultProperty."' ";
		...
		if (!empty($tariffs) && $room_and_tariff_info != array(0 => '')) {
			$query = 'SELECT `rates_uid`,`rate_title` FROM #__castor_rates WHERE `rates_uid` IN ('.castor_implode($tariffs).') ';
			...
			foreach ($room_and_tariff_info as $e) {
				...
				if (isset($tariffNames[$rt[1]])) {
					$this->contract[$contract_uid]['roomdeets'][$rt[0]]['rate_title'] = castor_decode($tariffNames[$rt[1]]);
				} else {
		...
		if (!empty($guesttype_ids)) {
			$query = 'SELECT `id`, `type` FROM #__castor_customertypes WHERE id IN ('.castor_implode($guesttype_ids).') ';
			$guesttype_titles = doSelectSql($query);

			foreach ($guesttype_titles as $g) {
				$guesttypeNames[$g->id] = jr_gettext('_CASTOR_CUSTOMTEXT_GUESTTYPE'.$g->id, stripslashes($g->type), false);
			}
		...
		if (!empty($extraOptionsArray)) {
			...
			if (!empty($extraOptionsArray)) {
				$query = "SELECT 
								...
								price,
								tax_rate 
							FROM #__castor_extras 
							WHERE property_uid = '" .(int) $defaultProperty."' AND uid IN (".castor_implode($extraOptionsArray).') 
							ORDER BY name ';
				$extrasList = doSelectSql($query);

				foreach ($extrasList as $e) {
					$this->contract[$contract_uid]['extradeets'][$e->uid]['uid'] = $e->uid;
					$this->contract[$contract_uid]['extradeets'][$e->uid]['name'] = jr_gettext('_CASTOR_CUSTOMTEXT_EXTRANAME'.$e->uid, castor_decode($e->name));
