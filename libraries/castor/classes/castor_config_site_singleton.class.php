...
// ################################################################
	...
	class castor_config_site_singleton
	{
		public function __construct()
		{
			...
			$this->config_file = CASTORCONFIG_ABSOLUTE_PATH.JRDS.CASTOR_ROOT_DIRECTORY.JRDS.'configuration.php';
...
'
				...
				if (!array_key_exists($k, $this->config)) {
					$query = "INSERT INTO #__castor_site_settings (akey,value) VALUES ('".$k."','".$v."')";
...
'
				...
				$query = "UPDATE #__castor_site_settings SET `value` = '".$v."' WHERE `akey` = '".$k."' ";
				$castor_db->setQuery($query);
				$castor_db->query();
		...
		public function init()
		{
			...
			if (file_exists($this->config_file)) {
				...
				// We need to bypass doSelectSql here because doSelectSql does $siteConfig = castor_singleton_abstract::getInstance('castor_config_site_singleton'); too, and that'll cause a fatal error.
				$castor_db = castor_singleton_abstract::getInstance('castor_database');
				$castorConfig_dbprefix = get_showtime('dbprefix');
				$castorConfig_db = get_showtime('db');

				// Castor probably hasn't been installed yet, does the site settings table exist yet?
				$tablesFound = false;
				$query = 'SHOW TABLES';
				$castor_db->setQuery($query);
				$result = $castor_db->loadObjectList();
				$string = 'Tables_in_'.$castorConfig_db;
				if (!empty($result)) {
					foreach ($result as $r) {
						if (strstr($r->$string, $castorConfig_dbprefix.'castor_site_settings')) {
				...
				if ($this->site_settings_table_exists) {
					$query = 'SELECT akey,value FROM #__castor_site_settings';
					$castor_db->setQuery($query);
					$castor_db->loadObjectList();
					if (!empty($castor_db->result)) {
						foreach ($castor_db->result as $setting) {
							$akey = $setting->akey;
							$value = $setting->value;
				...
				if (empty($castor_db->result)) {
					...
					if (!$this->site_settings_table_exists) {
						...
						//return false;
						//we`re installing Castor for the first time
