...
class castor_media_centre_images
{
	protected $jrConfig;
	...
	public function __construct()
	{
		...
		$this->multi_query_images [ 'noimage-large' ] = get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/assets/images/noimage.svg';
		$this->multi_query_images [ 'noimage-medium' ] = get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/assets/images/noimage.svg';
		$this->multi_query_images [ 'noimage-small' ] = get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/assets/images/noimage_small.gif';
	}
	...
	public function get_images($property_id = null)
	{
		...
		if (isset($castor_room_types->property_specific_room_type[$property_id])) {
			//add default images for each room if no other images are set
			foreach ($castor_room_types->property_specific_room_type[$property_id] as $room_class_uid => $val) {
	...
	public function get_images_multi($property_uids)
	{
		...
		if ($this->use_db) { //get images from db
			...
			foreach ($property_uids as $property_id) {
				...
				$rel_path = CASTOR_IMAGELOCATION_RELPATH.$property_id.'/';
	...
	private function get_images_multi_from_db($property_uids = array())
	{
		...
		$query = "SELECT 
						...
						`filename`, 
						`version` 
					FROM #__castor_images 
					WHERE `property_uid` IN (".castor_implode($property_uids).") 
			...
			if (isset($resource_types[$r->resource_type])) {
				if ($r->version == 'large') {
					$this->multi_query_images [ $r->property_uid ] [ $r->resource_type ] [ $r->resource_id ] [$r->filename] ['large'] =
						CASTOR_IMAGELOCATION_RELPATH.$r->property_uid.'/'.$r->resource_type.'/'.$r->resource_id.'/'.$r->filename;
				} elseif ($r->version == 'small') {
					$this->multi_query_images [ $r->property_uid ] [ $r->resource_type ] [ $r->resource_id ] [$r->filename] ['small'] =
						CASTOR_IMAGELOCATION_RELPATH.$r->property_uid.'/'.$r->resource_type.'/'.$r->resource_id.'/thumbnail/'.$r->filename;
				} else {
					$this->multi_query_images [ $r->property_uid ] [ $r->resource_type ] [ $r->resource_id ] [$r->filename] ['medium'] =
						CASTOR_IMAGELOCATION_RELPATH.$r->property_uid.'/'.$r->resource_type.'/'.$r->resource_id.'/'.$r->version.'/'.$r->filename;
				}
			}
		}
	...
	//do castor specific stuff for the newly uploaded image
	public function handle_uploaded_image($property_uid = 0, $resource_type = '', $resource_id = '', $file_name = '', $version = '', $resource_id_required = true)
	{
		...
		if ($this->optimize_images) {
			$castor_media_centre_images_optimizer = castor_singleton_abstract::getInstance('castor_media_centre_images_optimizer');
			$castor_media_centre_images_optimizer->optimize(CASTOR_IMAGELOCATION_ABSPATH.$filepath);
		}
	...
	private function save_image_details_to_db($property_uid = 0, $resource_type = '', $resource_id = '', $file_name = '', $version = '', $resource_id_required = true)
	{
		...
		$query = "INSERT INTO #__castor_images (
												`property_uid`, 
												`resource_type`, 
												`resource_id`, 
												`filename`, 
	...
	public function delete_image($property_uid = 0, $resource_type = '', $resource_id = '', $file_name = '', $abs_path = '', $resource_id_required = true)
	{
		...
		if ($passed) {
			if ($this->use_db) {
				$query = "DELETE FROM #__castor_images 
	...
	public function delete_all_images($property_uid = 0)
	{
		...
		if ($this->use_db) {
			$query = "DELETE FROM #__castor_images WHERE `property_uid` = ".(int)$property_uid;
		...
		//mount castor filesystem
		$filesystem = castor_singleton_abstract::getInstance('castor_filesystem')->getFilesystem();
