...
class castor_user_budget
{
		...
		// This can be called via either the "search" task, or more complicatedly through ajax search. If it is, instead of having a simple js function to call we need to call a different one, using the form name variable. Incidently, this is the same onclick function that's used to return to search results. If we didn't do this, clicking on the dropdown after doing an ajax search would reload the current page, e.g. "viewproperty" or somesuch.
			...
			if (((defined('CASTOR_NOHTML') && CASTOR_NOHTML == 0) || get_showtime('task') == 'ajax_search_filter')) {
				if ($this->get_budget(true) != '') {
					$r = array();
					$r[ 'TITLE' ] = jr_gettext('_CASTOR_COM_A_RESET', '_CASTOR_COM_A_RESET', false, false);
					if (!isset($_REQUEST['ajax_search_form_name'])) {
						$r[ 'ONCLICK' ] = 'onclick="set_budget( 0 , true , \'\')"';
					} else {
						$formname = castorGetParam($_REQUEST, 'ajax_search_form_name', '');
	...
	public function get_price_ranges()
	{
		...
		if (file_exists(CASTOR_TEMP_ABSPATH.'price_ranges.php')) {
			$last_modified = filemtime(CASTOR_TEMP_ABSPATH.'price_ranges.php');
			$seconds_timediff = time() - $last_modified;
			if ($seconds_timediff > 3600) {
				unlink(CASTOR_TEMP_ABSPATH.'price_ranges.php');
			} else {
				$prices = json_decode(file_get_contents(CASTOR_TEMP_ABSPATH.'price_ranges.php'));
			}
		}

		if (!file_exists(CASTOR_TEMP_ABSPATH.'price_ranges.php') || empty($prices)) {
			$query = "SELECT DISTINCT roomrateperday FROM #__castor_rates WHERE roomrateperday > '0' LIMIT 100";
			$rates = doSelectSql($query);

			// Is Jintour installed?
			$tour_prices = array();
			if (function_exists('jintour_get_tours_for_arrdep_dates')) {
				$query = 'SELECT DISTINCT (CASE WHEN `price_adults` > 0 THEN `price_adults` ELSE `price_kids` END) AS price FROM #__castor_jintour_tours LIMIT 100';
			...
			file_put_contents(CASTOR_TEMP_ABSPATH.'price_ranges.php', json_encode($prices));
		}
