...
// ################################################################
	...
	class castor_shortcode_parser
	{
		...
		public function __construct()
		{
			...
			$this->shortcodes_file = CASTOR_TEMP_ABSPATH.'shortcodes.php';
		}
			...
			if (!file_exists($this->shortcodes_file) || $force) {
				...
				foreach ($MiniComponents->registeredClasses as $eventPoint => $ev) {
					foreach ($ev as $eventName => $eventDetails) {
						...
						if ( isset($eventDetails[ 'filepath' ]) && file_exists($eventDetails[ 'filepath' ].$filename)) {
