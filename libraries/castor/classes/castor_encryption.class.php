...
require_once(CASTORPATH_BASE.'libraries'.JRDS.'php-encryption'.JRDS.'autoload.php');
...
class castor_encryption
{
	...
	public function __construct()
	{
		...
		if (!isset($jrConfig['secret_key_location']) || $jrConfig['secret_key_location'] == '') {
			$this->key_location = CASTORPATH_BASE;
		} else {
...
';
	 ...
	 */
		...
		try {
			...
			if (castor_cmsspecific_areweinadminarea()) { // We can continue to allow Castor to operate in admin area, but we must output a big fat warning
				if (!AJAXCALL) {
