...
// ################################################################
	 ...
	 *
	 * Allows Castor to determine gateway settings
	 *
	 */
	...
	public function get_settings_for_property_uid($property_uid = 0)
	{
		...
		if (!isset($MiniComponents->miniComponentData['00509'])) { // No gateways installed
			...
			if (!empty($gateway_array)) {
				...
				$query = "SELECT `plugin`,`setting`,`value` FROM #__castor_pluginsettings WHERE `plugin` IN ( ".castor_implode($installed_gateways, false).' ) AND prid = 0';
				...
				if ($property_uid > 0) {
					$query = "SELECT `plugin`,`setting`,`value` FROM #__castor_pluginsettings WHERE `plugin` IN ( ".castor_implode($installed_gateways, false).' ) AND prid = '.(int)$property_uid;
					$plugin_settings = doSelectSql($query);
