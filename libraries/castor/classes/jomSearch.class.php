...
// ################################################################
	...
	class jomSearch
	{
		...
		public function __construct($calledByModule, $includedInModule = false)
		{
			...
			if (!isset($calledByModule) || empty($calledByModule) && isset($_REQUEST['calledByModule'])) {
				$this->calledByModule = $calledByModule = castorGetParam($_REQUEST, 'calledByModule', 'mod_jomsearch_m0');
			}
			...
			if (strlen($calledByModule) > 0 && !isset($this->templateFile)) {
				...
				if (isset($_REQUEST['search_widget']) && is_dir(CASTOR_COREPLUGINS_ABSPATH . 'search_widget' . JRDS . $_REQUEST['search_widget'] . JRDS . 'bootstrap' . castor_bootstrap_version())) {
					$this->templateFilePath = CASTOR_COREPLUGINS_ABSPATH . 'search_widget' . JRDS . $_REQUEST['search_widget'] . JRDS . 'bootstrap' . castor_bootstrap_version();
					$this->templateFile = 'index.html';
				} else {
					$this->templateFilePath = CASTORCONFIG_ABSOLUTE_PATH . JRDS . 'modules' . JRDS . $calledByModule . JRDS . $calledByModule;
			...
			$vals = castor_cmsspecific_getSearchModuleParameters($calledByModule);
			...
			*/
			...
			$query = "SELECT DISTINCT property_country FROM  #__castor_propertys WHERE published = '1' ORDER BY  property_country ASC";
			...
			$query = "SELECT DISTINCT property_region,property_country FROM #__castor_propertys WHERE published = '1' ORDER BY property_region ASC";
			...
			$query = "SELECT DISTINCT (CASE WHEN (a.propertys_uid = b.property_uid 
															AND b.constant = '_CASTOR_CUSTOMTEXT_PROPERTY_TOWN' 
			...
			if ($result && !empty($result)) {
				foreach ($result as $feature) {
					if (!empty($feature[ 'title' ])) {
						$this->prep[ 'features' ][ ] = array('id' => $feature[ 'id' ], 'image' => CASTOR_IMAGELOCATION_RELPATH.'pfeatures/'.$feature[ 'image' ], 'title' => $feature[ 'title' ], 'description' => $feature[ 'description' ]);
					}
				}
			}
			...
			if ($result && !empty($result)) {
				foreach ($result as $rtype) {
					if (!empty($rtype[ 'id' ]) && !empty($rtype[ 'title' ])) {
						$this->prep[ 'rtypes' ][ ] = array('id' => $rtype[ 'id' ], 'image' => CASTOR_IMAGELOCATION_RELPATH.'rmtypes/'.$rtype[ 'image' ], 'title' => $rtype[ 'title' ], 'description' => $rtype[ 'description' ]);
					}
				}
			}
		...
		public function jomSearch_showresults()
		{
			...
			if (!empty($tmpResultsArray)) {
				...
				echo jr_gettext('_CASTOR_FRONT_NORESULTS', '_CASTOR_FRONT_NORESULTS', $editable = true, $islink = false);
			}
		...
		public function jomSearch_country()
		{
			...
			if (!empty($filter) && $property_ors) {
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND property_country LIKE '$filter' $property_ors ";
		...
		public function jomSearch_region()
		{
			...
			if (!empty($this->filter[ 'region' ]) && $property_ors) {
				...
				if ($filter != '%') {
					$this->filter[ 'region' ] = castor_cmsspecific_stringURLSafe($this->filter[ 'region' ]);
				...
				if ($this->filter[ 'region' ] == "%") {
					$this->filter[ 'region' ] = $this->filter[ 'region' ]."'";
				}
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND property_region LIKE '".$this->filter[ 'region' ]." $property_ors ";
		...
		public function jomSearch_town()
		{
			...
			if (!empty($filter) && $property_ors) {
				if ($filter != '%') {
					$filter = castor_cmsspecific_stringURLSafe($filter);
					$filter = str_replace('-', '%', $filter);
				}
				$query = "SELECT a.propertys_uid  
										FROM #__castor_propertys a
										LEFT JOIN #__castor_custom_text b ON (
		...
		public function jomSearch_description()
		{
			...
			if (!empty($filter) && $property_ors) {
				...
				$query = 'SELECT propertys_uid FROM #__castor_propertys ';
				...
				foreach ($words as $word) {
					$wheres2 = array();
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_NAME%' AND LOWER(a.customtext) LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_STREET%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_TOWN%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_POSTCODE%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_DESCRIPTION%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_CHECKINTIMES%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_AREAACTIVITIES%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_DIRECTIONS%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_AIRPORTS%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_OTHERTRANSPORT%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang'";
					$wheres2[ ] = "a.constant LIKE '_CASTOR_CUSTOMTEXT_ROOMTYPE_DISCLAIMERS%' AND LOWER(a.customtext)  LIKE '%$word%' AND a.language = '$lang' ";
					$wheres[ ] = implode(' OR ', $wheres2);
				}
				$where = '('.implode(($phrase == 'all' ? ') AND (' : ') OR ('), $wheres).')';
				$query = 'SELECT a.property_uid FROM #__castor_custom_text a, #__castor_propertys b ';
		...
		public function jomSearch_features()
		{
			...
			if (!empty($filter) && $property_ors) {
				...
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND property_features LIKE $st  $property_ors ";
				$this->resultBucket = doSelectSql($query);
			}
			$this->sortResult();
		}
		...
		public function jomSearch_roomtypes()
		{
			...
			$property_ors = $this->ors;
			if (!empty($filter) && $property_ors) {
				$query = "SELECT propertys_uid FROM #__castor_rooms WHERE  room_classes_uid LIKE '$filter'  $property_ors ";
				$this->resultBucket = doSelectSql($query);
			}
			$this->sortResult();
		}
		...
		public function jomSearch_ptypes()
		{
			...
			$property_ors = $this->ors;
			if (!empty($filter) && $property_ors) {
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND ptype_id LIKE '$filter'  $property_ors ";
				$this->resultBucket = doSelectSql($query);
			}
			$this->sortResult();
		}
		...
		public function jomSearch_categories()
		{
			...
			if (!empty($filter) && $filter > 0 && $property_ors) {
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND cat_id = $filter  $property_ors ";
				$this->resultBucket = doSelectSql($query);
			}
			$this->sortResult();
		}
		...
		public function jomSearch_guestnumber()
		{
			...
			$res = array();
			if (!empty($filter) && $property_ors) {
				$query = 'SELECT property_uid FROM #__castor_rates WHERE maxpeople '.$clause.' '.$filter.' '.$property_ors;
				$result = doSelectSql($query);
		...
		public function jomSearch_sleeps_adults()
		{
			...
			$comparison = '=';
			if ($jrConfig[ 'guestnumbersearch' ] == 'lessthan') {
				$comparison = '<=';
			}
			if ($jrConfig[ 'guestnumbersearch' ] == 'greaterthan') {
				$comparison = '>=';
			}

			if (!empty($filter) && $property_ors) {
				$result_array = array();
				$query = "SELECT property_uid ,value FROM #__castor_settings WHERE akey = 'accommodates_adults' AND value ".$comparison." ".(int)$filter." ".$property_ors;
		...
		public function jomSearch_sleeps_children()
		{
			...
			if (!empty($filter) && $property_ors) {
				...
				$result_array = array();
				$query = "SELECT property_uid ,value FROM #__castor_settings WHERE akey = 'accommodates_children' AND value ".$comparison." ".(int)$filter." ".$property_ors;
				$result = doSelectSql($query);
		...
		public function jomSearch_stars()
		{
			...
			$property_ors = $this->ors;
			if (!empty($filter) && $property_ors) {
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE $filter $property_ors";
				$this->resultBucket = doSelectSql($query);
			}
			$this->sortResult();
		}
			...
			if (!empty($filter) && $property_ors) {
				if ($filter != '%') {
					...
					if (isset($_REQUEST[ 'arrivalDate' ]) && $_REQUEST[ 'arrivalDate' ] != '') {
						$arrivalDate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'arrivalDate', ''));
					...
					if (is_array($filter)) {
						$query = 'SELECT property_uid FROM #__castor_rates WHERE roomrateperday >= '.$filter[ 'from' ].' AND roomrateperday <= '.$filter[ 'to' ]." $clause $property_ors ";
					} else {
						$query = "SELECT property_uid FROM #__castor_rates WHERE roomrateperday LIKE '%' $clause $property_ors ";
					}
					$result = doSelectSql($query);
					...
					if (is_array($filter)) {
						...
						$query = 'SELECT propertys_uid FROM #__castor_propertys WHERE property_key > '.$filter[ 'from' ].' AND property_key <= '.$filter[ 'to' ]."  $property_ors ";
		...
		public function jomSearch_availability()
		{
			...
			if ($arrivalDate != '' && $departureDate != '' && $property_ors) {
				...
				$query = 'SELECT propertys_uid, room_uid, room_classes_uid, max_people FROM #__castor_rooms WHERE propertys_uid IN ('.castor_implode(end($this->propertys_uid)).') ';
				...
				$query = 'SELECT property_uid,room_uid,`date` FROM #__castor_room_bookings WHERE property_uid IN ('.castor_implode(end($this->propertys_uid)).') AND `date` IN ('.castor_implode($dateRangeArray, false).') ';
				...
				foreach (end($this->propertys_uid) as $property) {
					...
					// $query="SELECT room_uid,room_classes_uid FROM #__castor_rooms WHERE propertys_uid = '".(int)$property."'";
					...
					foreach ($roomsList as $room) {
						...
						if ($ok) {
							...
							// $query="SELECT room_uid FROM #__castor_room_bookings WHERE room_uid = '".(int)$room->room_uid."' AND property_uid ='".(int)$property."'  AND (".$st.")";
							// $datesList=doSelectSql($query);
		...
		public function jomSearch_autocomplete()
		{
			...
			$autocomplete_value = castorGetParam($_REQUEST, 'autocomplete_value', '');
			...
			if (isset($castor_countries->countries[$autocomplete_value])) {
				$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND property_country = '$autocomplete_value' $property_ors ";
				$this->resultBucket = doSelectSql($query);
			} else {
				...
				foreach ($castor_regions->regions as $region) {
					if ($region['regionname'] == $autocomplete_value) {
						...
						$query = "SELECT propertys_uid FROM #__castor_propertys WHERE published = '1' AND property_region = '".$region_id."' $property_ors ";
						$found = true;
						$this->resultBucket = doSelectSql($query);
					}
				}
	...
	function prepOccupancyLevels()
	{
		...
		$castor_occupancy_levels = new castor_occupancy_levels(0);
		$all_occupancy_levels = $castor_occupancy_levels->get_max_occupancy_levels();
	...
	function prepGeographicSearch()
	{
		...
		foreach ($propertyLocations as $location) {
			...
			if (is_numeric($location->property_region)) {
				$castor_regions = castor_singleton_abstract::getInstance('castor_regions');
				$r[ 'region' ] = jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$location->property_region, $castor_regions->get_region_name($location->property_region), false);
			} else {
	...
	function prepGuestnumberSearch()
	{
		...
		if ($jrConfig[ 'guestnumbersearch' ] == 'equal') {
			$query = 'SELECT DISTINCT maxpeople FROM #__castor_rates ORDER by maxpeople ASC';
			$rateList = doSelectSql($query);
			foreach ($rateList as $rate) {
				$result[ ] = $rate->maxpeople;
			}
		} else {
			$query = 'SELECT MAX(maxpeople) FROM #__castor_rates LIMIT 1';
	...
	function prepFeatureSearch()
	{
		...
		$query = "SELECT property_features FROM #__castor_propertys WHERE published = '1'";
	...
	function prepRoomtypeSearch()
	{
		...
		$roomTypeList = $castor_room_types->room_types;


		if ($jrConfig['frontend_room_type_editing_show_property_room_types_in_search_options'] == "1") {
			if (!empty($castor_room_types->property_specific_room_types)) {
				foreach ($castor_room_types->property_specific_room_types as $property) {
	...
	function prepAvailabilitySearch()
	{
		...
		if (isset($_REQUEST[ 'arrivalDate' ]) && castorGetParam($_REQUEST, 'arrivalDate', '') != '') {
			...
			if (isset($_REQUEST[ 'departureDate' ])) {
				$departureDate = JSCalConvertInputDates(castorGetParam($_REQUEST, 'departureDate', ''), $siteCal = true);
			} else {
	...
	function prepPropertyCategoriesSearch()
	{
		...
		if (!empty($castor_property_categories->property_categories)) {
	...
	function prepPriceRangeSearch($increments = 10)
	{
		...
		$query = 'SELECT DISTINCT roomrateperday FROM #__castor_rates,#__castor_propertys WHERE #__castor_rates.property_uid = #__castor_propertys.propertys_uid AND #__castor_propertys.published = 1 ORDER by #__castor_rates.roomrateperday';
		$rateList = doSelectSql($query);

		$query = 'SELECT DISTINCT property_key FROM #__castor_propertys WHERE published = 1 ORDER by property_key';
