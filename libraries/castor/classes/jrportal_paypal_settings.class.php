...
class jrportal_paypal_settings
{
	...
	{
		$query = "SELECT setting,value FROM #__castor_pluginsettings WHERE prid = 0 AND plugin = 'paypal'";
	...
	public function save_paypal_settings()
	{
		...
		foreach ($values as $k => $v) {
			$query = "SELECT id FROM #__castor_pluginsettings WHERE prid = 0 AND plugin = 'backend_paypal_settings' AND setting = '$k'";
			$settingList = doSelectSql($query);
			if (!empty($settingList)) {
				foreach ($settingList as $set) {
					$id = $set->id;
				}
				$query = "UPDATE #__castor_pluginsettings SET `value`='$v' WHERE prid = 0 AND plugin = 'backend_paypal_settings' AND setting = '$k'";
				doInsertSql($query, '');
			} else {
