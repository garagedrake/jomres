...
class castor_ribbon_generator
{
	...
	{
		$query = 'SELECT `property_uid` FROM #__castor_settings WHERE (`akey` = "lastminuteactive" AND `value` = 1) OR (`akey` = "wisepriceactive" AND `value` = 1) ';
	...
	private function find_text()
	{
		if ($this->review_score > 5) {
			...
			$this->ribbon_text = jr_gettext('CASTOR_RIBBON_TEXT_DISCOUNTED', 'CASTOR_RIBBON_TEXT_DISCOUNTED', false);
