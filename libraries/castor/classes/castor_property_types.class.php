...
class castor_property_types
{
	...
	public function __construct()
	{
		...
		$this->property_type['marker_image'] = $castor_media_centre_images->multi_query_images['noimage-small'];
	}
	...
	public function get_all_property_types()
	{
		...
		$query = 'SELECT `id`, `ptype`, `ptype_desc`, `published`, `order`, `mrp_srp_flag`, `marker`, `has_stars` FROM #__castor_ptypes ORDER BY `ptype` ASC';
		...
		foreach ($result as $r) {
			...
			$this->property_types[$r->id]['ptype'] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTYTYPE'.(int) $r->id, $r->ptype, false); // property type name;
	...
	public function get_property_type($id = 0)
	{
		...
		$query = 'SELECT `id`, `ptype`, `ptype_desc`, `published`, `order`, `mrp_srp_flag`, `marker`, `has_stars` FROM #__castor_ptypes WHERE `id` = '.(int) $id;
		$result = doSelectSql($query);
		...
		foreach ($result as $r) {
			...
			$this->property_type['ptype'] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTYTYPE'.(int) $r->id, $r->ptype, false); // property type name;
			...
			if (is_dir(CASTOR_IMAGELOCATION_ABSPATH.'markers') &&
				is_file(CASTOR_IMAGELOCATION_ABSPATH.'markers'.JRDS.$this->property_type['marker'])
				) {
				$this->property_type['marker_image'] = CASTOR_IMAGELOCATION_RELPATH.'markers/'.$this->property_type['marker'];
			} elseif (CASTOR_IMAGES_ABSPATH.'markers'.JRDS.$this->property_types['marker']) {
				$this->property_type['marker_image'] = CASTOR_IMAGES_RELPATH.'markers/'.$this->property_type['marker'];
			}
		}
	...
	public function save_property_type()
	{
		if ($this->property_type['id'] > 0) {
			...
			$query = "INSERT INTO #__castor_ptypes 
								(
								`ptype`,
								`ptype_desc` ,
								`mrp_srp_flag`,
	...
	public function delete_property_type($ids = array())
	{
		...
		foreach ($ids as $id) {
			$query = 'SELECT `propertys_uid` , `ptype_id` FROM #__castor_propertys WHERE `ptype_id` = '.(int) $id;
			$result = doSelectSql($query);
			...
			if (!empty($result)) {
				...
				$query = "DELETE FROM #__castor_ptypes WHERE `id` = ".(int) $id;
				if (!doInsertSql($query, false)) {
					$success = false;
				}
			}
	...
	public function publish_property_type($id = 0)
	{
		...
		if ($this->property_type['id'] > 0) {
			...
			$query = 'UPDATE #__castor_ptypes SET `published` = '.$publish.' WHERE `id` = '.$this->property_type['id'];
	...
	public function ptype_is_used($id = 0, $include_unpublished_properties = true)
	{
		...
		if ($include_unpublished_properties) {
			...
			$query = 'SELECT `propertys_uid` , `ptype_id` FROM #__castor_propertys WHERE `ptype_id` = '.(int) $id . " AND published = 1";
		}
	...
	public function get_property_types_with_no_room_types_assigned($show_link_to_room_types = false)
	{
		...
		foreach ($this->property_types as $t) {
			if ($t['mrp_srp_flag'] == 0 || $t['mrp_srp_flag'] == 1) {
				if (!isset($castor_room_types->all_ptype_rtype_xrefs[$t['id']])) {
		...
		if (!empty($property_types_requiring_attention)) {
			...
			$o['_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES'] = jr_gettext('_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES', '_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES', false);
			$o['_CASTOR_PROPERTYTYPE_FLAG_BOTH_COLHEAD'] = jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_BOTH_COLHEAD', '_CASTOR_PROPERTYTYPE_FLAG_BOTH_COLHEAD', false);
			$o['_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES_LIST_ROOM_TYPES'] = jr_gettext('_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES_LIST_ROOM_TYPES', '_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES_LIST_ROOM_TYPES', false);
			$o['HPTYPE'] = jr_gettext('_CASTOR_FRONT_PTYPE', '_CASTOR_FRONT_PTYPE', false);

			$o['LINK'] = '';
			if ($show_link_to_room_types) {
				$o['LINK'] = '<a href="'.CASTOR_SITEPAGE_URL_ADMIN.'&task=listGlobalroomTypes'.'" class="btn btn-primary">'.jr_gettext('_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES_LINK_TEXT', '_CASTOR_PROPERTY_TYPES_WITHOUT_ROOM_TYPES_LINK_TEXT', false).'</a>';
			}
			...
			foreach ($property_types_requiring_attention as $property_type) {
				...
				if ($property_type['process'] == 0) {
					$r['MANAGEMENT_PROCESS'] = jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_HOTEL', '_CASTOR_PROPERTYTYPE_FLAG_HOTEL', false);
				} else {
					$r['MANAGEMENT_PROCESS'] = jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_VILLA', '_CASTOR_PROPERTYTYPE_FLAG_VILLA', false);
				}
				$rws[] = $r;
			}
	...
	function get_all_property_type_images()
	{
		...
		$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
		$castor_media_centre_images->get_site_images('markers');
		
		if (isset($castor_media_centre_images->site_images['markers'])) {
			foreach ($castor_media_centre_images->site_images['markers'] as $image) {
				$r = array();
	...
	public function getPropertyTypeDropdown($selected = '', $extended = false, $is_disabled = false, $input_name = 'propertyType')
	{
		...
		foreach ($this->property_types as $p) {
			if ($p['published'] == 1) {
				$ptype = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTYTYPE'.(int) $p['id'], $p['ptype'], false);

				if ($extended) {
					switch ($p['mrp_srp_flag']) {
						case 1:
							$ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_VILLA', '_CASTOR_PROPERTYTYPE_FLAG_VILLA', false);
							break;
						case 2:
							$ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_BOTH', '_CASTOR_PROPERTYTYPE_FLAG_BOTH', false);
							break;
						case 3:
							$ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_TOURS', '_CASTOR_PROPERTYTYPE_FLAG_TOURS', false);
							break;
						case 4:
							$ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_REALESTATE', '_CASTOR_PROPERTYTYPE_FLAG_REALESTATE', false);
							break;
                        case 5:
                            $ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_HIRE', '_CASTOR_PROPERTYTYPE_FLAG_HIRE', false);
                            break;
						default:
							$ptype .= ' - '.jr_gettext('_CASTOR_PROPERTYTYPE_FLAG_HOTEL', '_CASTOR_PROPERTYTYPE_FLAG_HOTEL', false);
							break;
					}
				}

				$options[] = castorHTML::makeOption($p['id'], $ptype);
			}
		}
		...
		$dropdown = castorHTML::selectList($options, $input_name, ''.$disabled, 'value', 'text', $selected);
	...
	public function getPropertyTypeDescDropdown($selected = '0', $input_name = 'language_context', $javascript = '')
	{
		...
		$options[] = castorHTML::makeOption('0', jr_gettext('_CASTOR_SEARCH_ALL', '_CASTOR_SEARCH_ALL', false));
		
		foreach ($this->property_types as $p) {
			if ($p['published'] == 1 && !in_array($p['ptype_desc'], $descriptions)) {
				$options[] = castorHTML::makeOption($p['ptype_desc'], $p['ptype_desc']);
				$descriptions[] = $p['ptype_desc'];
			}
		}

		$dropdown = castorHTML::selectList($options, $input_name, ' '.$javascript, 'value', 'text', $selected);
