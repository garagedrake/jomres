...
class castor_property_features
{
	...
	function get_all_property_features()
	{
		...
		$query = "SELECT `hotel_features_uid`, `hotel_feature_abbv`, `hotel_feature_full_desc`, `image`, `property_uid`, `ptype_xref`, `cat_id` , `include_in_filters` FROM #__castor_hotel_features WHERE `property_uid` = 0 ORDER BY `hotel_feature_abbv`";
		...
		foreach ($result as $r) {
			...
			$this->property_features[$r->hotel_features_uid]['abbv']				= jr_gettext('_CASTOR_CUSTOMTEXT_FEATURES_ABBV' . (int) $r->hotel_features_uid, $r->hotel_feature_abbv, false); // property feature name/abbv;
			$this->property_features[$r->hotel_features_uid]['desc']				= jr_gettext('_CASTOR_CUSTOMTEXT_FEATURES_DESC' . (int) $r->hotel_features_uid, $r->hotel_feature_full_desc, false); // property feature description
			...
			if (array_key_exists((int)$r->cat_id, $castor_property_features_categories->property_features_categories)) {
				$this->property_features[$r->hotel_features_uid]['cat_title']	= $castor_property_features_categories->property_features_categories[(int)$r->cat_id]['title'];	// property feature category title
			} else {
	...
	function get_property_feature($id = 0)
	{
		...
		$query = "SELECT `hotel_features_uid`, `hotel_feature_abbv`, `hotel_feature_full_desc`, `image`, `property_uid`, `ptype_xref`, `cat_id` , `include_in_filters` FROM #__castor_hotel_features WHERE `property_uid` = 0 AND `hotel_features_uid` = " . (int)$id;
		$result = doSelectSql($query);
		...
		foreach ($result as $r) {
			...
			$this->abbv				= jr_gettext('_CASTOR_CUSTOMTEXT_FEATURES_ABBV' . (int) $r->hotel_features_uid, $r->hotel_feature_abbv, false); // property feature name/abbv;
			$this->desc				= jr_gettext('_CASTOR_CUSTOMTEXT_FEATURES_DESC' . (int) $r->hotel_features_uid, $r->hotel_feature_full_desc, false); // property feature description
			...
			//get all property features categories
			$castor_property_features_categories = castor_singleton_abstract::getInstance('castor_property_features_categories');
			$castor_property_features_categories->get_property_features_category((int)$r->cat_id);
		
			$this->cat_title = $castor_property_features_categories->title;	// property feature category title
		}
	...
	function commit_new_property_feature()
	{
		...
		$query = "INSERT INTO #__castor_hotel_features 
										(
										 `hotel_feature_abbv`,
	...
	function commit_update_property_feature()
	{
		...
		$query = "UPDATE #__castor_hotel_features SET 
							`image` = '" . $this->image . "',
	...
	function delete_property_features($ids = array())
	{
		...
		$query = "SELECT `propertys_uid`, `property_features` FROM #__castor_propertys WHERE `property_features` != '' ";
		...
		foreach ($ids as $id) {
			...
			if (in_array($id, $used_property_features)) {
				...
				$query = "DELETE FROM #__castor_hotel_features WHERE `hotel_features_uid` = " . (int)$id;
	...
	function get_all_property_features_images()
	{
		...
		$castor_media_centre_images->get_site_images('pfeatures');

		foreach ($castor_media_centre_images->site_images['pfeatures'] as $image) {
			$r = array();
