...
class castor_gdpr_cleanup
{
	...
	*/
	private function clean_table($table = '', $id = 0)
	{
		...
		$valid_tables =
		array (
			"castor_contracts" => "contract_uid",
			"jomcomp_notes" => "contract_uid",
			"castor_room_bookings" => "contract_uid",
			
			"castor_invoice_pii_buyers" => "invoice_id",
			"castor_invoice_pii_sellers" => "invoice_id",
			"castorportal_invoices" => "id",
			"castorportal_lineitems" => "inv_id",
			"castorportal_invoices_transactions" => "invoice_id"
		);
	...
	public function cleanup_booking($contract_uid = 0, $invoice_id = 0)
	{
		...
		try {
			$this->clean_table('castor_contracts', $contract_uid);
			$this->clean_table('jomcomp_notes', $contract_uid);
			$this->clean_table('castor_room_bookings', $contract_uid);
	...
	*/
	public function cleanup_invoice($invoice_id)
	{
		...
		try {
			$this->clean_table('castor_invoice_pii_buyers', $invoice_id);
			$this->clean_table('castor_invoice_pii_sellers', $invoice_id);
			$this->clean_table('castorportal_invoices', $invoice_id);
			$this->clean_table('castorportal_invoices_transactions', $invoice_id);
			$this->clean_table('castorportal_lineitems', $invoice_id);
		} catch (Exception $e) {
			error_logging('Failed to cleanup invoice id '.$invoice_id.' because : '.$e->getMessage());
		}
	}
