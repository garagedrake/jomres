...
class castor_geolocation
{
	...
	public function __construct()
	{
		...
		$this->temp_dir_abs = CASTOR_TEMP_ABSPATH.'geolocation'.JRDS;
		...
		if ($this->api_key != '') {
			if (!isset($tmpBookingHandler->user_settings[ 'geolocated_country' ])) {
				...
				if (file_exists($this->temp_dir_abs.$hash)) {
					...
					if ($ip != '127.0.0.1' && $ip != '0.0.0.0' && $castor_gdpr_optin_consent->user_consents_to_storage()) {
						jr_import('castor_ip2locationlite');
						$ipLite = new castor_ip2location_lite();
						$ipLite->setKey($this->api_key);
						$locations = $ipLite->getCountry($ip);

						$this->detected_country = 'UK';

						if (isset($locations[ 'statusCode' ]) && $locations[ 'statusCode' ] == 'OK') {
	...
	public function auto_set_user_currency_code()
	{
		...
		if ((isset($tmpBookingHandler->user_settings[ 'geolocated_country' ]) && $tmpBookingHandler->user_settings[ 'geolocated_country' ] != '') &&
		...
		if ($castor_currency_conversion->this_code_can_be_converted($currency_code)) {
