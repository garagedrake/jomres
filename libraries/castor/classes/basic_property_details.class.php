...
// ################################################################
	 ...
	 *
	 *          Used extensively throughout Castor, this is used as a singleton to ensure that we maintain property information in ram for as long as possible without having to constantly query the database for lots of complex information
	 */
...
class basic_property_details
{
	...
	{
		$query = "SELECT `property_uid` FROM #__castorportal_featured_properties";
	...
	public function get_property_name_multi($property_uids = array(), $database_obj = false)
	{
		...
		if (!empty($property_uids)) {
			$query = 'SELECT `propertys_uid`, `property_name`, `ptype_id` FROM #__castor_propertys WHERE `propertys_uid` IN ('.castor_implode($property_uids).') ';
			...
			if (!get_showtime('heavyweight_system')) {
				...
				foreach ($property_names as $p) {
					...
					$this->property_names[ $p->propertys_uid ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_NAME', $p->property_name, false);
				}
			} else {
				foreach ($property_names as $p) {
	...
	public function gather_data($property_uid = 0)
	{
		...
		if (isset($castor_room_types->all_ptype_rtype_xrefs[$this->ptype_id])) {
			foreach ($castor_room_types->all_ptype_rtype_xrefs[$this->ptype_id] as $rtype) {
				if (isset($this->classAbbvs[ $rtype ]) &&
					(
						(isset($castor_room_types->room_types[$rtype]) && $castor_room_types->room_types[$rtype]['property_uid'] == 0) ||
						isset($castor_room_types->property_specific_room_types[$this->property_uid][$rtype])
					)
				) {
		...
		if (!empty($propertyFeaturesArray)) {
			foreach ($propertyFeaturesArray as $f) {
				...
				$image = str_replace('/castor/', '/CASTOR_ROOT_DIRECTORY/', $this->all_property_features[$f]['image']);
	...
	public function gather_data_multi($property_uids = array(), $editable = false)
	{
		...
		$customTextObj = castor_singleton_abstract::getInstance('custom_text');
		$jrportal_taxrate = castor_singleton_abstract::getInstance('jrportal_taxrate');
		$castor_config_property_singleton = castor_singleton_abstract::getInstance('castor_config_property_singleton');
		...
		if (!empty($property_uids)) {
			$query = 'SELECT 
							...
							`completed`,
							`cat_id` 
						FROM #__castor_propertys 
						WHERE propertys_uid IN (' .castor_implode($property_uids).') ';
			$propertyData = doSelectSql($query);

			//load all property specific settings
			$castor_config_property_singleton->get_property_settings($property_uids);
			...
			foreach ($propertyData as $data) {
				...
				$this->multi_query_result[ $data->propertys_uid ][ 'property_name' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_NAME_'.$data->propertys_uid, $data->property_name, $editable, false);
				$this->multi_query_result[ $data->propertys_uid ][ 'property_street' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_STREET_'.$data->propertys_uid, $data->property_street, $editable, false);
				$this->multi_query_result[ $data->propertys_uid ][ 'property_town' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_TOWN_'.$data->propertys_uid, $data->property_town, $editable, false);
				...
				if (is_numeric($data->property_region)) {
					...
					if ($castor_regions->get_region_name($data->property_region) == null) {
						$this->multi_query_result[ $data->propertys_uid ][ 'published' ] = 0;
						if ((int) $data->published == 1) {
							$query = "UPDATE #__castor_propertys SET `published` = 0 WHERE `propertys_uid` = ".(int)$data->propertys_uid.' LIMIT 1';
							doInsertSql($query, "System automatically unpublished property with incorrect region id");
						}
					}

					$this->multi_query_result[ $data->propertys_uid ][ 'property_region' ] = jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$data->property_region, $castor_regions->get_region_name($data->property_region), $editable, false);
					...
					$this->multi_query_result[ $data->propertys_uid ][ 'property_region' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_REGION', $data->property_region, $editable, false);
					$this->multi_query_result[ $data->propertys_uid ][ 'property_region_id' ] = 0;
				}

				$this->multi_query_result[ $data->propertys_uid ][ 'property_country' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_COUNTRY', getSimpleCountry($data->property_country), $editable, false);
				...
				$this->multi_query_result[ $data->propertys_uid ][ 'metatitle' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_METATITLE_'.$data->propertys_uid, $data->metatitle, false);
				$this->multi_query_result[ $data->propertys_uid ][ 'metadescription' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_METADESCRIPTION_'.$data->propertys_uid, $data->metadescription, false);
				$this->multi_query_result[ $data->propertys_uid ][ 'metakeywords' ] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_METAKEYWORDS_'.$data->propertys_uid, $data->metakeywords, false);
				...
				if (!empty($this->all_property_features) && !empty($this->multi_query_result[ $data->propertys_uid ][ 'property_features' ]) ) {
					...
					foreach ($feature_ids as $feature_id) {
						if ( trim($feature_id) != "" && (int)$feature_id > 0 && isset($this->all_property_features[$feature_id]) ) {
							...
							$url = CASTOR_SITEPAGE_URL.'&task=search&calledByModule=mod_jomsearch_m0&feature_uids[]='.$feature_id;
				...
				$this->multi_query_result[ $data->propertys_uid ][ 'property_description' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_DESCRIPTION_'.$data->propertys_uid, $data->property_description, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_checkin_times' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_CHECKINTIMES_'.$data->propertys_uid, $data->property_checkin_times, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_area_activities' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_AREAACTIVITIES_'.$data->propertys_uid, $data->property_area_activities, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_driving_directions' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_DIRECTIONS_'.$data->propertys_uid, $data->property_driving_directions, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_airports' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_AIRPORTS_'.$data->propertys_uid, $data->property_airports, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_othertransport' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_OTHERTRANSPORT_'.$data->propertys_uid, $data->property_othertransport, $editable, false));
				$this->multi_query_result[ $data->propertys_uid ][ 'property_policies_disclaimers' ] = castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPE_DISCLAIMERS_'.$data->propertys_uid, $data->property_policies_disclaimers, $editable, false));
				...
				$this->property_names[$data->propertys_uid] = jr_gettext('_CASTOR_CUSTOMTEXT_PROPERTY_NAME_'.$data->propertys_uid, $data->property_name, $editable, false);
			}

			$temp_rooms = array();
			$query = 'SELECT `room_uid`,`room_classes_uid`,`propertys_uid`,`max_people` , `max_adults` , `max_children` , `room_features_uid`   FROM #__castor_rooms WHERE propertys_uid IN ('.castor_implode($property_uids).') ';
			...
			foreach ($rooms as $room) {
				...
				if (isset($this->all_room_types[ $room->room_classes_uid ])) {
					...
					$this->multi_query_result[ $room->propertys_uid ][ 'room_types' ][ $room->room_classes_uid ][ 'desc' ] = castor_decode($this->all_room_types[ $room->room_classes_uid ][ 'room_class_full_desc' ]);
	...
	private function get_all_room_types()
	{
		...
		if (!empty($castor_room_types->room_types)) {
			foreach ($castor_room_types->room_types as $rt) {
				$this->all_room_types[ $rt['room_classes_uid'] ][ 'room_classes_uid' ] = $rt['room_classes_uid'];
				$this->all_room_types[ $rt['room_classes_uid'] ][ 'room_class_abbv' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_ABBV'.$rt['room_classes_uid'], stripslashes($rt['room_class_abbv']), false);
				$this->all_room_types[ $rt['room_classes_uid'] ][ 'room_class_full_desc' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_DESC'.$rt['room_classes_uid'], stripslashes($rt['room_class_full_desc']), false);
				...
				$this->classAbbvs[ $rt['room_classes_uid'] ][ 'abbv' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_ABBV'.$rt['room_classes_uid'], stripslashes($rt['room_class_abbv']), false);
				$this->classAbbvs[ $rt['room_classes_uid'] ][ 'desc' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_DESC'.$rt['room_classes_uid'], stripslashes($rt['room_class_full_desc']), false);
				$this->classAbbvs[ $rt['room_classes_uid'] ][ 'image' ] = $rt['image'];
			}
		}

		if (!empty($castor_room_types->property_specific_room_types)) {
			foreach ($castor_room_types->property_specific_room_types as $property_room_type) {
		...
		$this->roomtypes_propertytypes_xref = $castor_room_types->all_ptype_rtype_xrefs;
	}
	...
	private function get_all_property_features()
	{
		...
		$this->all_property_features = $castor_property_features->property_features;
	}
	...
	public function get_all_resource_features($property_uid = 0, $including_global_room_features = false)
	{
		...
		$query = 'SELECT `room_features_uid`, `feature_description`, `property_uid`, `ptype_xref`, `image` FROM #__castor_room_features WHERE '.$clause;
		...
		if (!empty($roomFeatures)) {
			foreach ($roomFeatures as $f) {
				...
				$this->all_room_features[ $f->room_features_uid ][ 'feature_description' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION'.(int) $f->room_features_uid, stripslashes($f->feature_description), false);
	...
	public function get_global_room_features()
	{
		...
		$query = 'SELECT `room_features_uid`, `feature_description`, `property_uid`, `ptype_xref`, `image` , `property_uid` FROM #__castor_room_features';
		$roomFeatures = doSelectSql($query);
		if (!empty($roomFeatures)) {
			foreach ($roomFeatures as $f) {
				...
				$this->global_room_features[ $f->room_features_uid ][ 'feature_description' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION'.(int) $f->room_features_uid, stripslashes($f->feature_description), false);
