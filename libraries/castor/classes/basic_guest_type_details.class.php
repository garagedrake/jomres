...
class basic_guest_type_details
{
	...
	public function get_all_guest_types($property_uid = 0)
	{
		...
		foreach ($result as $r) {
			...
			$this->guest_types[$r->id]['type'] = jr_gettext('_CASTOR_CUSTOMTEXT_GUESTTYPE'.$r->id, stripslashes($r->type));
			$this->guest_types[$r->id]['notes'] = jr_gettext('_CASTOR_CUSTOMTEXT_GUESTNOTES'.$r->id, stripslashes($r->notes));
