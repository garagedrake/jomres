...
// Creates - adds - removes all castor cron jobs. A pseudo cron class.
...
class castor_cron
{
	...
	public function lockJobs($jobIds = array())
	{
		if (!empty($jobIds)) {
			...
			$query = 'UPDATE #__jomcomp_cron SET `locked` = 1 WHERE `id` IN ('.castor_implode($jobIds).') ';
	...
	public function unlockJobs($jobIds = array())
	{
		if (!empty($jobIds)) {
			...
			$query = 'UPDATE #__jomcomp_cron SET `locked` = 0 WHERE `id` IN ('.castor_implode($jobIds).') ';
			if (!doInsertSql($query, '')) {
	...
	// We will use castorConfig_secret to prevent outsiders from triggering a specific cron job remotely. The cron 6000 minicomponent can be edited to disable this check if the developer wants to run the job manually while testing
	public function runDueJobs()
	{
		$lockedJobs = array();

		$castorConfig_secret = base64_encode(get_showtime('secret'));
		...
		if (!empty($this->dueJobs)) {
			...
			if (preg_match('/[^\x20-\x7f]/', $livesite)) {
				...
				$new = str_replace($livesite, $encoded, CASTOR_SITEPAGE_URL_AJAX);
				$livesite = $new;
			} else {
				$livesite = CASTOR_SITEPAGE_URL_AJAX;
			}
	...
	public function updateJob_lastrans($jobIds = array())
	{
		if (!empty($jobIds)) {
			...
			$query = 'UPDATE #__jomcomp_cron SET `last_ran` = '.$this->now.' WHERE `id` IN ('.castor_implode($jobIds).')';
			if (!doInsertSql($query, '')) {
