...
class castor_singleton_abstract
{
	...
	public static function getInstance($class, $arg1 = null)
	{
		...
		if (isset($classes[$class])) {
			...
			//if it`s not where expected, redirect to castor default frontpage so the classes and minicomponents registries will be rebuilt
			if ($class == 'minicomponent_registry') {
				if (!file_exists($classes[$class] . $class . '.class.php')) {
					...
					if (file_exists(CASTOR_TEMP_ABSPATH . 'registry.php')) {
						unlink(CASTOR_TEMP_ABSPATH . 'registry.php');
		...
		if (!isset(self::$_instances[ $class ])) {
			self::$_instances[ $class ] = new castor_empty_class();
		}
