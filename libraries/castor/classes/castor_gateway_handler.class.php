...
class castor_gateway_handler
{
	...
	public function callback($internal_call = false, $args = array())
	{
		...
		if (!$internal_call) {
			$pp_sent_invoice_id = (int) castorGetParam($_POST, 'invoice', 0);
			$pp_sent_receiver_email = (string) castorGetParam($_POST, 'receiver_email', '');
			$pp_sent_txn_id = (string) castorGetParam($_POST, 'txn_id', '');
			$pp_sent_mc_gross = (float) castorGetParam($_POST, 'mc_gross', 0.00);
			$pp_sent_mc_fee = (float) castorGetParam($_POST, 'mc_fee', 0.00);
			$pp_sent_mc_currency = (string) castorGetParam($_POST, 'mc_currency', '');
			$pp_sent_payment_status = (string) castorGetParam($_POST, 'payment_status', '');
			$pp_sent_subscr_id = (string) castorGetParam($_POST, 'subscr_id', '');
			$pp_sent_subscr_effective = (string) castorGetParam($_POST, 'subscr_effective', '');
			$pp_sent_retry_at = (string) castorGetParam($_POST, 'retry_at', '');
			$pp_sent_pending_reason = (string) castorGetParam($_POST, 'pending_reason', '');
			$pp_sent_reason_code = (string) castorGetParam($_POST, 'reason_code', '');
			$pp_sent_txn_type = (string) castorGetParam($_POST, 'txn_type', '');
			$pp_sent_custom_subscription_id = (int) castorGetParam($_REQUEST, 'subscription_id', 0);
		} else {
		...
		if ($callback_confirmed) {
			...
			if ($pp_sent_invoice_id > 0) {
				...
				$query = 'SELECT transaction_id FROM #__castorportal_invoices_transactions WHERE payment_ref = "'.$pp_sent_txn_id.'" AND payment_result = "Payment: Completed"';
	...
	public function validate_ipn()
	{
		...
		if (function_exists('curl_init')) {
			...
			curl_setopt($ch, CURLOPT_USERAGENT, 'Castor');
	...
	public function log_transaction($invoice_id, $result, $currency, $amount, $fees, $ref, $notes = '')
	{
		...
		$query = 'INSERT INTO #__castorportal_invoices_transactions SET
