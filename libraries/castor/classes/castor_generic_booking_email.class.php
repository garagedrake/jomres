...
class castor_generic_booking_email
{
	...
	public function gather_data($contract_uid = 0, $property_uid = 0, $print = false)
	{
		...
		$this->data[$contract_uid]['LABEL_ADULTS']		= jr_gettext('_CASTOR_SEARCH_FORM_ADULTS', '_CASTOR_SEARCH_FORM_ADULTS', false);
		$this->data[$contract_uid]['LABEL_CHILDREN']	= jr_gettext('_CASTOR_SEARCH_FORM_CHILDREN', '_CASTOR_SEARCH_FORM_CHILDREN', false);
		...
		//links
		$this->data[$contract_uid]['LINK_TO_PROPERTY'] = '<a href="'.get_property_details_url($property_uid, 'nosef').'">'.jr_gettext('_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_WEBSITE', '_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_WEBSITE', false, false).'</a>';

		if (!$thisJRUser->userIsManager && $thisJRUser->userIsRegistered) {
			$this->data[$contract_uid]['LINK_TO_BOOKING'] = '<a href="'.CASTOR_SITEPAGE_URL_NOSEF.'&task=muviewbooking&contract_uid='.$contract_uid.'">'.jr_gettext('_JOMCOMP_MYUSER_VIEWBOOKING', '_JOMCOMP_MYUSER_VIEWBOOKING', false, false).'</a>';
		}
		...
		//qr codes
		$url = CASTOR_SITEPAGE_URL_NOSEF.'&task=edit_booking&thisProperty='.$property_uid.'&contract_uid='.$contract_uid;
		$this->data[$contract_uid]['QR_CODE_OFFICE'] = castor_make_qr_code($url);

		$url = make_gmap_url_for_property_uid($property_uid);
		$this->data[$contract_uid]['QR_CODE_MAP'] = castor_make_qr_code($url);
		...
		if (!empty($allCustomFields)) {
			...
			foreach ($allCustomFields as $f) {
				...
				if (isset($tmpBookingHandler->tmpbooking[ $formfieldname ])) {
					$this->data[$contract_uid]['CUSTOM_FIELDS'] .= jr_gettext('CASTOR_CUSTOMTEXT'.$f[ 'uid' ], $f[ 'description' ]).': '.$tmpBookingHandler->tmpbooking[ $formfieldname ].'; ';
				}
			}
		}

		//other output
		$this->data[$contract_uid]['PAYMENT_LINK'] = CASTOR_SITEPAGE_URL_NOSEF.'&task=confirmbooking&selectedProperty='.$property_uid.'&sk='.$current_contract_details->contract[$contract_uid]['contractdeets']['secret_key'].'&nofollowtmpl=nofollowtmpl';
		...
		$this->data[$contract_uid]['SPECIAL_REQUIREMENTS'] = castor_decode($current_contract_details->contract[$contract_uid]['contractdeets']['special_reqs']);
		$this->data[$contract_uid]['CANCELLED_REASON'] = castor_decode($current_contract_details->contract[$contract_uid]['contractdeets']['cancelled_reason']);
	...
	public function parse_email($email_type = '', $contract_uid = 0)
	{
		...
		if (isset($this->property_uid) && (int)$this->property_uid > 0) {
			...
			if (!file_exists(CASTOR_MPDF_ABSPATH.JRDS.$terms_pdf_name)) {
				$MiniComponents = castor_singleton_abstract::getInstance('mcHandler');
				$pdf = $MiniComponents->specificEvent('06000', 'terms', array('property_uid' => $this->property_uid , "as_pdf" => true , "output_now" => false ));
				file_put_contents(CASTOR_MPDF_ABSPATH.JRDS.$terms_pdf_name, $pdf);
			}
			
			$terms_pdf = array('type' => 'pdf', 'path' => CASTOR_MPDF_ABSPATH, 'filename' => $terms_pdf_name);
