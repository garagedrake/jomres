...
class castor_currency_conversion
{
	...
	public function __construct()
	{
		...
		if (!empty($castor_currency_exchange_rates->rates)) {
			$this->rates = $castor_currency_exchange_rates->rates;
		} else {
			$this->rates = array();
	...
	public function get_exchange_rate_dropdown($base = '', $alternate_template = false)
	{
		...
		foreach ($this->rates[ 'GBP' ] as $currency_code => $rate) {
			if ($this->check_currency_code_valid($currency_code)) {
				...
				$fmts[ ] = castorHTML::makeOption($currency_code, $text);
		...
		$javascript = 'onChange="switch_exchange_rate(\''.CASTOR_SITEPAGE_URL_AJAX.'\',this.value);"';
		if ($alternate_template && using_bootstrap()) {
			...
			$dropdown = castorHTML::selectList($fmts, 'user_current_currency_code', ' '.$javascript, 'value', 'text', $base);
		}
