...
class castor_language
{
	...
	public function init()
	{
		...
		$castorConfig_lang = castor_cmsspecific_getcmslang();

		if (isset($_POST['castorlang'])) {
			$castorConfig_lang = (string)RemoveXSS(castorGetParam($_POST, 'castorlang', ''));
		} elseif (isset($_GET['castorlang'])) {
			$castorConfig_lang = (string)RemoveXSS(castorGetParam($_GET, 'castorlang', ''));
		} elseif (isset($_REQUEST['castorlang'])) {
			$castorConfig_lang = (string)RemoveXSS(castorGetParam($_REQUEST, 'castorlang', ''));
		} elseif (isset($_REQUEST['lang'])) {
			$castorConfig_lang = (string)RemoveXSS(castorGetParam($_REQUEST, 'lang', ''));
		} elseif (!defined('AUTO_UPGRADE')) {
			$castorConfig_lang = (string) RemoveXSS(castor_cmsspecific_getcmslang());
		} else {
		...
		if (!array_key_exists($castorConfig_lang, $langfile_crossref)) {
			$castorConfig_lang = $this->get_shortcode_to_longcode($castorConfig_lang);
		}

		$tmpBookingHandler->tmplang[ 'castorlang' ] = $castorConfig_lang;

		$this->lang = $castorConfig_lang;

		$this->datepicker_lang = 'en';
		if (isset($this->datepicker_crossref[ $castorConfig_lang ])) {
			$this->datepicker_lang = $this->datepicker_crossref[ $castorConfig_lang ];
		}
		...
		$key = array_search($castorConfig_lang, $this->shortcodes);
	...
	public function get_language($property_type = '')
	{
		...
		if ($property_type != '') {
			if (file_exists(CASTORPATH_BASE.JRDS.'language'.JRDS.strtolower($property_type).JRDS.$this->lang.'.php')) {
				require_once CASTORPATH_BASE.JRDS.'language'.JRDS.strtolower($property_type).JRDS.$this->lang.'.php';

				return true;
			} elseif (file_exists(CASTORPATH_BASE.JRDS.'language'.JRDS.strtolower($property_type).JRDS.'en-GB.php')) {
				require_once CASTORPATH_BASE.JRDS.'language'.JRDS.strtolower($property_type).JRDS.'en-GB.php';
			...
			if ( file_exists(CASTORPATH_BASE.'language'.JRDS.$this->lang.'.php')) {
				require_once CASTORPATH_BASE.'language'.JRDS.$this->lang.'.php';
			}


			//load language context language file if no property type specific language file found above
			if ($jrConfig[ 'language_context' ] != '' && file_exists(CASTORPATH_BASE.'language'.JRDS.$jrConfig[ 'language_context' ].JRDS.$this->lang.'.php')) {
				if (file_exists(CASTORPATH_BASE.'language'.JRDS.$jrConfig[ 'language_context' ].JRDS.$this->lang.'.php')) {
					require_once CASTORPATH_BASE.'language'.JRDS.$jrConfig[ 'language_context' ].JRDS.$this->lang.'.php';
					return true;
				}
			}
			...
			if (file_exists(CASTORPATH_BASE.'language'.JRDS.$this->lang.'.php')) {
				require_once CASTORPATH_BASE.'language'.JRDS.$this->lang.'.php';
				return true;
			} elseif (file_exists(CASTORPATH_BASE.'language'.JRDS.'en-GB.php')) {
				require_once CASTORPATH_BASE.'language'.JRDS.'en-GB.php';
	...
	public function get_languageselection_dropdown($config_option = false, $default_lang = '', $custom_input_name = '')
	{
		...
		if (!empty($this->selected_languages)) {
			foreach ($this->selected_languages as $lang_code) {
				$langfile_options[] = castorHTML::makeOption($lang_code, $language_names[$lang_code]);
			}
		} else {
			foreach ($language_names as $lang_code => $lang_name) {
				$langfile_options[] = castorHTML::makeOption($lang_code, $lang_name);
			}
		}
		...
		$dropdown = castorHTML::selectList($langfile_options, $input_name, ' '.$javascript.' ', 'value', 'text', $lang);

		$selecthtml = '';
		if ($custom_input_name == '') {
			if (!$config_option) {
				$selecthtml = '<form action="" method="post" name="castorlang">';
			}
			$selecthtml .= $dropdown;
			if (!$config_option) {
				$selecthtml .= '</form>';
