...
class jrportal_guest_types
{
	...
	public function get_guest_type()
	{
		...
		foreach ($result as $r) {
			$this->id = (int) $r->id;
			$this->type = jr_gettext('_CASTOR_CUSTOMTEXT_GUESTTYPE'.$r->id, stripslashes($r->type));
			$this->notes = jr_gettext('_CASTOR_CUSTOMTEXT_GUESTNOTES'.$r->id, stripslashes($r->notes));
	...
	public function commit_new_guest_type()
	{
		...
		$query = "INSERT INTO #__castor_customertypes 
							(
							`id`,
							`type`,
							`notes`,
		...
		$this->id = doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_INSERT_CUSTOMERTYPE', '_CASTOR_MR_AUDIT_INSERT_CUSTOMERTYPE', false));
	...
	public function commit_update_guest_type()
	{
		...
		$query = "UPDATE #__castor_customertypes 
					SET 
						`type` = '" .$this->type."',
						`notes` = '".$this->notes."',
		...
		if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_UPDATE_CUSTOMERTYPE', '_CASTOR_MR_AUDIT_UPDATE_CUSTOMERTYPE', false))) {
	...
	public function delete_guest_type()
	{
		...
		$query = 'DELETE FROM #__castor_customertypes WHERE `id` = '.(int) $this->id.' AND `property_uid` = '.(int) $this->property_uid;
		if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_DELETE_CUSTOMERTYPE', '_CASTOR_MR_AUDIT_DELETE_CUSTOMERTYPE', false))) {
	...
	public function publish_guest_type()
	{
		...
		$query = 'SELECT `published` FROM #__castor_customertypes WHERE `id` = '.(int) $this->id.' LIMIT 1';
		$result = doSelectSql($query);
		...
		if ($result) {
			...
			$query = 'UPDATE #__castor_customertypes SET `published` = '.$published.' WHERE `id` = '.(int) $this->id.' AND `property_uid` = '.(int) $this->property_uid;
	...
	public function save_guest_type_order()
	{
		...
		$query = 'UPDATE  #__castor_customertypes SET `order` = '.$this->order.' WHERE `id` = '.(int) $this->id.' AND `property_uid` = '.(int) $this->property_uid;
		if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_REORDER_CUSTOMERTYPE', '_CASTOR_MR_AUDIT_REORDER_CUSTOMERTYPE', false))) {
