...
// ################################################################
/**
 * New for v3.2 of Castor. Allows the system to create a registry file so that the minicomponent handler doesn't need to constantly search folders and record minicomponents on each run.
 */
...
class minicomponent_registry_trigger_xref
{
	...
	private function buildCoreTriggers()
	{
		...
		$this->trigger_xref['event_numbers']['00021'] = [ 'event_number' => '00021', 'event_name' => 'onInsertCastorCss' , 'comment' => 'Used to insert Castor specific css files into the system. ' ];
		...
		$this->trigger_xref['event_numbers']['07015'] = [ 'event_number' => '07015', 'event_name' => 'onListViewPriceConfirmPluginSetsPrices' , 'comment' => 'Plugins can elect to populate the $castor_property_list_prices->lowest_prices variable themselves depending on the property UID being viewed. This trigger is used to allow the plugin to confirm if it will return a price for this particular property. See 07016.' ];
		$this->trigger_xref['event_numbers']['07016'] = [ 'event_number' => '07016', 'event_name' => 'onListViewPriceGetPluginPrice' , 'comment' => 'Plugin will return a price for the property, instead of allowing Castor to calculate the price. Currently used by Jintour so that Jintour can calculate it\'s own prices for properties' ];
		...
		$this->trigger_xref['event_numbers']['01012'] = [ 'event_number' => '01012', 'event_name' => 'onShowSearchResultsPopulateOutputArray' , 'comment' => 'Gives scripts the opportunity to populate the $property_deets array after Castor populates it. This gives the script the opportunity to change output without forcing a developer to customise 01010 itself.' ];
		...
		$this->trigger_xref['event_numbers']['03150'] = [ 'event_number' => '03150', 'event_name' => 'onSendBookingEmail' , 'comment' => 'castor_generic_booking_email -> parse_email method calls this trigger. It is used to retrieve email template editing labels and template file contents, and uses the template file contents to build emails sent to guests.' ];
		...
		$this->trigger_xref['event_numbers']['03700'] = [ 'event_number' => '03700', 'event_name' => 'onBeforeSaveAccount' , 'comment' => 'Called when saving account details. Gives plugins the option to parse saved before it\'s saved to the database. Reason created : Twilio cannot send messages to invalid numbers, therefore parsing the $_REQUEST[\'mobile\'] value, checking it and if necessary resetting it to empty if it cannot be validated will prevent Twilio throwing errors. From Castor 10.5.4' ];
