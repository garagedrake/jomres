...
class castor_generic_booking_amend
{
	...
	private function validate()
	{
		...
		$query = 'SELECT `room_bookings_uid`,`date` FROM #__castor_room_bookings WHERE `room_uid` = '.(int) $this->room_uid.' AND `contract_uid` = '.(int) $this->contract_uid.' ORDER BY `date` ASC ';
	...
	private function do_amend_contract()
	{
		...
		$query = 'SELECT `rooms_tariffs` FROM #__castor_contracts WHERE `contract_uid` = '.(int) $this->contract_uid;
		...
		if ($clause != '') {
			$query = "UPDATE #__castor_contracts SET $clause WHERE `contract_uid` = ".(int) $this->contract_uid;
		...
		if ($this->dates_changed) {
			...
			$query = 'DELETE FROM #__castor_room_bookings WHERE `contract_uid` = '.(int) $this->contract_uid;
			if (!doInsertSql($query, '')) {
			...
			$query = 'INSERT INTO #__castor_room_bookings 
										(
										`room_uid`,
										`date`,
										`contract_uid`,
			...
			$query = 'UPDATE #__castor_room_bookings SET `room_uid`= '.(int) $this->new_room_uid.' WHERE `contract_uid` = '.(int) $this->contract_uid.' AND `room_uid` = '.(int) $this->room_uid.' ';
			if (!doInsertSql($query, '')) {
	...
	private function check_availability()
	{
		...
		if (!$this->event_resized) {
			$clause = ' `date` IN ('.castor_implode($this->date_range_array, false).')';
		} else {
			if (count($this->date_range_array) > count($this->contract_booked_dates)) {
			...
			if (!empty($booked_dates_diff)) {
				$clause = ' `date` IN ('.castor_implode($booked_dates_diff, false).')';
			} else {
				throw new Exception('Something went wrong.');
			}
		}

		$query = 'SELECT `room_bookings_uid` FROM #__castor_room_bookings 
						WHERE `room_uid` IN ('.castor_implode($this->this_contract_room_uids).")  
						AND $clause 
