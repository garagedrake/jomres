	...
	*/
	...
	public function construct_warning($message_array)
	{
		...
		if (isset($message_array['MESSAGE'])) {
			...
			$output['WARNING_WORD'] = jr_gettext('_CASTOR_WARNINGS_DANGERWILLROBINSON', '_CASTOR_WARNINGS_DANGERWILLROBINSON', false);
	...
	public function mark_as_complete()
	{
		...
		$castor_properties->propertys_uid = $this->property_uid;
		$castor_properties->setCompleted(1);
	...
	public function check_occupancy_levels()
	{
		...
		if (!isset($jrConfig[ 'secret_setting_use_old_guest_types' ]) || $jrConfig[ 'secret_setting_use_old_guest_types' ] === "0") {
			if ($this->mrConfig[ 'allow_children' ] == '1') {
				...
				$castor_occupancy_levels = new castor_occupancy_levels($this->property_uid);

				$total_children_allowed = 0;
				foreach ($castor_occupancy_levels->occupancy_levels as $room) {
					$total_children_allowed = $total_children_allowed + $room['max_children'];
				}
				if ($total_children_allowed == 0) {
					$message = jr_gettext('_CASTOR_SANITYCHECK_OCCUPANCY_LEVELS', '_CASTOR_SANITYCHECK_OCCUPANCY_LEVELS', false);
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=list_occupancy_levels');
					$button_text = jr_gettext('_CASTOR_SANITYCHECK_OCCUPANCY_LEVELS_BUTTON', '_CASTOR_SANITYCHECK_OCCUPANCY_LEVELS_BUTTON', false);
	...
	public function check_child_rates()
	{
		...
		if (!isset($jrConfig[ 'secret_setting_use_old_guest_types' ]) || $jrConfig[ 'secret_setting_use_old_guest_types' ] === "0") {
			if ($this->mrConfig[ 'allow_children' ] == '1') {
				...
				if (!isset($castor_child_rates->child_rates) || empty($castor_child_rates->child_rates)) {
					$message = jr_gettext('_CASTOR_SANITYCHECK_CHILD_RATES', '_CASTOR_SANITYCHECK_CHILD_RATES', false);
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=child_policies');
					$button_text = jr_gettext('_CASTOR_SANITYCHECK_CHILD_RATES_BUTTON', '_CASTOR_SANITYCHECK_CHILD_RATES_BUTTON', false);
	...
	public function check_property_type_published()
	{
		...
		if ($castor_property_types->property_types[$current_property_details->ptype_id]['published'] == 0) {
			$message = jr_gettext('_CASTOR_PROPERTYTYPE_UNPUBLISHED_SANITY_CHECK', '_CASTOR_PROPERTYTYPE_UNPUBLISHED_SANITY_CHECK', false);
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=edit_property');
			$button_text = jr_gettext('_CASTOR_PROPERTYTYPE_UNPUBLISHED_SANITY_CHECK_LINK', '_CASTOR_PROPERTYTYPE_UNPUBLISHED_SANITY_CHECK_LINK', false);
	...
	public function check_for_unreviewed_bookings()
	{
		...
		$query = 'SELECT contract_uid FROM #__castor_contracts WHERE guest_uid IN ('.castor_implode($allGuestUids).') AND bookedout  = 1';
		...
		if ($unreviewed_bookings > 0 && ( get_showtime("task") != "dobooking" && get_showtime("task") != "confirmbooking" )) {
			...
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=mulistbookings&unreviewed');
			$button_text = jr_gettext('_CASTOR_FRONT_MR_MENU_ADMIN_LISTBOOKINGS', '_CASTOR_FRONT_MR_MENU_ADMIN_LISTBOOKINGS', false);
	...
	public function check_tours_exist()
	{
		...
		if (empty($future_tours)) {
			$message = jr_gettext('_CASTOR_JINTOUR_SANITY_CHECK', '_CASTOR_JINTOUR_SANITY_CHECK', false);
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=jintour');
			$button_text = jr_gettext('_CASTOR_JINTOUR_SANITY_CHECK_LINK', '_CASTOR_JINTOUR_SANITY_CHECK_LINK', false);
	...
	public function check_address()
	{
		//if (get_showtime('task') != 'edit_property') {
			$current_property_details = castor_singleton_abstract::getInstance('basic_property_details');
			$current_property_details->gather_data($this->property_uid);
		if ($current_property_details->property_street == '' ||
				$current_property_details->property_town == '' ||
				$current_property_details->property_country_code == ''
				) {
			$message = jr_gettext('_CASTOR_ADDRESS_SANITY_CHECK', '_CASTOR_ADDRESS_SANITY_CHECK', false);
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=edit_property');
			$button_text = jr_gettext('_CASTOR_ADDRESS_SANITY_CHECK_LINK', '_CASTOR_ADDRESS_SANITY_CHECK_LINK', false);
	...
	public function check_main_image()
	{
		//if (get_showtime('task') != 'media_centre') {
			...
			$castor_media_centre_images->get_images($this->property_uid, array('property'));
			$noimage = get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/assets/images/noimage.svg';

		if (!isset($castor_media_centre_images->images['property'][0][0]['large']) || $castor_media_centre_images->images['property'][0][0]['large'] == $noimage) {
			$message = jr_gettext('_CASTOR_IMAGES_EXIST_SANITY_CHECK', '_CASTOR_IMAGES_EXIST_SANITY_CHECK', false);
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=media_centre&upload_context=properties');
			$button_text = jr_gettext('_CASTOR_IMAGES_EXIST_SANITY_CHECK_LINK', '_CASTOR_IMAGES_EXIST_SANITY_CHECK_LINK', false);
	...
	public function check_approved()
	{
		...
		if (!$current_property_details->approved) {
			$message = jr_gettext('_CASTOR_APPROVALS_NOT_APPROVED_YET', '_CASTOR_APPROVALS_NOT_APPROVED_YET', false);
	...
	public function check_suspended()
	{
		...
		if ($thisJRUser->userIsSuspended) {
			$message = jr_gettext('_CASTOR_SUSPENSIONS_MANAGER_SUSPENDED', '_CASTOR_SUSPENSIONS_MANAGER_SUSPENDED', false);
	...
	/*public function checks_guest_types_pppn()
	{
		if ($this->mrConfig[ 'is_real_estate_listing' ] == '0') {
			...
			//if (!in_array(get_showtime('task'), $ignore_on_tasks)) {
				$query = 'SELECT `id` FROM `#__castor_customertypes` where property_uid = '.(int) $this->property_uid.' AND published = 1';
				$result = doSelectSql($query);
			if ((int) $this->mrConfig[ 'perPersonPerNight' ] == 1 && empty($result) && $this->mrConfig[ 'tariffmode' ] != 5) {
				$message = jr_gettext('_CASTOR_WARNINGS_PERPERSONPERNIGHT_NOGUESTTYPES', '_CASTOR_WARNINGS_PERPERSONPERNIGHT_NOGUESTTYPES', false);
				$link = castorURL(CASTOR_SITEPAGE_URL.'&task=listCustomerTypes');
				$button_text = jr_gettext('_CASTOR_CONFIG_VARIANCES_CUSTOMERTYPES', '_CASTOR_CONFIG_VARIANCES_CUSTOMERTYPES', false);
	...
	public function checks_tariffs_exist()
	{
		if ($this->mrConfig[ 'is_real_estate_listing' ] == '0') {
			...
			//if (!in_array(get_showtime('task'), $ignore_on_tasks)) {
				$query = 'SELECT `rates_uid` FROM `#__castor_rates` where property_uid = '.(int) $this->property_uid.'';
				$result = doSelectSql($query);
			if (empty($result)) {
				if ($this->mrConfig[ 'singleRoomProperty' ] == 0) {
					$message = jr_gettext('_CASTOR_WARNINGS_TARIFFS_NOTARIFFS', '_CASTOR_WARNINGS_TARIFFS_NOTARIFFS', false);
				} else {
					$message = jr_gettext('_CASTOR_WARNINGS_TARIFFS_NOTARIFFS_SRP', '_CASTOR_WARNINGS_TARIFFS_NOTARIFFS_SRP', false);
				}

				$mrConfig = getPropertySpecificSettings($this->property_uid);

				if ($mrConfig['tariffmode'] == '0') {
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=edit_tariffs_normal');
					$button_text = jr_gettext('_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', '_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', false, false);
				} elseif ($mrConfig['tariffmode'] == '1') {
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=list_tariffs_advanced');
					$button_text = jr_gettext('_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', '_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', false);
				} elseif ($mrConfig['tariffmode'] == '2') {
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=list_tariffs_micromanage');
					$button_text = jr_gettext('_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', '_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', false);
				} else { // Changed default tariff mode to micromanage Castor 10.7
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=list_tariffs_micromanage');
					$button_text = jr_gettext('_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', '_CASTOR_TARIFFS_EXIST_SANITY_CHECK_LINK', false);
				}
	...
	public function check_editing_mode()
	{
		...
		if ($this->jrConfig[ 'editingModeAffectsAllProperties' ] == '1' && $tmpBookingHandler->user_settings[ 'editing_on' ] == true && $thisJRUser->superPropertyManager) {
			$message = jr_gettext('_CASTOR_WARNINGS_GLOBALEDITINGMODE', '_CASTOR_WARNINGS_GLOBALEDITINGMODE', false);
	...
	public function check_published()
	{
		...
		if ($current_property_details->published != 1 && $thisJRUser->userIsManager) {
			$message = jr_gettext('_CASTOR_SANITY_CHECK_NOT_PUBLISHED', '_CASTOR_SANITY_CHECK_NOT_PUBLISHED', false);
			$link = castorURL(CASTOR_SITEPAGE_URL.'&task=publish_property&property_uid='.$this->property_uid);
			$button_text = jr_gettext('_CASTOR_COM_MR_VRCT_PUBLISH', '_CASTOR_COM_MR_VRCT_PUBLISH', false);
	...
	public function check_srp_room_exists()
	{
		if ($this->mrConfig[ 'is_real_estate_listing' ] == '0' && $this->mrConfig[ 'tariffmode' ] != '0') {
			...
			if (!isset($current_property_details->rooms) || empty($current_property_details->rooms)) {
				$message = jr_gettext('_CASTOR_SRP_RESOURCE_TYPE_SANITY_CHECK', '_CASTOR_SRP_RESOURCE_TYPE_SANITY_CHECK', false);
				$link = castorURL(CASTOR_SITEPAGE_URL.'&task=edit_resource');
				$button_text = jr_gettext('_CASTOR_SRP_RESOURCE_TYPE_SANITY_CHECK_LINK', '_CASTOR_SRP_RESOURCE_TYPE_SANITY_CHECK_LINK', false);
	...
	public function check_mrp_rooms_exists()
	{
		if ($this->mrConfig[ 'is_real_estate_listing' ] == '0') {
			//if (get_showtime('task') != 'edit_resource') {
				...
				$current_property_details->gather_data($this->property_uid);
			if (!isset($current_property_details->rooms) || empty($current_property_details->rooms)) {
				$message = jr_gettext('_CASTOR_MRP_ROOMS_EXIST_SANITY_CHECK', '_CASTOR_MRP_ROOMS_EXIST_SANITY_CHECK', false);
				if ($this->mrConfig['tariffmode'] == 0) {
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=edit_tariffs_normal');
				} else {
					$link = castorURL(CASTOR_SITEPAGE_URL.'&task=list_resources');
				}
				$button_text = jr_gettext('_CASTOR_MRP_ROOMS_EXIST_SANITY_CHECK_LINK', '_CASTOR_MRP_ROOMS_EXIST_SANITY_CHECK_LINK', false);
