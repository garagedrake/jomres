...
class jrportal_invoice
{
	...
	private function create_pii_buyer($alternative_data = false)
	{
		...
		if (! $alternative_data) {
			...
			$user_details['enc_firstname']			= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['firstname']);
			$user_details['enc_surname']			= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['surname']);
			$user_details['enc_house']				= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['house']);
			$user_details['enc_street']				= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['street']);
			$user_details['enc_town']				= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['town']);
			$user_details['enc_county']				= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['region']);
			$user_details['enc_country']			= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['country']);
			$user_details['enc_postcode']			= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['postcode']);
			$user_details['enc_tel_landline']		= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['tel_landline']);
			$user_details['enc_tel_mobile']			= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['tel_mobile']);
			$user_details['enc_email']				= $this->castor_encryption->encrypt($tmpBookingHandler->tmpguest['email']);
			$user_details['enc_vat_number']			= $this->castor_encryption->encrypt('');
		} else {
			$user_details = $alternative_data;
		}
 
		$query = "INSERT INTO #__castor_invoice_pii_buyers 
			(
				`invoice_id`,
				`enc_firstname`,
				`enc_surname`,
	...
	private function create_pii_seller()
	{
		...
		if ((int) $this->contract_id > 0) { // It's a booking invoice, therefore the seller is the property manager
			...
			$query = "SELECT
						`enc_firstname`,
						`enc_surname`
					FROM `#__castor_guest_profile` WHERE cms_user_id = ".(int)$manager_id."
					";
			$manager_details = doSelectSql($query, 2);
			if (!$manager_details === false) {
				$manager_details = $this->castor_encryption->decrypt($manager_details['enc_firstname'])." ".$this->castor_encryption->decrypt($manager_details['enc_surname']);
			} else {
				$manager_details = "";
			}
			...
			$user_details['enc_firstname']	=$this->castor_encryption->encrypt($mrConfig['property_business_name']);
			$user_details['enc_surname']	=$this->castor_encryption->encrypt($manager_details);
			$user_details['enc_house']		=$this->castor_encryption->encrypt($mrConfig['property_business_houseno']);
			$user_details['enc_street']		=$this->castor_encryption->encrypt($mrConfig['property_business_street']);
			$user_details['enc_town']		=$this->castor_encryption->encrypt($mrConfig['property_business_town']);
			$user_details['enc_county']		=$this->castor_encryption->encrypt($mrConfig['property_business_region']);
			$user_details['enc_country']	=$this->castor_encryption->encrypt($mrConfig['property_business_country']);
			$user_details['enc_postcode']	=$this->castor_encryption->encrypt($mrConfig['property_business_postcode']);
			$user_details['enc_tel_landline']=$this->castor_encryption->encrypt($mrConfig['property_business_telephone']);
			$user_details['enc_tel_mobile']	='';
			$user_details['enc_email']		=$this->castor_encryption->encrypt($mrConfig['property_business_email']);
			$user_details['enc_vat_number']	=$this->castor_encryption->encrypt($mrConfig['property_vat_number']);
			...
			$user_details['enc_firstname']	=$this->castor_encryption->encrypt($jrConfig[ 'business_name' ]);
			$user_details['enc_surname']	= '';
			$user_details['enc_house']		=$this->castor_encryption->encrypt($jrConfig[ 'business_address' ]);
			$user_details['enc_street']		=$this->castor_encryption->encrypt($jrConfig[ 'business_street' ]);
			$user_details['enc_town']		=$this->castor_encryption->encrypt($jrConfig[ 'business_town' ]);
			$user_details['enc_county']		=$this->castor_encryption->encrypt($jrConfig[ 'business_region' ]);
			$user_details['enc_country']	=$this->castor_encryption->encrypt($jrConfig[ 'business_country' ]);
			$user_details['enc_postcode']	=$this->castor_encryption->encrypt($jrConfig[ 'business_postcode' ]);
			$user_details['enc_tel_landline']=$this->castor_encryption->encrypt($jrConfig[ 'business_telephone' ]);
			$user_details['enc_tel_mobile']	='';
			$user_details['enc_email']		=$this->castor_encryption->encrypt($jrConfig[ 'business_email' ]);
			$user_details['enc_vat_number']	=$this->castor_encryption->encrypt($jrConfig[ 'business_vat_number' ]);
		}
		
		$query = "INSERT INTO #__castor_invoice_pii_sellers
			(
				`invoice_id`,
				`enc_firstname`,
				`enc_surname`,
	...
	{
		$query = 'DELETE FROM #__castorportal_lineitems WHERE inv_id ='.$this->id;
		$result = doInsertSql($query.'');
	...
	public function commitNewInvoice()
	{
		...
		if ($this->id < 1) {
			$query = 'INSERT INTO #__castorportal_invoices
							(
							`cms_user_id`,
							`status`,
							`raised_date`,
	...
	private function generate_invoice_number()
	{
		...
		if ($mrConfig['use_custom_invoice_numbers'] == "0" || $this->property_uid == 0) {
			$query = "SELECT MAX(id) FROM #__castorportal_invoices";
	...
	public function commitLineItem()
	{
		...
		if ($this->lineitem['id'] < 1) {
			$query = 'INSERT INTO #__castorportal_lineitems
				(
				`name`,
				`description`,
				`init_price`,
	...
	public function commitUpdateInvoice()
	{
		...
		$query = 'UPDATE #__castorportal_invoices SET
	...
	public function commitUpdateLineItem()
	{
		...
		$query = "UPDATE #__castorportal_lineitems SET
	...
	public function deleteLineItemById($line_item_id = 0)
	{
		...
		$query = 'DELETE FROM #__castorportal_lineitems WHERE `id` = '.$line_item_id;
	...
	//Get the invoice booking number by contract uid. TODO: Is this really needed here? We already have a _castor_contracts query in j06005view_invoice.class.php.
	public function get_invoice_booking_number($contract_uid = 0)
	{
		...
		$query = 'SELECT tag FROM #__castor_contracts WHERE contract_uid = '.(int) $contract_uid;
	...
	public function mark_invoice_paid()
	{
		...
		if (number_format($balance, 2, '.', '') > 0.00) {
			$line_item_data = array('tax_code_id' => 0,
									'name' => '_CASTOR_AJAXFORM_BILLING_BALANCE_PAYMENT',
									'description' => '('.$today.')',
	...
	public function mark_invoice_unpaid()
	{
		...
		$query = 'DELETE FROM #__castorportal_lineitems WHERE `inv_id` = '.(int) $this->id.' AND `is_payment` = 1 ';
		$result = doInsertSql($query, '');
	...
	public function get_line_items_balance()
	{
		...
		$query = 'SELECT SUM(init_total_inclusive) FROM #__castorportal_lineitems WHERE inv_id = '.(int)$this->id;
	...
	public function get_invoice_id_by_contract_uid($contract_uid = 0)
	{
		...
		$query = 'SELECT id FROM #__castorportal_invoices WHERE `contract_id`='.(int) $contract_uid.' LIMIT 1 ';
