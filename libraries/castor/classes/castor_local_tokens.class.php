...
class castor_local_tokens
{
	...
	public function get_token( $token_name = '' , $identifier = '' , $scope = '' )
	{
		...
		if (!isset($params[$token_name]) ) {
			...
			$CLIENT_ID=generateCastorRandomString( 15 );
			...
			if ($result == true ) {
				...
				if ($json_decoded != false) {
					$query = "SELECT  `params` FROM #__castor_guest_profile WHERE `cms_user_id` = " . (int)$this->user_id;
					$user_params_check = doSelectSql($query);
					if (empty($user_params_check)) {
						$query = "INSERT INTO #__castor_guest_profile (`cms_user_id`) VALUES ( '".(int) $this->user_id."' )";
						doInsertSql($query , '' );
					}

					$params[$token_name] = $json_decoded->access_token;
					$query = "UPDATE #__castor_guest_profile SET `params` = '".json_encode($params)."' WHERE `cms_user_id` = " . (int)$this->user_id;
			...
			if ($expired) { // The token is expired, we need to get a new one. First we'll find the client id and secret, and if they don't exist, we'll generate new ones
				$query = "SELECT client_id , client_secret FROM #__castor_oauth_clients  WHERE scope = '".$scope."' AND user_id = ".$this->user_id ;
