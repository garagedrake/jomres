 ...
 *
 */
class data_source_regions extends castor_data_source_base
{
	public function build_data_cache_file()
	{
		...
		$query = "SELECT `id`,`countrycode`,`regionname` FROM #__castor_regions WHERE `id` IN (SELECT DISTINCT `property_region` FROM #__castor_propertys WHERE published = 1 ) ORDER BY `regionname`";
			...
			foreach ($result as $region) {
				foreach ($this->cms_languages as $lang) {
					if (isset($custom_region_names[$lang])) {
						$const = "_CASTOR_CUSTOMTEXT_REGIONS_".$region->id;
