...
class castorItemToolbar
{
	...
	{
		$this->newToolbar();
		$this->bs_version = castor_bootstrap_version();
	}
	...
	public function addItem($icon, $buttonClass, $task, $link, $title, $submitOnClick = false, $otherParams = '')
	{
		...
		if ($submitOnClick) {
			$item = '<a class="btn '.$buttonClass.'" href="javascript:castor_submitbutton(\''.$task.'\');" '.$otherParams.'><i class="'.$icon.'"></i> '.$title.'</a> ';
		} else {
	...
	public function getToolbar()
	{
		...
		if ($this->bs_version == '3' || $this->bs_version == '2') {
			$this->toolbar = '
						<div id="castor-item-toolbar" class="btn-group">
			';
		} else { // Bootstrap 5
			$this->toolbar = '
