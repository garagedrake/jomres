...
class jrportal_taxrate
{
	...
	public function getAllTaxRates()
	{
		...
		$query = 'SELECT `id`,`code`,`description`,`rate`,`is_eu_country` FROM #__castorportal_taxrates';
	...
	public function commitTaxRate()
	{
		if ($this->tmp_taxrate['id'] < 1) {
			$query = "INSERT INTO #__castorportal_taxrates
				(
				`code`,
				`description`,
				`rate`,
	...
	public function commitUpdateTaxRate()
	{
		if ($this->tmp_taxrate['id'] > 0) {
			$query = "UPDATE #__castorportal_taxrates SET
	...
	{
		if ((int) $id > 0) {
			$query = 'DELETE FROM #__castorportal_taxrates WHERE `id` = '.(int) $id;
			$result = doInsertSql($query, '');
	...
	public function makeTaxratesDropdown($selected = '0', $name = 'taxrate')
	{
		...
		if (!empty($this->taxrates)) {
			foreach ($this->taxrates as $r) {
				$ratesOptions[ ] = castorHTML::makeOption($r[ 'id' ], $r[ 'code' ].' '.$r[ 'description' ]);
			}
			$ratesDropdown = castorHTML::selectList($ratesOptions, $name, '', 'value', 'text', $selected);
		}
