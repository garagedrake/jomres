...
class core_package_management
{
	...
	{
		$this->download_location = CASTOR_TEMP_ABSPATH . 'package_libs' . JRDS ;
...
/* 		 if ( $this->dirmv( $this->download_location.JRDS.$library , $destination ) ) {
	...
	{
		$url = 'http://updates.castor.net/library_packages/castor_'.$library.'_last_modified.txt';
		$ch = curl_init();
	...
	private function download_package($library, $remote_archive, $local_archive)
	{
		...
		if ($last_local_file_mtime < $last_remote_file_mtime || is_null($last_remote_file_mtime)) { // The remote file modification time is less than the remote file modification time. This means that the remote file was more recently updated, so we need to download it. This prevents us from needing to constantly download the same file time and again, if it has already been downloaded once, as it's possible that the server timed out previously due to it being slow to download or unzip the file locally.
			if (!is_dir(CASTOR_TEMP_ABSPATH)) {
				mkdir(CASTOR_TEMP_ABSPATH);
				if (!is_dir(CASTOR_TEMP_ABSPATH)) {
					throw new Exception("Can't make temporary dir ".CASTOR_TEMP_ABSPATH);
				}
			}
	...
	function curl_get_file_size($url)
	{
	  ...
	  // Issue a HEAD request and follow any redirects.
		...
		curl_setopt($curl, CURLOPT_USERAGENT, "Castor package manager");
	...
	private function check_repo_local_dirs_exist($repo)
	{
		...
		if (!is_dir(CASTOR_PACKAGES_ABSPATH)) {
			mkdir(CASTOR_PACKAGES_ABSPATH);
		}
	...
	private function set_repos()
	{
		...
		$this->repos['node_modules'] =  array(
			'download_url'	=> 'http://updates.castor.net/library_packages/index.php?repo=castor_node_modules',
			'local_abs_path'	=> CASTOR_NODE_MODULES_ABSPATH
			);
		
		$this->repos['vendor'] =  array(
			'download_url' => 'http://updates.castor.net/library_packages/index.php?repo=castor_vendor',
			'local_abs_path'	=> CASTOR_VENDOR_ABSPATH
			);
	}
	...
	public function show_installer_html()
	{
		...
		$current_url = str_replace("option=com_installer", "option=com_castor", http_build_url('', $parse));
		...
		<html lang="en">
			<head>
				...
				*/
				...
				<title>Castor Local Package Installer</title>
			</head>

			<body class="text-center">
				<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
					<main role="main" class="inner cover">
						<h1 class="cover-heading">Castor Local Package Installer</h1>
						<p class="lead">Before you can continue to use to Castor we need to download and install the latest versions of the Vendor and Node Module repo packages</p>
