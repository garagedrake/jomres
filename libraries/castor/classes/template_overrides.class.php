...
class template_overrides
{
	...
	{
		$query = 'SELECT `template_name`, `path` FROM #__castor_template_package_overrides';
		...
		foreach ($result as $r) {
			if (file_exists(CASTORPATH_BASE.$r->path.$r->template_name)) {
				...
				if (file_exists(CASTORPATH_BASE.$r->path."templates".JRDS."bootstrap".$bs_version.JRDS.$r->template_name)) {
	...
	{
		$query = "SELECT `template_name` FROM #__castor_template_package_overrides WHERE  `template_name` = '".$template_name."'";
		$result = doSelectSql($query);
		
		if (empty($result)) {
			$query = "INSERT INTO #__castor_template_package_overrides 
				(
				`template_name`,
				`path`
				)
			...
			$query = "UPDATE #__castor_template_package_overrides 
						SET 
	...
	{
		$success = true;
		$query = "DELETE FROM #__castor_template_package_overrides WHERE `template_name` = '".$template_name."'";
		if (!doInsertSql($query, false)) {
			$success = false;
		}
		return $success;
