...
class jrportal_guests
{
	...
	public function get_guest()
	{
		...
		foreach ($result as $r) {
			...
			$this->vat_number						= $this->castor_encryption->decrypt($r->enc_vat_number);
	...
	public function commit_new_guest()
	{
		...
		$this->cms_user_id = castor_cmsspecific_createNewUser($this->email);
		...
		$query = "INSERT INTO #__castor_guests
						...
						VALUES 
							...
							'".$this->castor_encryption->encrypt($this->firstname)."',
							'".$this->castor_encryption->encrypt($this->surname)."',
							'".$this->castor_encryption->encrypt($this->house)."',
							'".$this->castor_encryption->encrypt($this->street)."',
							'".$this->castor_encryption->encrypt($this->town)."',
							'".$this->castor_encryption->encrypt($this->region)."',
							'".$this->castor_encryption->encrypt($this->country)."',
							'".$this->castor_encryption->encrypt($this->postcode)."',
							'".$this->castor_encryption->encrypt($this->tel_landline)."',
							'".$this->castor_encryption->encrypt($this->tel_mobile)."',
							".(int)$this->property_uid.",
							'".$this->castor_encryption->encrypt($this->email)."',
							".(int)$this->discount.",
							'".$this->castor_encryption->encrypt(trim($this->vat_number))."',
	...
	public function commit_update_guest()
	{
		...
		$query = "UPDATE #__castor_guests 
					SET 
						`guests_uid` = ".(int)$this->id.",
						`mos_userid` = ".(int)$this->cms_user_id.",
						`enc_firstname` = '".$this->castor_encryption->encrypt($this->firstname)."',
						`enc_surname` = '".$this->castor_encryption->encrypt($this->surname)."',
						`enc_house` = '".$this->castor_encryption->encrypt($this->house)."',
						`enc_street` = '".$this->castor_encryption->encrypt($this->street)."',
						`enc_town` = '".$this->castor_encryption->encrypt($this->town)."',
						`enc_county` = '".$this->castor_encryption->encrypt($this->region)."', 
						`enc_country` = '".$this->castor_encryption->encrypt($this->country)."', 
						`enc_postcode` = '".$this->castor_encryption->encrypt($this->postcode)."', 
						`enc_tel_landline` = '".$this->castor_encryption->encrypt($this->tel_landline)."',
						`enc_tel_mobile` = '".$this->castor_encryption->encrypt($this->tel_mobile)."',
						`property_uid` = ".(int)$this->property_uid.",
						`enc_email` = '".$this->castor_encryption->encrypt($this->email)."',
						`discount` = ".(int)$this->discount.",
						`enc_vat_number` = '".$this->castor_encryption->encrypt($this->vat_number)."',
	...
	public function delete_guest()
	{
		...
		if ($this->get_guest() && $this->guest_can_be_deleted()) {
			$query = 'DELETE FROM #__castor_guests WHERE `guests_uid` = '.(int) $this->id.' AND `property_uid` = '.(int)$this->property_uid;
			if (!doInsertSql($query, '')) {
	...
	function guest_can_be_deleted()
	{
		...
		$query = "SELECT `contract_uid` FROM #__castor_contracts WHERE `guest_uid` = ".(int)$this->id." AND ( `cancelled` != 1 AND `bookedout` != 1 ) " ;
		$result = doSelectSql($query);
