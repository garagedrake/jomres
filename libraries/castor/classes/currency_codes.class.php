...
class currency_codes
{
	...
	public function __construct()
	{
		...
		$this->codes = array(
			...
			'VES' => 'Venezuelan Bolivar VEF ',
			'VEF_BLKMKT' => 'Venezuelan Bolivar BLKMKT ',
		...
		// Some of these symbols or their pre/post assignments may be wrong, if so, please email support@castor.net and I'll put them right
		$this->symbols = array(
			'AFA' => array('pre' => '؋', 'post' => ''),
			...
			'VES' => array('pre' => 'Bs', 'post' => ''),
			'VEF_BLKMKT' => array('pre' => 'Bs', 'post' => ''),
			'VND' => array('pre' => '₫', 'post' => ''),
	...
	public function makeCodesDropdown($code = '', $global = false, $input_name = '')
	{
		...
		// We'll leave this as a programattic thing, as the codes will change depending on the codes that yahoo give to Castor
		...
		foreach ($this->codes as $k => $v) {
			if ($castor_currency_conversion->this_code_can_be_converted($k) || ($jrConfig[ 'useGlobalCurrency' ] == '0' && $jrConfig[ 'use_conversion_feature' ] == '0') || trim($jrConfig[ 'openexchangerates_api_key' ]) == '') {
				$options[] = castorHTML::makeOption($k, $v);
			}
		}

		return castorHTML::selectList($options, $this->input_name, '', 'value', 'text', $this->code);
	}
