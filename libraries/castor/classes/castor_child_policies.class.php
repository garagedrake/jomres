...
class castor_child_policies
{
	...
	public function save_child_policies()
	{
		...
		$query = "SELECT uid FROM #__castor_settings WHERE property_uid = '".(int) $this->property_uid."' and akey = 'child_policies' ";
		$result = doSelectSql($query);
		if (empty($result)) {
			$query = "INSERT INTO #__castor_settings (`property_uid`,`akey`,`value`) VALUES (".(int) $this->property_uid." , 'child_policies' , '".$policies."')";
		} else {
			$query = "UPDATE #__castor_settings SET `value`='".$policies."' WHERE `property_uid` = ".(int)  $this->property_uid." and `akey` = 'child_policies' ";
		}
		doInsertSql($query);
	}
}
