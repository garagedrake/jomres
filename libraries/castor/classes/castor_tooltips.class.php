...
class castor_tooltips
{
	...
	public function generate_tooltip($div, $hover_title, $hover_content, $div_content, $class, $type, $type_arguments, $url = '#')
	{
		...
		$div = generateCastorRandomString(10);
		while ($keeplooking) :
		...
		switch ($type) {
			...
			case 'infoimage':
				...
				$output[ 'IMAGE' ] = CASTOR_IMAGES_RELPATH.'SymbolInformation.png';
			...
			case 'room_type':
				...
				if ($use_javascript) {
					...
					if (strlen($class) > 0) {
						...
						$div_string .= ' class="castor_bt_tooltip_features" >';
					}
		...
		function get_javascript($div)
			...
			if (count($this->divs)>0)
				...
				foreach ($this->divs as $div)
					{
					$javascript.="castorJquery(".$div['id']."').bt();";
					}
				}
			$javascript.="</script>";
			return $javascript;
