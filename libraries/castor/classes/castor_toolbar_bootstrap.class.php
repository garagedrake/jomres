...
class castor_toolbar_bootstrap
{
	/**
	...
	public function endTable()
	{
		...
		$output .= '</div>'; //  close castor-toolbar
		$this->items = array();
	...
	public function makeCell($image, $targetTask, $link, $text, $submitOnClick = false, $submitTask = '')
	{
		...
		if ($submitOnClick) {
			$submit = '<a class="btn '.$emphasis.'" href="javascript:castor_submitbutton(\''.$submitTask.'\');"><i class="'.$icon.'"></i> '.$text.'</a> ';
		} else {
	...
	public function getStandardActivityImagesArray()
	{
		...
		$array[ 'edit_deposit' ] = array('image' => 'enterDeposit', 'label' => jr_gettext('_CASTOR_COM_MR_EB_PAYM_DEPOSIT_PAID_UPDATE', '_CASTOR_COM_MR_EB_PAYM_DEPOSIT_PAID_UPDATE', $editable = false, $isLink = true));
		$array[ 'edit_booking' ] = array('image' => 'ContactCardEdit', 'label' => jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_LINKTEXT', '_CASTOR_COM_MR_VRCT_ROOM_LINKTEXT', false));
		$array[ 'cancel_booking' ] = array('image' => 'HotelReservationStop', 'label' => jr_gettext('_CASTOR_COM_MR_EB_GUEST_CASTOR_CANCELBOOKING', '_CASTOR_COM_MR_EB_GUEST_CASTOR_CANCELBOOKING', $editable = false, $isLink = false));
