...
// ################################################################
	...
	{
		/**
		 * Constructor for the castor_booking object, sets a bunch of variables, finds configuration settings & gets the current state of the booking from the tmpbooking table.
		 */
		public function __construct()
		{
			...
			$this->castor_occupancy_levels = new castor_occupancy_levels($this->property_uid);
			...
			if (!isset($mrConfig[ 'cal_input' ])) {
				echo "Error: Configuration settings don't appear to be set. There are ".count($mrConfig).' elements in the mrConfig var and '.count($jrConfig).' in the jrConfig var. If you have just installed Castor you should log into the frontend as a property manager. This will set up sufficient data so that you can proceed.';
				exit;
			}
		...
		{
			$room_type_filter = (int)castorGetParam($_REQUEST, 'room_type_filter', 0);
			...
			$images = $castor_media_centre_images->get_images($this->property_uid, array('rooms')); // Gets the rooms images
			...
			foreach ($basic_room_details->rooms as $r) {
				if ($room_type_filter == 0 || $r['room_classes_uid'] == $room_type_filter) {
					$this->allPropertyRooms[ $r['room_uid'] ] = array(
						...
						'tagline' => $r['tagline'],
						'description' => $castor_markdown->get_markdown($r['description']),
		...
		public function getAllTariffsData()
		{
			...
			FROM #__castor_rates WHERE property_uid = '$this->property_uid'
		...
		{
			$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
			$castor_media_centre_images->get_images($this->property_uid, array('room_features'));
		...
		{
			$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
			$castor_media_centre_images->get_images($this->property_uid);
			...
			$query = 'SELECT room_classes_uid,room_class_abbv,room_class_full_desc,image FROM #__castor_room_classes WHERE room_classes_uid IN ('.castor_implode($this->allRoomClassIds).') ';
			$roomClasses = doSelectSql($query);
			foreach ($roomClasses as $c) {
				$roomtype_abbv = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_ABBV'.(int) $c->room_classes_uid, stripslashes($c->room_class_abbv), false, false);
				$roomtype_desc = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMTYPES_DESC'.(int) $c->room_classes_uid, stripslashes($c->room_class_full_desc), false, false);
				...
				if (isset($castor_media_centre_images->images [$resource_type] [$resource_id])) {
					...
					if (isset($basic_property_details->this_property_room_classes[$resource_id]['image']) && file_exists(CASTOR_IMAGELOCATION_ABSPATH.'rmtypes/'.$basic_property_details->this_property_room_classes[$resource_id]['image'])) {
						$images = array( array ( "large" => CASTOR_IMAGELOCATION_RELPATH.'rmtypes/'.$basic_property_details->this_property_room_classes[$resource_id]['image']) );
					} else {
						$images = array ( array(
		...
		public function getAllBookings()
		{
			...
			if (!$amend_contract) {
				$query = "SELECT room_uid,date FROM #__castor_room_bookings WHERE property_uid = '$this->property_uid' AND room_uid IN (".castor_implode($this->allPropertyRoomUids).') ';
			} else {
				$query = "SELECT room_uid,date FROM #__castor_room_bookings WHERE property_uid = '$this->property_uid' AND contract_uid != '$amend_contractuid' AND room_uid IN (".castor_implode($this->allPropertyRoomUids).') ';
			}
		...
		{
			$castor_media_centre_images = castor_singleton_abstract::getInstance('castor_media_centre_images');
			$castor_media_centre_images->get_images($this->property_uid, array('extras'));
			...
			if ($mrConfig[ 'showExtras' ] == '1') {
				$query = "SELECT `uid`,`name`,`desc`,`maxquantity`,`price`,`auto_select`,`tax_rate`,`chargabledaily`,`property_uid`,`published`,`validfrom`,`validto`,`limited_to_room_type` FROM `#__castor_extras` where property_uid = '$selectedProperty' AND published = '1' ORDER BY name";
				...
				foreach ($exList as $ex) {
					...
					if (!$show_extra) {
						...
						switch ($model[ 'model' ]) {
							case '1': // Per week
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERWEEK', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERWEEK'));
								break;
							case '2': // per days
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYS', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYS'));
								break;
							case '3': // per booking
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERBOOKING', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERBOOKING'));
								break;
							case '4': // per person per booking
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERBOOKING', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERBOOKING'));
								break;
							case '5': // per person per day
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERDAY', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERDAY'));
								break;
							case '6': // per person per week
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERWEEK', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERPERSONPERWEEK'));
								break;
							case '7': // per person per days min days
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYSMINDAYS', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYSMINDAYS'));
								break;
							case '8': // per days per room
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYSPERROOM', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERDAYSPERROOM'));
								break;
							case '9': // per room
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERROOMPERBOOKING', '_CASTOR_CUSTOMTEXT_EXTRAMODEL_PERROOMPERBOOKING'));
								break;
							case '100': // commission
								$model_text = $this->sanitiseOutput(jr_gettext('_CASTOR_COMMISSION', '_CASTOR_COMMISSION'));
								break;
						}
						...
						$extra_deets[ 'NAME' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRANAME'.$ex->uid, castor_decode($ex->name)));
						...
						if ($mrConfig[ 'wholeday_booking' ] == '1') {
							if ($ex->chargabledaily == '1') {
								$extra_deets[ 'PERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', false, true));
							} else {
								$extra_deets[ 'PERNIGHT' ] = '';
							}
						} else {
							if ($ex->chargabledaily == '1') {
								$extra_deets[ 'PERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_PERDAY', '_CASTOR_COM_PERDAY', false, true));
							} else {
								$extra_deets[ 'PERNIGHT' ] = '';
							}
						}
						$extra_deets[ 'DESCRIPTION' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_EXTRADESC'.$ex->uid, castor_decode($ex->desc)));

						$descriptionForOverlib = jr_gettext('_CASTOR_CUSTOMTEXT_EXTRADESC'.$ex->uid, castor_decode($ex->desc), false, true);
						...
						if (strlen($inputId) == 0) {
							$inputId = generateCastorRandomString(10);
						}
						...
						if ($ex->maxquantity > 1) {
							...
							$extra_deets[ 'DROPDOWN' ] = castorHTML::integerSelectList(01, $ex->maxquantity, 1, 'quantity'.$ex->uid, ' autocomplete="off" '.$extra_quantity_dropdown_disabled.' onchange="getResponse_extrasquantity(\'extrasquantity\',this.value,'.$ex->uid.');"', $extraDefaultQuantity, '%02d', $use_bootstrap_radios = false);

							$extra_deets[ 'INPUTBOX' ] =
								$extra_deets[ 'INPUTBOX' ].'&nbsp;&nbsp;'.
								castorHTML::integerSelectList(01, $ex->maxquantity, 1, 'quantity'.$ex->uid, ' autocomplete="off" '.$extra_quantity_dropdown_disabled.' onchange="getResponse_extrasquantity(\'extrasquantity\',this.value,'.$ex->uid.');"', $extraDefaultQuantity, '%02d', $use_bootstrap_radios = false);
						}

						$extra_deets[ 'AJAXFORM_EXTRAS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS', '_CASTOR_AJAXFORM_EXTRAS'));
						$extra_deets[ 'AJAXFORM_EXTRAS_DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS_DESC', '_CASTOR_AJAXFORM_EXTRAS_DESC', false));
						$extra_deets[ 'EXTRAS_TOTAL' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS_TOTAL', '_CASTOR_AJAXFORM_EXTRAS_TOTAL'));
			...
			if (!empty($extra_details)) {
				...
				$extras_template = $this->sanitiseOutput(jr_gettext('_CASTOR_EXTRAS_NOEXTRAS', '_CASTOR_EXTRAS_NOEXTRAS'));
			}
		...
		public function makeOutputText()
		{
			...
			$output = array();
			if ($mrConfig[ 'wholeday_booking' ] == '1') {
				$output[ 'HARRIVALDATE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL_WHOLEDAY', '_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL_WHOLEDAY'));
			} else {
				$output[ 'HARRIVALDATE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL', '_CASTOR_COM_MR_VIEWBOOKINGS_ARRIVAL'));
			}
			if ($mrConfig[ 'showdepartureinput' ] == '1') {
				if ($mrConfig[ 'fixedPeriodBookings' ] == '1') {
					$output[ 'HDEPARTUREDATE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_MR_FIXEDPRIOD1', '_CASTOR_FRONT_MR_FIXEDPRIOD1'));
				} else {
					if ($mrConfig[ 'wholeday_booking' ] == '1') {
						$output[ 'HDEPARTUREDATE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE_WHOLEDAY', '_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE_WHOLEDAY'));
					} else {
						$output[ 'HDEPARTUREDATE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE', '_CASTOR_COM_MR_VIEWBOOKINGS_DEPARTURE'));
					}
				}
			} else {
			...
			$output[ 'BOOKINGHEADER' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_QUICKRESDESC', '_CASTOR_COM_MR_QUICKRESDESC'));
			$output[ 'ADDRESSHEADER' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_PROPERTIESLISTING_THISPROPERTYADDRESS', '_CASTOR_COM_MR_PROPERTIESLISTING_THISPROPERTYADDRESS'));
			$output[ 'HEXTITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_MR_BOOKING_EXTRAS_HELP', '_CASTOR_FRONT_MR_BOOKING_EXTRAS_HELP'));

			$output[ 'REQUIREDTEXT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_JAVASCRIPT_', '_CASTOR_JAVASCRIPT_'));
			if ($mrConfig[ 'tariffChargesStoredWeeklyYesNo' ] == '0') {
				if ($mrConfig[ 'wholeday_booking' ] == '1') {
					$output[ 'BILLING_ROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY'));
				} else {
					$output[ 'BILLING_ROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_ROOM', '_CASTOR_AJAXFORM_BILLING_ROOM'));
				}
			}
			$output[ 'BILLING_ROOMTOTAL' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_ROOM_TOTAL', '_CASTOR_AJAXFORM_BILLING_ROOM_TOTAL'));
			if ($mrConfig[ 'showExtras' ] == '1') {
				$output[ 'BILLING_EXTRAS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_EXTRAS', '_CASTOR_AJAXFORM_BILLING_EXTRAS'));
			}
			if (get_showtime('include_room_booking_functionality')) {
				$output[ 'BILLING_TAX' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TAX', '_CASTOR_AJAXFORM_BILLING_TAX'));
			}

			$output[ 'BILLING_DISCOUNT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_DISCOUNT', '_CASTOR_AJAXFORM_BILLING_DISCOUNT'));

			$output[ 'BILLING_TOTAL' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TOTAL', '_CASTOR_AJAXFORM_BILLING_TOTAL'));
			if ($mrConfig[ 'chargeDepositYesNo' ] == '1') {
				$output[ 'DEPOSIT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_EB_PAYM_DEPOSITREQUIRED', '_CASTOR_COM_MR_EB_PAYM_DEPOSITREQUIRED'));
			}
			//$output['CURRENCY_SYMBOL']		=$mrConfig['currency'];
			$output[ 'BILLING_TOTALINPARTY' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TOTALINPARTY', '_CASTOR_AJAXFORM_BILLING_TOTALINPARTY'));
			$output[ 'AJAXFORM_PARTICULARS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_PARTICULARS', '_CASTOR_AJAXFORM_PARTICULARS'));
			$output[ 'AJAXFORM_PARTICULARS_DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_PARTICULARS_DESC', '_CASTOR_AJAXFORM_PARTICULARS_DESC'));
			$output[ 'AJAXFORM_AVAILABLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLE', '_CASTOR_AJAXFORM_AVAILABLE'));
			if ($thisJRUser->is_partner) {
				$output[ 'AJAXFORM_ADDRESS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_PARTNERS_GUEST_ADDRESS', '_CASTOR_PARTNERS_GUEST_ADDRESS'));
			} else {
				$output[ 'AJAXFORM_ADDRESS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ADDRESS', '_CASTOR_AJAXFORM_ADDRESS'));
			}
			$output[ 'AJAXFORM_ADDRESS_DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ADDRESS_DESC', '_CASTOR_AJAXFORM_ADDRESS_DESC', false));
			$output[ 'AJAXFORM_AVAILABLEROOMS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLEROOMS', '_CASTOR_AJAXFORM_AVAILABLEROOMS'));
			$output[ 'AJAXFORM_SELECTEDROOMS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_SELECTEDROOMS', '_CASTOR_AJAXFORM_SELECTEDROOMS', false));

			if ($mrConfig[ 'singleRoomProperty' ] == '0') {
				$output[ 'AJAXFORM_AVAILABLEROOMS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLEROOMS', '_CASTOR_AJAXFORM_AVAILABLEROOMS'));
				$output[ 'AJAXFORM_SELECTEDROOMS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_SELECTEDROOMS', '_CASTOR_AJAXFORM_SELECTEDROOMS', false));
				$output[ 'AJAXFORM_AVAILABLE_DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLE_DESC', '_CASTOR_AJAXFORM_AVAILABLE_DESC', false));
				$output[ 'AJAXFORM_INSTRUCTIONS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_INSTRUCTIONS', '_CASTOR_AJAXFORM_INSTRUCTIONS'));
			} else {
				$output[ 'AJAXFORM_AVAILABLEROOMS' ] = '';
				$output[ 'AJAXFORM_SELECTEDROOMS' ] = '';
				$output[ 'AJAXFORM_AVAILABLE_DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_AVAILABLE_DESC_SRP', '_CASTOR_AJAXFORM_AVAILABLE_DESC_SRP', false));
				$output[ 'AJAXFORM_INSTRUCTIONS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_INSTRUCTIONS_SRP', '_CASTOR_AJAXFORM_INSTRUCTIONS_SRP'));
			}

			$output[ 'ACCOMMODATION_TOTAL' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_TOTAL', '_CASTOR_AJAXFORM_ACCOMMODATION_TOTAL')).$tax_output;

			switch ($this->cfg_booking_form_daily_weekly_monthly) {
				case 'D':
					if ($mrConfig[ 'wholeday_booking' ] == '1') {
						$output[ 'ACCOMMODATION_NIGHTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_NIGHTS_WHOLEDAY', '_CASTOR_AJAXFORM_ACCOMMODATION_NIGHTS_WHOLEDAY'));
					} else {
						$output[ 'ACCOMMODATION_NIGHTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_NIGHTS', '_CASTOR_AJAXFORM_ACCOMMODATION_NIGHTS'));
					}
					break;
				case 'W':
					$output[ 'ACCOMMODATION_NIGHTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS', '_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS'));
					break;
				case 'M':
					$output[ 'ACCOMMODATION_NIGHTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS', '_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS'));
					break;
			}

			/* 			if ($mrConfig['perPersonPerNight'] == "0" )
			$output['ACCOMMODATION_PERROOM']=$this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_PERROOM',_CASTOR_AJAXFORM_ACCOMMODATION_PERROOM));
		else
			$output['ACCOMMODATION_PERROOM']=$this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_PERPERSON',_CASTOR_AJAXFORM_ACCOMMODATION_PERPERSON)); */

			if ($mrConfig[ 'perPersonPerNight' ] == '0') {
				switch ($this->cfg_booking_form_daily_weekly_monthly) {
					case 'D':
						if ($mrConfig[ 'wholeday_booking' ] == '1') {
							$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY'));
						} else {
							$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY'));
						}
						break;
					case 'W':
						$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY'));
						break;
					case 'M':
						$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY'));
						break;
				}
			} else {
				switch ($this->cfg_booking_form_daily_weekly_monthly) {
					case 'D':
						if ($mrConfig[ 'wholeday_booking' ] == '1') {
							$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PERPERSON', '_CASTOR_BOOKINGFORM_PERPERSON')).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY'));
						} else {
							$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PERPERSON', '_CASTOR_BOOKINGFORM_PERPERSON')).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY'));
						}
						break;
					case 'W':
						$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PERPERSON', '_CASTOR_BOOKINGFORM_PERPERSON')).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY'));
						break;
					case 'M':
						$output[ 'ACCOMMODATION_PERROOM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PERPERSON', '_CASTOR_BOOKINGFORM_PERPERSON')).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY'));
						break;
				}
			}

			$output[ 'PRICE_SUMMARY' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_PRICE_SUMMARY', '_CASTOR_AJAXFORM_PRICE_SUMMARY'));
			...
			if ($mrConfig[ 'wholeday_booking' ] == '1') {
				if ($mrConfig[ 'booking_form_daily_weekly_monthly' ] == 'D') {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_QUICKRES_STEP4_STAYDAYS_WHOLEDAY', '_CASTOR_COM_MR_QUICKRES_STEP4_STAYDAYS_WHOLEDAY'));
				} elseif ($mrConfig[ 'booking_form_daily_weekly_monthly' ] == 'W') {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS', '_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS'));
				} else {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS', '_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS'));
				}
			} else {
				if ($mrConfig[ 'booking_form_daily_weekly_monthly' ] == 'D') {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_QUICKRES_STEP4_STAYDAYS', '_CASTOR_COM_MR_QUICKRES_STEP4_STAYDAYS'));
				} elseif ($mrConfig[ 'booking_form_daily_weekly_monthly' ] == 'W') {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS', '_CASTOR_AJAXFORM_ACCOMMODATION_WEEKS'));
				} else {
					$output[ 'STAYDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS', '_CASTOR_AJAXFORM_ACCOMMODATION_MONTHS'));
			...
			if ($mrConfig[ 'requireApproval' ] == '1' && !$thisJRUser->userIsManager) {
				$output[ 'SUBMIT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKING_ENQUIRY_REVIEW', '_CASTOR_BOOKING_ENQUIRY_REVIEW', false, false));
			} else {
				$output[ 'SUBMIT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_MR_REVIEWBOOKING', '_CASTOR_FRONT_MR_REVIEWBOOKING', false, false));
			}

			$output[ 'LOOKRIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_LOOKRIGHT', '_CASTOR_BOOKINGFORM_LOOKRIGHT', false, false));
			$output[ 'ROOM_TOTAL_INC_TAX' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGORM_ROOMTOTAL_INC_TAX', '_CASTOR_BOOKINGORM_ROOMTOTAL_INC_TAX', false, false));
			if ($this->jrConfig[ 'show_tax_in_totals_summary' ] == '1') {
				$output[ 'ROOM_TOTAL_EX_TAX' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGORM_ROOMTOTAL_EX_TAX', '_CASTOR_BOOKINGORM_ROOMTOTAL_EX_TAX', false, false));
				$output[ 'ROOM_TOTAL_ACCOM_TAX' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TAX_ACCOM', '_CASTOR_AJAXFORM_BILLING_TAX_ACCOM', false, false));
				$output[ 'EXTRAS_TAX' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TAX_EXTRAS', '_CASTOR_AJAXFORM_BILLING_TAX_EXTRAS'));
			}
			$output[ '_CASTOR_AJAXFORM_BUTTON_OPEN_BOOKINGFORM' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BUTTON_OPEN_BOOKINGFORM', '_CASTOR_AJAXFORM_BUTTON_OPEN_BOOKINGFORM', false, false));
			$output[ '_CASTOR_AJAXFORM_BUTTON_BACK_TO_PROPERTY_DETAILS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BUTTON_BACK_TO_PROPERTY_DETAILS', '_CASTOR_AJAXFORM_BUTTON_BACK_TO_PROPERTY_DETAILS', false, false));
			if ($mrConfig[ 'chargeDepositYesNo' ] == '1') {
				$output[ '_CASTOR_BOOKINGORM_ROOMTOTAL_BALANCE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGORM_ROOMTOTAL_BALANCE', '_CASTOR_BOOKINGORM_ROOMTOTAL_BALANCE', false, false));
			}
			$output[ '_CASTOR_BOOKINGORM_ROOMFEATURE_FILTER' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGORM_ROOMFEATURE_FILTER', '_CASTOR_BOOKINGORM_ROOMFEATURE_FILTER', false, false));

			if (get_showtime('include_room_booking_functionality') && $this->cfg_singlePersonSuppliment == '1') {
				$output[ 'SINGLE_PERSON_SUPPLIMENT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_A_SUPPLIMENTS_SINGLEPERSON_COST', '_CASTOR_COM_A_SUPPLIMENTS_SINGLEPERSON_COST'));
			}
			...
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_FIRSTNAME' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_FIRSTNAME', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_FIRSTNAME', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_SURNAME' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_SURNAME', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_SURNAME', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_HOUSENO' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_HOUSENO', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_HOUSENO', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_STREET' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_STREET', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_STREET', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_TOWN' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_TOWN', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_TOWN', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_REGION' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_REGION', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_REGION', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_POSTCODE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_POSTCODE', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_POSTCODE', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_COUNTRY' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_COUNTRY', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_COUNTRY', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_LANDLINE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_LANDLINE', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_LANDLINE', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_CELLPHONE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_CELLPHONE', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_CELLPHONE', false, false));
			$output[ '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_EMAIL' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_EMAIL', '_CASTOR_BOOKINGFORM_MONITORING_REQUIRED_EMAIL', false, false));

			if (!$mrConfig['item_hire_property']) {
				$output['CASTOR_CHILDREN_BOOKING_FORM_LABEL'] = $this->sanitiseOutput(jr_gettext('CASTOR_CHILDREN_BOOKING_FORM_LABEL', 'CASTOR_CHILDREN_BOOKING_FORM_LABEL', false, false));
			}
			$output[ 'CASTOR_GUEST_BOOKING_FORM_LABEL' ] = $this->sanitiseOutput(jr_gettext('CASTOR_GUEST_BOOKING_FORM_LABEL', 'CASTOR_GUEST_BOOKING_FORM_LABEL', false, false));
			$output[ 'CASTOR_GUEST_BOOKING_FORM_LABELINFO' ] = $this->sanitiseOutput(jr_gettext('CASTOR_GUEST_BOOKING_FORM_LABELINFO', 'CASTOR_GUEST_BOOKING_FORM_LABELINFO', false, false));
			...
			if ($mrConfig['city_tax_value'] > 0) {
				...
				switch ($mrConfig[ 'city_tax_models' ]) {
					case 'single':
						$city_tax_model_string = jr_gettext('CASTOR_CITY_TAX_MODEL_SINGLE', 'CASTOR_CITY_TAX_MODEL_SINGLE', false);
						break;
					case 'pernight':
						$city_tax_model_string = jr_gettext('CASTOR_CITY_TAX_MODEL_PER_NIGHT', 'CASTOR_CITY_TAX_MODEL_PER_NIGHT', false) ;
						break;
					case 'perguest':
						$city_tax_model_string = jr_gettext('CASTOR_CITY_TAX_MODEL_PER_GUEST', 'CASTOR_CITY_TAX_MODEL_PER_GUEST', false) ;
						break;
					case 'perguestpernight':
						$city_tax_model_string = jr_gettext('CASTOR_CITY_TAX_MODEL_PER_GUEST_PER_NIGHT', 'CASTOR_CITY_TAX_MODEL_PER_GUEST_PER_NIGHT', false) ;
						break;
					case 'percentbookingtotal':
						$city_tax_model_string = jr_gettext('CASTOR_CITY_TAX_MODEL_PERCENTAGE_OF_BOOKING_TOTAL', 'CASTOR_CITY_TAX_MODEL_PERCENTAGE_OF_BOOKING_TOTAL', false);
						break;
				}
			...
			$output[ 'CASTOR_CITY_TAX_HEADING' ] = $this->sanitiseOutput(jr_gettext('CASTOR_CITY_TAX_HEADING', 'CASTOR_CITY_TAX_HEADING', false, false))." : ".$city_tax_model_string;
			$output[ 'CASTOR_CLEANING_FEE_HEADING' ] = $this->sanitiseOutput(jr_gettext('CASTOR_CLEANING_FEE_HEADING', 'CASTOR_CLEANING_FEE_HEADING', false, false));
			$output[ 'CASTOR_BOOKING_FORM_CHILDREN_AGES' ] = $this->sanitiseOutput(jr_gettext('CASTOR_BOOKING_FORM_CHILDREN_AGES', 'CASTOR_BOOKING_FORM_CHILDREN_AGES', false, false));
		...
		*/
		...
		public function initRoomFeatureFiltering()
		{
			...
			if (isset($this->allRoomFeatures) && !empty($this->allRoomFeatures)) {
				...
				foreach ($this->allRoomFeatures as $feature_id => $feature) {
					...
					$arr[ 'DESCRIPTION' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION'.(int) $feature_id, $feature);
		...
		public function initCoupons()
		{
			...
			$query = 'SELECT `coupon_id` FROM #__castor_coupons WHERE (`property_uid` = '.(int) $this->property_uid." OR `property_uid` = 0) AND DATE_FORMAT(`valid_to`, '%Y/%m/%d') >= DATE_FORMAT(NOW(), '%Y/%m/%d') ";
		...
		public function saveCoupon($coupon_code)
		{
			$thisJRUser = castor_getSingleton('jr_user');
			$today = date('Y-m-d');
			$query = "SELECT `coupon_id`,`amount`,`is_percentage`,`rooms_only`,`booking_valid_from`,`booking_valid_to`,`guest_uid` FROM #__castor_coupons WHERE coupon_code = '$coupon_code' AND `valid_from` <= '$today' AND `valid_to` >= '$today' AND (property_uid = $this->property_uid OR property_uid = 0) ";
			$response = doSelectSql($query, 2);

			$guest_uid = 0;
			if ($thisJRUser->userIsRegistered) {
				$query = "SELECT guests_uid FROM #__castor_guests WHERE property_uid = $this->property_uid AND mos_userid =".(int) $thisJRUser->id;
			...
			if ($response && ((int) $response[ 'guest_uid' ] == $guest_uid || (int) $response[ 'guest_uid' ] == 0)) {
				...
				return $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_COUPON_COUPONSAVED', '_CASTOR_AJAXFORM_COUPON_COUPONSAVED'));
			} else {
				$this->coupon_id = 0;
				$this->coupon_code = '';
				$this->coupon_details = array();
				...
				return $this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_COUPON_COUPONNOTFOUND', '_CASTOR_AJAXFORM_COUPON_COUPONNOTFOUND'));
			}
		}
		...
		public function checkGuestVariantIdAndQty($id, $qty)
		{
			...
			$query = "SELECT maximum FROM #__castor_customertypes WHERE property_uid = '$this->property_uid' AND id = '$id' AND published = '1' ";
		...
		public function makeCustomerTypes($selectedProperty)
		{
			$mrConfig = $this->mrConfig;
			$cust = array();
			$query = "SELECT `id`,`type`,`maximum` , `is_child` FROM `#__castor_customertypes` where property_uid = '$selectedProperty' AND published = '1' ORDER BY `order`";
			$exList = doSelectSql($query);
			...
			foreach ($exList as $ct) {
				...
				$customerTypes[ 'TEXT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_GUESTTYPE'.$ct->id, htmlspecialchars(trim(stripslashes($ct->type)), ENT_QUOTES)));
				...
				$customerTypes[ 'DROPDOWN' ] = castorHTML::integerSelectList($default_start, $ct->maximum, 1, 'guesttype'.$ct->id, ' onchange="getResponse_guesttype(\''.$ct->id.'\',this.value);"', $defNo, '0', $use_bootstrap_radios = false);
		...
		public function checkExtra($extra)
		{
			...
			$query = "SELECT name FROM #__castor_extras WHERE property_uid = '$this->property_uid' AND uid = '$extra' ";
			$result = doSelectSql($query);
			if (count($result) == 1) {
				return true;
			}
				...
				if ($this->cfg_fixedPeriodBookings == '1' && $this->cfg_singleRoomProperty == '1') {
					...
					if (!$amend_contract) {
						if (!isset($this->fixedDaysArrivaldateDropdown_checkdates)) {
							$query = 'SELECT room_bookings_uid FROM #__castor_room_bookings WHERE `date` IN ('.castor_implode($dateRangeArray, false).") AND property_uid = '$this->property_uid'";
		...
		*/
		...
		public function makeDepartureDateOutput($departureDate)
		{
			if ($this->cfg_fixedPeriodBookings == '0') {
				...
				if ($this->cfg_fixedPeriodBookingsShortYesNo == '1') {
					...
					$periods[ ] = castorHTML::makeOption($this->cfg_fixedPeriodBookingsShortNumberOfDays, $this->cfg_fixedPeriodBookingsShortNumberOfDays);
				}
				for ($i = 1, $n = $this->cfg_numberofFixedPeriods; $i <= $n; ++$i) {
					//$selected="";
					$step = $i * $this->cfg_fixedPeriodBookingsNumberOfDays;
					$periods[ ] = castorHTML::makeOption($step, $step);
				}
				$fixedPeriodDropdown = castorHTML::selectList($periods, 'fixedPeriod_periodsRequested', ' onchange="getResponse_particulars(\'departure_period\',this.value)";', 'value', 'text', $defaultPeriod);
		...
		/**
		 * Converts a date passed from the javascript calendar into the yyyy/mm/dd format that Castor expects to deal with.
		 */
		public function JSCalConvertInputDates($inputDate)
		{
			...
			switch ($inputFormat) {
				case '%d/%m/%Y':
					$date_elements = explode('/', $inputDate);
					$unixDate = mktime(0, 0, 0, (int)$date_elements[ 1 ], (int)$date_elements[ 0 ], (int)$date_elements[ 2 ]);
					break;
				case '%Y/%m/%d':
					$date_elements = explode('/', $inputDate);
				...
				case '%m/%d/%Y':
					$date_elements = explode('/', $inputDate);
					$unixDate = mktime(0, 0, 0, (int)$date_elements[ 0 ], (int)$date_elements[ 1 ], (int)$date_elements[ 2 ]);
					break;
				case '%d-%m-%Y':
					$date_elements = explode('-', $inputDate);
					$unixDate = mktime(0, 0, 0, (int)$date_elements[ 1 ], (int)$date_elements[ 0 ], (int)$date_elements[ 2 ]);
		...
		public function getExistingCustomerData($id)
		{
			...
			if ($thisJRUser->is_partner || $thisJRUser->userIsManager) {
				if ($thisJRUser->userIsManager) {
					$query = "SELECT guests_uid,mos_userid,enc_firstname,enc_surname,enc_house,enc_street,enc_town,enc_county ,enc_country,enc_postcode,enc_tel_landline,enc_tel_mobile,enc_email,discount FROM #__castor_guests WHERE guests_uid = '$id' AND property_uid IN (".castor_implode($thisJRUser->authorisedProperties).') LIMIT 1';
				} else {
					$query = 'SELECT guests_uid,mos_userid,enc_firstname,enc_surname,enc_house,enc_street,enc_town,enc_county ,enc_country,enc_postcode,enc_tel_landline,enc_tel_mobileenc_,email,discount FROM #__castor_guests WHERE partner_id = '.$thisJRUser->id." AND guests_uid = '$id' LIMIT 1";
				}
				...
				if (!empty($result)) {
					...
					$this->setGuest_firstname($this->castor_encryption->decrypt($result[ 'enc_firstname' ]));
					$this->setGuest_surname($this->castor_encryption->decrypt($result[ 'enc_surname' ]));
					$this->setGuest_house($this->castor_encryption->decrypt($result[ 'enc_house' ]));
					$this->setGuest_street($this->castor_encryption->decrypt($result[ 'enc_street' ]));
					$this->setGuest_town($this->castor_encryption->decrypt($result[ 'enc_town' ]));
					$this->setGuest_region($this->castor_encryption->decrypt($result[ 'enc_county' ]));
					$this->setGuest_country($this->castor_encryption->decrypt($result[ 'enc_country' ]));
					$this->setGuest_postcode($this->castor_encryption->decrypt($result[ 'enc_postcode' ]));
					$this->setGuest_tel_landline($this->castor_encryption->decrypt($result[ 'enc_tel_landline' ]));
					$this->setGuest_tel_mobile($this->castor_encryption->decrypt($result[ 'enc_tel_mobile' ]));
					if ($this->checkEmail($this->castor_encryption->decrypt($result[ 'enc_email' ]))) {
						$this->setGuest_email($this->castor_encryption->decrypt($result[ 'enc_email' ]));
					}
		...
		public function makeRequiredIcons()
		{
			...
			// From 10.7.2
			$output[ 'REQUIRED_FIRSTNAME' ] = '';
			$output[ 'REQUIRED_SURNAME' ] = '';
			$output[ 'REQUIRED_HOUSENO' ] = '';
			$output[ 'REQUIRED_STREET' ] = '';
			$output[ 'REQUIRED_TOWN' ] = '';
			$output[ 'REQUIRED_POSTCODE' ] = '';
			$output[ 'REQUIRED_REGION' ] = '';
			$output[ 'REQUIRED_COUNTRY' ] = '';
			$output[ 'REQUIRED_LANDLINE' ] = '';
			$output[ 'REQUIRED_CELLMOBILE' ] = '';
			$output[ 'REQUIRED_EMAIL' ] = '';
			...
			if ($mrConfig[ 'bookingform_requiredfields_name' ] == '1') {
				...
				$output[ 'REQUIRED_FIRSTNAME' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_surname' ] == '1') {
				...
				$output[ 'REQUIRED_SURNAME' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_houseno' ] == '1') {
				...
				$output[ 'REQUIRED_HOUSENO' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_street' ] == '1') {
				...
				$output[ 'REQUIRED_STREET' ] = ' required ';
			}
			...
			if ($mrConfig[ 'bookingform_requiredfields_postcode' ] == '1') {
				...
				$output[ 'REQUIRED_POSTCODE' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_region' ] == '1') {
				...
				$output[ 'REQUIRED_REGION' ] = ' required ';
			}
			...
			if ($mrConfig[ 'bookingform_requiredfields_country' ] == '1') {
				...
				$output[ 'REQUIRED_COUNTRY' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_tel' ] == '1') {
				...
				$output[ 'REQUIRED_LANDLINE' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_mobile' ] == '1') {
				...
				$output[ 'REQUIRED_CELLMOBILE' ] = ' required ';
			}
			if ($mrConfig[ 'bookingform_requiredfields_email' ] == '1') {
				...
				$output[ 'REQUIRED_EMAIL' ] = ' required ';
			}
		...
		public function makeGuestData()
		{
			...
			if ($id != 0 && !$userIsManager) {
				$query = "SELECT  cms_user_id,enc_firstname,enc_surname,enc_house,enc_street,enc_town,enc_county,enc_country,enc_postcode,enc_tel_landline,enc_tel_mobile,enc_email FROM #__castor_guest_profile WHERE cms_user_id = '$id'";
				$guestList = doSelectSql($query);
				if (!empty($guestList)) {
					foreach ($guestList as $data) {
						$guests_uid = $data->cms_user_id;
						$guest_firstname = $this->castor_encryption->decrypt($data->enc_firstname);
						$guest_surname = $this->castor_encryption->decrypt($data->enc_surname);
						$guest_house = $this->castor_encryption->decrypt($data->enc_house);
						$guest_street = $this->castor_encryption->decrypt($data->enc_street);
						$guest_town = $this->castor_encryption->decrypt($data->enc_town);
						$guest_region = $this->castor_encryption->decrypt($data->enc_county);
						$guest_country = $this->castor_encryption->decrypt($data->enc_country);
						$guest_postcode = $this->castor_encryption->decrypt($data->enc_postcode);
						$guest_tel_landline = $this->castor_encryption->decrypt($data->enc_tel_landline);
						$guest_tel_mobile = $this->castor_encryption->decrypt($data->enc_tel_mobile);
						$guest_email = $this->castor_encryption->decrypt($data->enc_email);

						$query = "SELECT discount FROM #__castor_guests WHERE mos_userid = '$id' AND property_uid = '$this->property_uid' limit 1";
			...
			$guest_deets[ 'HFIRSTNAME' ] = jr_gettext('_CASTOR_FRONT_MR_DISPGUEST_FIRSTNAME', '_CASTOR_FRONT_MR_DISPGUEST_FIRSTNAME', false);
			$guest_deets[ 'HSURNAME' ] = jr_gettext('_CASTOR_FRONT_MR_DISPGUEST_SURNAME', '_CASTOR_FRONT_MR_DISPGUEST_SURNAME', false);
			$guest_deets[ 'HHOUSENO' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_HOUSE_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_HOUSE_EXPL', false);
			$guest_deets[ 'HSTREET' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_STREET_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_STREET_EXPL', false);
			$guest_deets[ 'HTOWN' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_TOWN_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_TOWN_EXPL', false);
			$guest_deets[ 'HPOSTCODE' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_POSTCODE_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_POSTCODE_EXPL', false);
			$guest_deets[ 'HREGION' ] = jr_gettext('_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_REGION', '_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_REGION', false);
			$guest_deets[ 'HCOUNTRY' ] = jr_gettext('_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_COUNTRY', '_CASTOR_COM_MR_VRCT_PROPERTY_HEADER_COUNTRY', false);
			$guest_deets[ 'HTEL' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_LANDLINE_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_LANDLINE_EXPL', false);
			$guest_deets[ 'HMOBILE' ] = jr_gettext('_CASTOR_FRONT_MR_EB_GUEST_CASTOR_MOBILE_EXPL', '_CASTOR_FRONT_MR_EB_GUEST_CASTOR_MOBILE_EXPL', false);
			if ($thisJRUser->is_partner) {
				$guest_deets[ 'HEMAIL' ] = jr_gettext('_CASTOR_DEBUGGING_YOUREMAIL', '_CASTOR_DEBUGGING_YOUREMAIL', false);
			} else {
				$guest_deets[ 'HEMAIL' ] = jr_gettext('_CASTOR_COM_MR_EB_GUEST_CASTOR_EMAIL_EXPL', '_CASTOR_COM_MR_EB_GUEST_CASTOR_EMAIL_EXPL', false);
			}
		...
		public function AJgetGuestDropdown()
		{
			...
			if ($thisJRUser->is_partner) {
				...
				$query = 'SELECT guests_uid,enc_surname, enc_firstname FROM #__castor_guests WHERE partner_id = '.$thisJRUser->id.'';
				$existingCustomers = doSelectSql($query);
			} elseif ($thisJRUser->userIsManager) {
				$query = 'SELECT guests_uid,enc_surname, enc_firstname FROM #__castor_guests WHERE property_uid IN ('.castor_implode($thisJRUser->authorisedProperties).')';
				$existingCustomers = doSelectSql($query);
			}
			...
			if (!empty($existingCustomers)) {
				...
				$ec = array();
				$ec[ ] = castorHTML::makeOption('0', jr_gettext('_CASTOR_CLEAR_GUEST_DETAILS', '_CASTOR_CLEAR_GUEST_DETAILS', false, false));
				foreach ($temp_arr as $customer) {
					if ($customer['surname'] != jr_gettext('_CASTOR_GDPR_REDACTION_STRING', '_CASTOR_GDPR_REDACTION_STRING', false, false)) {
						$ec[ ] = castorHTML::makeOption($customer['guests_uid'], $customer['surname'].', '.$customer['firstname']);
					}
				}
				$dropDownList = castorHTML::selectList($ec, 'existingCustomers', ' onchange="getResponse_existing(\'existingCustomers\',this.value);" ', 'value', 'text', 0);
			}
		...
		public function bookingCountriesDropdown($selectedCountry)
		{
			...
			foreach ($countryCodes as $k => $v) {
				$thecountryCodes[ ] = castorHTML::makeOption($k, jr_gettext('_CASTOR_CUSTOMTEXT_COUNTRYNAMES_'.$v, $v, false, false));
			}
			$countryDropdown = castorHTML::selectList($thecountryCodes, 'country', ' ', 'value', 'text', $selectedCountry);
		...
		public function checkAllGuestsAllocatedToRooms()
		{
			...
			if ($mrConfig[ 'tariffmode' ] == '3' || count($this->castor_occupancy_levels->occupancy_levels) > 0 ) {
			...
			if (!empty($guests) || $mrConfig[ 'tariffmode' ] == '3' || count($this->castor_occupancy_levels->occupancy_levels) > 0) {
		...
		public function checkTariffTalliesWithRoom($rtArray)
		{
			$room = $rtArray[ 0 ];
			/*
		$query="SELECT room_classes_uid FROM #__castor_rooms WHERE room_uid = '".$room."'";
		$roomsList=doSelectSql($query);
		foreach ($roomsList as $roomClass)
		{
		$room_class=$roomClass->room_classes_uid;
		}
		$query="SELECT rates_uid FROM #__castor_rates WHERE rates_uid = '".$rtArray[1]."' AND roomclass_uid = '".$room_class."' AND property_uid = '$this->property_uid' ";
		...
		*/
		...
		public function getAllRoomUidsForProperty()
		{
			...
			$query = "SELECT room_uid FROM #__castor_rooms WHERE propertys_uid = '$this->property_uid'";
			$roomsList = doSelectSql($query);
		...
		public function checkPeopleNumbers($freeRoomsArray)
		{
			...
			if (!empty($freeRoomsArray)) {
				...
				$query = 'SELECT room_uid,max_people FROM #__castor_rooms WHERE ';
				$query .= ' (room_uid = ';
				for ($i = 0; $i < $n; ++$i) {
	...
	*/
				...
				if (!empty($roomAndTariffArray)) {
					...
					for ($i = 0; $i < $number; ++$i) {
						...
						if ($this->cfg_singleRoomProperty == '0') {
							...
							$return_output = jr_gettext('_CASTOR_SRP_WEHAVEVACANCIES', '_CASTOR_SRP_WEHAVEVACANCIES', false, false).'<font color="white">~</font><div id="availRooms" class="roomslist"></div>';
					...
					if ($this->cfg_singleRoomProperty == '0') {
						if (empty($roomAndTariffArray) && empty($this->requestedRoom)) {
							...
							$return_output = '<div class="'.$highlight.'">'.jr_gettext('_CASTOR_FRONT_MR_QUICKRES_STEP2_NOROOMSINRANGE', '_CASTOR_FRONT_MR_QUICKRES_STEP2_NOROOMSINRANGE', false).'</div>';
						} else {
							$return_output = '';
						}
					} elseif (!isset($_REQUEST['initialise_form'])) {
						$return_output = '<div>'.jr_gettext('_CASTOR_SRP_WEHAVENOVACANCIES', '_CASTOR_SRP_WEHAVENOVACANCIES', false, false).'<font color="white">~</font><div></div id="availRooms" class="ui-state-error"></div>';
					}
				}
			}
		...
		public function generate_room_type_dropdowns()
		{
			...
			if (isset($this->room_type_style_output)) {
				...
				foreach ($this->room_type_style_output as $tariff_id => $tariff_and_roomtypes) {
					...
					$rooms_list_style_dropdown[ ] = castorHTML::makeOption('0^'.$tariff_id, sprintf('%02d', 0));
					for ($i = 1; $i <= $number_of_rooms; ++$i) {
						$room_and_tariff_outputIds_string .= $tariff_and_roomtypes[ 'roomTariffOutputId' ][ $i - 1 ].',';
						$rooms_list_style_dropdown[ ] = castorHTML::makeOption($room_and_tariff_outputIds_string, sprintf('%02d', $i));
					}
					$dropdown_output[ $tariff_id ][ 'dropdown' ] = castorHTML::selectList($rooms_list_style_dropdown, 'fred', ' autocomplete="off" onchange="getResponse_multiroom_select(\'multiroom_select\',this.value);"', 'value', 'text', $already_selected_string);
			...
			$nor = jr_gettext('_CASTOR_NUMBER_OF_ROOMS', '_CASTOR_NUMBER_OF_ROOMS', false, false);
			$gpr_text = jr_gettext('_CASTOR_MAX_GUESTS_PER_ROOM', '_CASTOR_MAX_GUESTS_PER_ROOM', false, false);
			$rmtype_text = jr_gettext('_CASTOR_COM_MR_VRCT_ROOMTYPES_HEADER_LINK', '_CASTOR_COM_MR_VRCT_ROOMTYPES_HEADER_LINK', false, false);
			$tariffname_text = jr_gettext('_CASTOR_FRONT_TARIFFS_TITLE', '_CASTOR_FRONT_TARIFFS_TITLE', false, false);

			if ($this->cfg_tariffChargesStoredWeeklyYesNo != '1') {
				if ($this->mrConfig[ 'wholeday_booking' ] == '1') {
					$rate_text = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', false, false));
				} else {
					$rate_text = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERDAY', '_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERDAY', false, false));
				}
			} else {
				$rate_text = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', '_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', false, false));
			}

			$gpb_text = '';
			if ($this->cfg_bookingform_roomlist_showmaxpeople == '1') {
				$gpb_text = jr_gettext('_CASTOR_MAX_GUESTS_PER_BOOKING', '_CASTOR_MAX_GUESTS_PER_BOOKING', false, false);
			}
			...
			foreach ($dropdown_output as $routput) {
				...
				if ($routput[ 'number_of_rooms' ]<4) {
					...
					$r[ 'NUMBER_OF_ROOMS_PRE' ] = jr_gettext('_CASTOR_COM_MR_EB_HNUMBER_OF_ROOMS_PRE', '_CASTOR_COM_MR_EB_HNUMBER_OF_ROOMS_PRE', false, false);
					$r[ 'NUMBER_OF_ROOMS_POST' ] = jr_gettext('_CASTOR_COM_MR_EB_HNUMBER_OF_ROOMS_POST', '_CASTOR_COM_MR_EB_HNUMBER_OF_ROOMS_POST', false, false);
				} else {
					$r[ 'NUMBER_OF_ROOMS' ] = "";
		...
		public function makeRoomOverlibdata($roomUid, $tariffUid, $roomTariffOutputId, $roomTariffOutputText, $removing = false)
		{
			...
			if (!$removing) {
				...
				$caption = sanitiseOverlibOutput(jr_gettext('_CASTOR_AJAXFORM_CLICKHERECAPTION', '_CASTOR_AJAXFORM_CLICKHERECAPTION', false, false));
			} else {
				$roomStuff[ 'ICON' ] = 'icon-minus-2';
				$roomStuff[ 'BUTTON_CLASS' ] = 'btn-danger';
				$caption = sanitiseOverlibOutput(jr_gettext('_CASTOR_AJAXFORM_CLICKHERECAPTION_REMOVE', '_CASTOR_AJAXFORM_CLICKHERECAPTION_REMOVE', false, false));
			}
			...
			if ((float) $sc > 0) {
				...
				$roomStuff[ 'SURCHARGE_TEXT' ] = jr_gettext('_CASTOR_SURCHARGE_TITLE', '_CASTOR_SURCHARGE_TITLE', false);
			}
			...
			if (using_bootstrap() && castor_bootstrap_version() == '3') {
				$endrun_javascript_for_eval_by_handlereq = get_showtime('endrun_javascript_for_eval_by_handlereq');
				$endrun_javascript_for_eval_by_handlereq[$roomUid.'_'.$tariffUid] = ';castorJquery(document).ready(function(){if (castorJquery(\'body > #roomdetails'.$roomUid.'_'.$tariffUid.'\').length < 1){castorJquery(\'#roomdetails'.$roomUid.'_'.$tariffUid.'\').appendTo("body");}else{castorJquery(\'body > #roomdetails'.$roomUid.'_'.$tariffUid.'\').replaceWith(castorJquery(\'#roomdetails'.$roomUid.'_'.$tariffUid.'\'));};castorJquery(\'.castor_bt_tooltip_features\').tipsy({html: true, fade: true, gravity: \'sw\', delayOut: 100});});';
		...
		public function GetRoomDetails($roomUid)
		{
			...
			$classAbbv = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMCLASS_DESCRIPTION'.$room_classes_uid, $this->allRoomClasses[ $room_classes_uid ][ 'room_class_abbv' ], false, false));
			...
			$roomRow[ 'HEADER_ROOMNUMBER' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_NUMBER', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_NUMBER', false, false));
			$roomRow[ 'HEADER_ROOMTYPE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_TYPE', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_TYPE', false, false));
			$roomRow[ 'HEADER_ROOMNAME' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_NAME', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_NAME', false, false));
			$roomRow[ 'HEADER_ROOMFLOOR' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_FLOOR', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_FLOOR', false, false));
			$roomRow[ 'HEADER_MAXPEOPLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_MAXPEOPLE', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_MAXPEOPLE', false, false));
			$roomRow[ 'HEADER_FEATURES' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_EB_ROOM_FEATURES_LIST', '_CASTOR_COM_MR_EB_ROOM_FEATURES_LIST', false, false));
			$roomRow[ 'HTITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_TITLE', '_CASTOR_FRONT_TARIFFS_TITLE', false, false));
			$roomRow[ 'HMOREINFO' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_A_CLICKFORMOREINFORMATION', '_CASTOR_COM_A_CLICKFORMOREINFORMATION', false, false));
			...
			$roomRow[ 'ROOMNAME' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMNAME_TITLE'.$roomUid, stripslashes($room[ 'room_name' ]), false, false));
		...
		public function GetTariffDetails($tariffUid)
		{
			...
			$output[ 'HTITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_TITLE', '_CASTOR_FRONT_TARIFFS_TITLE', false, false));
			$output[ 'HDESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_DESC', '_CASTOR_FRONT_TARIFFS_DESC', false, false));
			$output[ 'HSTARTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_STARTS', '_CASTOR_FRONT_TARIFFS_STARTS', false, false));
			$output[ 'HENDS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_ENDS', '_CASTOR_FRONT_TARIFFS_ENDS', false, false));
			$output[ 'HMINDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MINDAYS', '_CASTOR_FRONT_TARIFFS_MINDAYS', false, false));
			$output[ 'HMAXDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MAXDAYS', '_CASTOR_FRONT_TARIFFS_MAXDAYS', false, false));
			$output[ 'HMINPEEPS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MINPEEPS', '_CASTOR_FRONT_TARIFFS_MINPEEPS', false, false));
			$output[ 'HMAXPEEPS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MAXPEEPS', '_CASTOR_FRONT_TARIFFS_MAXPEEPS', false, false));

			switch ($this->cfg_booking_form_daily_weekly_monthly) {
				case 'D':
					if ($mrConfig[ 'wholeday_booking' ] == '1') {
						$output[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', false, false));
					} else {
						if ($tariff[ 'ignore_pppn' ] || $this->cfg_perPersonPerNight == '0') {
							$output[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN', '_CASTOR_FRONT_TARIFFS_PN', false, false));
						} else {
							$output[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PPPN', '_CASTOR_FRONT_TARIFFS_PPPN', false, false));
						}
					}
					break;
				case 'W':
					$output[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', '_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', false, false));
					break;
				case 'M':
					$output[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', false, false));
					break;
			}

			$output[ 'HROOM_DETAILS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_EB_HROOM_DETAILS', '_CASTOR_COM_MR_EB_HROOM_DETAILS', false, false));
			$output[ 'HTARIFF_DETAILS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_EB_HTARIFF_DETAILS', '_CASTOR_COM_MR_EB_HTARIFF_DETAILS', false, false));
			...
			if ($this->cfg_tariffmode == '2') {
				...
				$output[ 'TITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_TARIFF_TITLE_TARIFFTYPE_ID'.$tarifftypeid, stripslashes($tariff[ 'rate_title' ]), false, false));
				$output[ 'DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_TARIFFDESC'.$tarifftypeid, stripslashes($tariff[ 'rate_description' ]), false, false));
			} else {
				$output[ 'TITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_TARIFF_TITLE'.$tariffUid, stripslashes($tariff[ 'rate_title' ]), false, false));
				$output[ 'DESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_TARIFFDESC'.$tariffUid, stripslashes($tariff[ 'rate_description' ]), false, false));
			}
			...
			if ($mrConfig[ 'wholeday_booking' ] == '1') {
				if ($tariff[ 'ignore_pppn' ] || $this->cfg_perPersonPerNight == '0') {
					$output[ 'ROOMRATEPERDAY' ] = output_price(($this->cfg_ratemultiplier * $tariff[ 'roomrateperday' ])).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', false, false));
				} else {
					$output[ 'ROOMRATEPERDAY' ] = output_price(($this->cfg_ratemultiplier * $tariff[ 'roomrateperday' ])).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PPPN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PPPN_DAY_WHOLEDAY', false, false));
				}
			} else {
				if ($tariff[ 'ignore_pppn' ] || $this->cfg_perPersonPerNight == '0') {
					$output[ 'ROOMRATEPERDAY' ] = output_price(($this->cfg_ratemultiplier * $tariff[ 'roomrateperday' ])).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN', '_CASTOR_FRONT_TARIFFS_PN', false, false));
				} else {
					$output[ 'ROOMRATEPERDAY' ] = output_price(($this->cfg_ratemultiplier * $tariff[ 'roomrateperday' ])).' '.$this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PPPN', '_CASTOR_FRONT_TARIFFS_PPPN', false, false));
				}
			}

			if ($tariff[ 'allow_we' ] == '0') {
				$output[ 'NOTWEEKENDS' ] = jr_gettext('_CASTOR_FRONT_TARIFFS_NOTWEEKEND', '_CASTOR_FRONT_TARIFFS_NOTWEEKEND', false, false);
			}
		...
		public function makeTariffHeaders()
		{
			...
			$tariffStuff[ 'HTITLE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_TITLE', '_CASTOR_FRONT_TARIFFS_TITLE', false, false));
			$tariffStuff[ 'HDESC' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_DESC', '_CASTOR_FRONT_TARIFFS_DESC', false, false));
			$tariffStuff[ 'HSTARTS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_STARTS', '_CASTOR_FRONT_TARIFFS_STARTS', false, false));
			$tariffStuff[ 'HENDS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_ENDS', '_CASTOR_FRONT_TARIFFS_ENDS', false, false));
			$tariffStuff[ 'HMINDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MINDAYS', '_CASTOR_FRONT_TARIFFS_MINDAYS', false, false));
			$tariffStuff[ 'HMAXDAYS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MAXDAYS', '_CASTOR_FRONT_TARIFFS_MAXDAYS', false, false));
			$tariffStuff[ 'HMINPEEPS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MINPEEPS', '_CASTOR_FRONT_TARIFFS_MINPEEPS', false, false));
			$tariffStuff[ 'HMAXPEEPS' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_MAXPEEPS', '_CASTOR_FRONT_TARIFFS_MAXPEEPS', false, false));
			if ($this->cfg_tariffChargesStoredWeeklyYesNo != '1') {
				if ($mrConfig[ 'wholeday_booking' ] == '1') {
					$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', '_CASTOR_FRONT_TARIFFS_PN_DAY_WHOLEDAY', false, false));
				} else {
					$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERDAY', '_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERDAY', false, false));
				}
			} else {
				$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', '_CASTOR_COM_MR_LISTTARIFF_ROOMRATEPERWEEK', false, false));
			}

			$tariffStuff[ 'HRESOURCE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_HEADER_LINK', '_CASTOR_COM_MR_VRCT_ROOM_HEADER_LINK', false, false));
			$tariffStuff[ 'HBOOKNOW' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_FRONT_MR_MENU_BOOKTHISPROPERTY', '_CASTOR_FRONT_MR_MENU_BOOKTHISPROPERTY', false, false));
			...
			switch ($this->cfg_booking_form_daily_weekly_monthly) {
				case 'D':
					if ($mrConfig[ 'wholeday_booking' ] == '1') {
						...
						$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_DAILY', false, false));
					}
					break;
				case 'W':
					$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_WEEKLY', false, false));
					break;
				case 'M':
					$tariffStuff[ 'HRATEPERNIGHT' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', '_CASTOR_BOOKINGFORM_PRICINGOUTPUT_MONTHLY', false, false));
					break;
			}
			...
			// New for 6.6.6
			$roomRow[ 'HEADER_IMAGE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_A_BOOKINGFORM_SHOWROOMIMAGE', '_CASTOR_COM_A_BOOKINGFORM_SHOWROOMIMAGE', false, false));
			$roomRow[ 'HEADER_ROOMTYPE' ] = $this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_VRCT_ROOMTYPES_HEADER_LINK', '_CASTOR_COM_MR_VRCT_ROOMTYPES_HEADER_LINK', false, false));
		...
		public function monitorBookingStatus()
		{
			...
			if (!empty($this->requestedRoom) && $this->email != '') {
				...
				if (!$this->email_address_can_be_used) {
					$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_EMAIL_ALREADY_IN_USE', '_CASTOR_BOOKINGFORM_MONITORING_EMAIL_ALREADY_IN_USE', false, false)));
				}
			}

			if (get_showtime('include_room_booking_functionality')) {
				if ($this->mininterval == 1000) { // Probably a tariff wasn't found
					$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_SRP_WEHAVENOVACANCIES', '_CASTOR_SRP_WEHAVENOVACANCIES', false, false)));
				...
				if ($this->stayDays < $this->mininterval && !$amend_contract && $this->mininterval < 1000 && empty($this->requestedRoom)) {
					...
					/*if ($mrConfig[ 'wholeday_booking' ] == '1') {
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT1_WHOLEDAY', '_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT1_WHOLEDAY', false, false)).' '.$this->mininterval.' '.$this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT2', '_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT2', false).' '.($this->stayDays - 1)));
					} else {
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT1', '_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT1', false, false)).' '.$this->mininterval.' '.$this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT2', '_CASTOR_BOOKINGFORM_MONITORING_BOOKING_TOO_SHORT2', false).' '.$this->stayDays));
					}*/
					...
					if ($this->jrConfig[ 'useCastorMessaging' ] == '1') {
								...
								foreach ($prices as $key => $val) {
									if ($val > 0) {
										if ($this->cfg_perPersonPerNight == '1') {
											$pernight = jr_gettext('_CASTOR_FRONT_TARIFFS_PPPN', '_CASTOR_FRONT_TARIFFS_PPPN', false);
										} else {
											$pernight = jr_gettext('_CASTOR_FRONT_TARIFFS_PN', '_CASTOR_FRONT_TARIFFS_PN', false);
										}
										//echo ';castorJquery.jGrowl(\'' . jr_gettext( '_CASTOR_STAYFORAMINIMUMOF', _CASTOR_STAYFORAMINIMUMOF, false ) . " " . $key . " " . jr_gettext( '_CASTOR_NIGHTSFOR', _CASTOR_NIGHTSFOR, false ) . " " . output_price( $val ) . $pernight . '\', { life: 20000 });';
									}
								}
							}
						}
						...
						if ($mrConfig[ 'tariffmode' ] == '1') {
							foreach ($this->simple_tariff_to_date_map as $tariff) {
								...
								//echo ';castorJquery.jGrowl(\'' . jr_gettext( '_CASTOR_STAYFORAMINIMUMOF', _CASTOR_STAYFORAMINIMUMOF, false ) . " " . $tariff[ 'mindays' ] . " " . jr_gettext( '_CASTOR_NIGHTSFOR', _CASTOR_NIGHTSFOR, false ) . " " . output_price( $tariff[ 'price' ] ) . " " . $pernight . '\', { life: 20000 });';
							}
						}
					}
				}
				...
				if (!$this->checkArrivalDate($this->arrivalDate)) {
					$this->resetPricingOutput = true;
					$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_ARRIVALDATE_INVALID', '_CASTOR_BOOKINGFORM_MONITORING_ARRIVALDATE_INVALID', false, false)));
				}
				...
				// $this->resetPricingOutput=true;
				// if ($mrConfig['wholeday_booking'] == "1")
				// $this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_DEPARTUREDATE_INVALID_WHOLEDAY',_CASTOR_BOOKINGFORM_MONITORING_DEPARTUREDATE_INVALID_WHOLEDAY,false,false)));
				// else
				// $this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_DEPARTUREDATE_INVALID',_CASTOR_BOOKINGFORM_MONITORING_DEPARTUREDATE_INVALID,false,false)));
				// }
				...
				if (isset($this->jrConfig['secret_setting_use_old_guest_types']) && $this->jrConfig['secret_setting_use_old_guest_types'] == '1') {
					...
					foreach ($numberOfGuestTypes as $r) {
						if (!$this->checkGuestVariantIdAndQty($r[ 'id' ], $r[ 'qty' ])) {
							$this->resetPricingOutput = true;
							$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_GUEST_TYPE_INCORRECT', '_CASTOR_BOOKINGFORM_MONITORING_GUEST_TYPE_INCORRECT', false, false)));
						}
					}

					if ($this->total_in_party < 1 && !empty($numberOfGuestTypes)) {
						$this->resetPricingOutput = true;
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_SELECT_GUEST_NUMBERS', '_CASTOR_BOOKINGFORM_MONITORING_SELECT_GUEST_NUMBERS', false, false)));
					}
					if (!empty($numberOfGuestTypes) && !$this->tariffsCanHostTotalInParty()) {
						$this->resetPricingOutput = true;
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_TOO_MANY_IN_PARTY_FOR_TARIFFS', '_CASTOR_BOOKINGFORM_MONITORING_TOO_MANY_IN_PARTY_FOR_TARIFFS', false, false)));
					}

					if ($this->total_in_party < $requestedRoom_count && !empty($numberOfGuestTypes)) {
						$this->resetPricingOutput = true;
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_MORE_ROOMS_THAN_GUESTS', '_CASTOR_BOOKINGFORM_MONITORING_MORE_ROOMS_THAN_GUESTS', false, false)));
					}
					...
					if ($this->total_in_party > $this->beds_available && !empty($numberOfGuestTypes)) {
						$this->resetPricingOutput = true;
						if ($this->cfg_singleRoomProperty != '1') {
							$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_TOO_MANY_GUESTS_FOR_BEDS', '_CASTOR_BOOKINGFORM_MONITORING_TOO_MANY_GUESTS_FOR_BEDS', false, false)));
						} else {
							$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_SRP_WEHAVENOVACANCIES', '_CASTOR_SRP_WEHAVENOVACANCIES', false, false)));
						}
					}
					if (!empty($numberOfGuestTypes) && $requestedRoom_count > 0 && !$this->selectedRoomsCanHostTotalInParty()) {
						$this->resetPricingOutput = true;
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_CHOOSE_MORE_ROOMS', '_CASTOR_BOOKINGFORM_MONITORING_CHOOSE_MORE_ROOMS', false, false)));
					}

					if ($requestedRoom_count == 0 ) {
						$this->resetPricingOutput = true;
						$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_BOOKINGFORM_MONITORING_SELECT_A_ROOM', '_CASTOR_BOOKINGFORM_MONITORING_SELECT_A_ROOM', false, false)));
					}
					...
					if (empty($this->requestedRoom)) {
						...
						if ($this->cfg_singleRoomProperty != '1') {
							...
							$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_COM_MR_QUICKRES_STEP4_TITLE', '_CASTOR_COM_MR_QUICKRES_STEP4_TITLE', false, false)));
						}
					}
				}
			}
			...
			if (!get_showtime('include_room_booking_functionality')) {
				...
				if ($quantity == 0) {
					$this->setMonitoring($this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_EXTRAS_SELECT', '_CASTOR_AJAXFORM_EXTRAS_SELECT', false, false)));
				}
			}
		...
		public function outputZeroPrices()
		{
			if ($this->getGuestVariantCount() > 0) {
				$this->echo_populate_div('; populateDiv("totalinparty","'.$this->getTotalInParty().' '.$this->sanitiseOutput(jr_gettext('_CASTOR_AJAXFORM_BILLING_TOTALINPARTY', '_CASTOR_AJAXFORM_BILLING_TOTALINPARTY', false, false)).'")');
			}
		...
		public function selectedRoomsCanHostTotalInParty()
		{
			...
			foreach ($this->requestedRoom as $rm) {
				$room = explode('^', $rm);
				$rmuid = $room[ 0 ];
				$query = "SELECT  max_people FROM #__castor_rooms WHERE room_uid = '$rmuid'";
		...
		{
			$query = "SELECT maxpeople FROM #__castor_rates WHERE property_uid = '$this->property_uid' ORDER by maxpeople desc LIMIT 1";
			$max = doSelectSql($query, 1);
		...
		public function setGuestTypeVariantValues()
		{
			...
			if (!empty($result)) {
				...
				foreach ($result as $r) {
					$id = $r[ 'id' ];
					$qty = $r[ 'qty' ];
					$query = "SELECT `is_percentage`,`posneg`,`variance` FROM `#__castor_customertypes` where id = '$id' ";
		...
		public function makeNightlyRoomCharges()
		{
			...
			if ($this->coupon_code != '') {
				...
				$note = jr_gettext('_CASTOR_AJAXFORM_COUPON_BOOKINGNOTE', '_CASTOR_AJAXFORM_COUPON_BOOKINGNOTE', false).' '.$this->coupon_id.' / '.$this->coupon_code.' / '.jr_gettext('_JRPORTAL_COUPONS_AMOUNT', '_JRPORTAL_COUPONS_AMOUNT', false).' '.$this->coupon_discount_value.' / ';
			...
			if ( count($this->requestedRoom) > 0) {
				...
				if ($this->stayDays >= 7 && $this->stayDays < 30 && isset($modifiers->modifier_7_days) && $modifiers->modifier_7_days > 0) {
					...
					$message =
						jr_gettext('CASTOR_BOOKING_DISCOUNTED_7_DAYS_1', 'CASTOR_BOOKING_DISCOUNTED_7_DAYS_1', false).
						output_price($this->room_total).
						jr_gettext('CASTOR_BOOKING_DISCOUNTED_7_DAYS_2', 'CASTOR_BOOKING_DISCOUNTED_7_DAYS_2', false).
						output_price($new_total).
						jr_gettext('CASTOR_BOOKING_DISCOUNTED_7_DAYS_NUMBER', 'CASTOR_BOOKING_DISCOUNTED_7_DAYS_NUMBER', false);
				...
				if ($this->stayDays >= 30 && isset($modifiers->modifier_30_days) && $modifiers->modifier_30_days > 0) {
					...
					$message =
						...
						jr_gettext('CASTOR_BOOKING_DISCOUNTED_7_DAYS_2', 'CASTOR_BOOKING_DISCOUNTED_7_DAYS_2', false).
						output_price($new_total).
						jr_gettext('CASTOR_BOOKING_DISCOUNTED_30_DAYS_NUMBER', 'CASTOR_BOOKING_DISCOUNTED_30_DAYS_NUMBER', false);
			...
			if ($this->guest_specific_discount > 0) {
				...
				$note = jr_gettext('_CASTOR_PERSONAL_DISCOUNT', '_CASTOR_PERSONAL_DISCOUNT', false, false).' '.output_price($percentage_to_remove);
				$this->addBookingNote(jr_gettext('_CASTOR_PERSONAL_DISCOUNT', '_CASTOR_PERSONAL_DISCOUNT', false), $note);
			...
			$query = 'SELECT discount FROM #__castor_partners_discounts WHERE partner_id = '.(int) $thisJRUser->id.' AND property_id = '.(int) $this->property_uid." and valid_from <= '".$first_date."' AND valid_to >= '".$last_date."'";
			$result = doSelectSql($query);
			...
			if (!empty($result)) {
				foreach ($result as $res) {
					...
					$this->room_total = $this->room_total - $percentage_to_remove;
					$note = jr_gettext('_CASTOR_PARTNER_DISCOUNT', '_CASTOR_PARTNER_DISCOUNT', false, false).' '.output_price($percentage_to_remove);
					$this->addBookingNote(jr_gettext('_CASTOR_PARTNER_DISCOUNT', '_CASTOR_PARTNER_DISCOUNT', false), $note);
			...
			if (!empty($this->extras)) {
				...
				foreach ($extrasArray as $extra) {
					if ( !empty($extra) ) {
						...
						$query = "SELECT price,tax_rate FROM #__castor_extras WHERE uid = '$extra'";
		...
		public function getRoomAllocationOutput()
		{
			...
			$text_room = jr_gettext('_CASTOR_ROOMALLOCATIONS_ROOM', '_CASTOR_ROOMALLOCATIONS_ROOM', false, false);
			$text_guests = jr_gettext('_CASTOR_ROOMALLOCATIONS_GUESTS', '_CASTOR_ROOMALLOCATIONS_GUESTS', false, false);
			$text_information = jr_gettext('_CASTOR_ROOMALLOCATIONS_INFORMATION', '_CASTOR_ROOMALLOCATIONS_INFORMATION', false, false);
			...
			if (!empty($discountData)) {
				...
				foreach ($discountData as $d) {
					if ($d[ 'isDiscounted' ]) {
						...
						$roomtype = $d[ 'roomType' ];
						$roomtype_abbr = $this->sanitiseOutput(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMCLASS_DESCRIPTION'.$roomtype, $this->allRoomClasses[ $roomtype ][ 'room_class_abbv' ], false, false));
						$roomrate = $d[ 'roomrate' ];
		...
		/**
		 *
		// This function is designed to check with the Castor system to see if the email address this user is trying to use can be used.
		// Scenario :
		...
		// If the system is not configured to create new users then a new user isn't created anyway, in which case Castor will simply return that it's ok to use the email address
		...
		public function email_usage_check($email)
		{
			...
			if ($this->amend_contract) { // Managers can re-use email addresses of guests.
				...
				if ($jrConfig[ 'useNewusers' ] == '0') { // We don't create new users on bookings from non-registered bookers, so it's ok to re-use an email address.
					...
					$all_users = castor_cmsspecific_getCMSUsers();
					$email_found = false;
					...
					if ($email_found) {
						if ($thisJRUser->userIsManager) { // At this point we have a manager who is using the dropdown to select a guest's details
							...
							if ($thisJRUser->userIsRegistered) {
								$users_id = castor_cmsspecific_getcurrentusers_id();
			...
			foreach ($this->child_prices as $index => $child_set) {
				if ($index != "total_price") {
					...
					if ($child_set['rate']['model'] == 'per_night') {
						$model = jr_gettext('CASTOR_POLICIES_CHILDREN_CHARGE_MODEL_PER_NIGHT', 'CASTOR_POLICIES_CHILDREN_CHARGE_MODEL_PER_NIGHT', false);
					} else {
						$model = jr_gettext('CASTOR_POLICIES_CHILDREN_CHARGE_MODEL_PER_STAY', 'CASTOR_POLICIES_CHILDREN_CHARGE_MODEL_PER_STAY', false);
					}
					$text_children = jr_gettext('_CASTOR_SEARCH_FORM_CHILDREN', '_CASTOR_SEARCH_FORM_CHILDREN', false);
					$text_age_from = jr_gettext('CASTOR_POLICIES_CHILDREN_CHILD_RATE_AGE_FROM', 'CASTOR_POLICIES_CHILDREN_CHILD_RATE_AGE_FROM', false);
					$text_age_to = jr_gettext('CASTOR_POLICIES_CHILDREN_CHILD_RATE_AGE_TO', 'CASTOR_POLICIES_CHILDREN_CHILD_RATE_AGE_TO', false);
