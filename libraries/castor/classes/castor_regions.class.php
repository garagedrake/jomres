...
class castor_regions
{
	...
	public function get_used_property_regions()
	{
		...
		$query = "SELECT `id`, `countrycode`, `regionname` FROM #__castor_regions WHERE `id` IN (SELECT DISTINCT `property_region` FROM #__castor_propertys) ORDER BY `countrycode`,`regionname`";
		$result = doSelectSql($query);
		
		if (!empty($result)) {
			foreach ($result as $region) {
				if ($this->region_names_are_translatable) {
					$this->regions[ $region->id ] = array('id' => $region->id, 'countrycode' => $region->countrycode, 'regionname' => jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$region->id, $region->regionname, false, false));
				} else {
	...
	public function get_country_regions($country_code = '')
	{
		...
		$query = "SELECT `id`, `regionname` FROM #__castor_regions WHERE `countrycode` = '$country_code' ORDER BY `regionname`";
		$result = doSelectSql($query);
		
		if (!empty($result)) {
			foreach ($result as $region) {
				if ($this->region_names_are_translatable) {
					$this->country_regions[ $country_code ][ $region->id ] = jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$region->id, $region->regionname, false, false);
				} else {
	...
	public function get_region($id = 0)
	{
		...
		if (isset($this->regions[ $id ])) {
			...
			$query = "SELECT `id`, `countrycode`, `regionname` FROM #__castor_regions WHERE `id` = ".(int)$id;
			$result = doSelectSql($query);
			
			if (!empty($result)) {
				foreach ($result as $region) {
					if ($this->region_names_are_translatable) {
						$this->regions[ $id ] = array('countrycode' => $region->countrycode, 'regionname' => jr_gettext('_CASTOR_CUSTOMTEXT_REGIONS_'.$region->id, $region->regionname, false, false));
					} else {
	...
	{
		$region_name = castor_cmsspecific_stringURLSafe($region_name);
		...
		foreach ($this->regions as $r) {
			if (strcasecmp(castor_cmsspecific_stringURLSafe($r[ 'regionname' ]), $region_name) == 0) {
				return (int) $r[ 'id' ];
			}
		}
	}
