...
// ################################################################
	 ...
	 *
	 * Any previously found rates are skipped, so multiple instances of Castor (e.g. shortcodes constantly calling the same property) will not cause multiple queries to be run
	 *
	 */
...
class basic_rate_details
{
	...
	public function get_rates_multi($property_uids = array())
	{
		...
		$query = "SELECT
				...
				WHERE 
					a.property_uid IN (".castor_implode($property_uids).") ";
		...
		foreach ($result as $r) {
			if (!is_null($r->tarifftype_id)) {
				$tariff_title_string = jr_gettext('_CASTOR_CUSTOMTEXT_TARIFF_TITLE_TARIFFTYPE_ID'.$r->tarifftype_id, stripslashes($r->name), false);
			} else {
				$tariff_title_string = jr_gettext('_CASTOR_CUSTOMTEXT_TARIFF_TITLE_ID'.$r->rates_uid, stripslashes($r->rate_title), false);
			}
