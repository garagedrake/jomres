...
class castor_property_payment_methods
{
	...
	public function get_gateways_multi($property_uids)
	{
		...
		$noimage = $castor_media_centre_images->multi_query_images["noimage-medium"];
		...
		if (!empty($property_uids)) {
			$query = "SELECT id,prid,plugin FROM #__castor_pluginsettings WHERE setting = 'active' AND value = '1' AND prid IN (".castor_implode($property_uids).') ';
			...
			foreach ($propertyData as $data) {
				...
				if (!is_null($result)) {
					...
					if ($data->plugin == 'stripe_standard') {
						$image = CASTOR_IMAGES_RELPATH.'j00510'.$data->plugin.'.png';
					} else {
