...
class minicomponent_registry
{
	...
	public function __construct()
	{
		...
		$this->temp_directory = CASTOR_TEMP_ABSPATH;
		$this->registry_file = CASTOR_TEMP_ABSPATH.'registry.php';
	...
	public function regenerate_registry($force_reload_allowed = false)
	{
		...
		if (!defined('AUTO_UPGRADE')) {
			...
			$castor_shortcode_parser->build_shortcodes($force = true);
		}
		...
		if ($this->original_filesize != $this->new_filesize && $force_reload_allowed) {
			if (!defined('AUTO_UPGRADE')) { // We don't want to do this if the installer is running this script
				echo "<script>	castorJquery.blockUI({ 
...
'
	...
	{
		$jrePath = _CASTOR_DETECTED_CMS_SPECIFIC_FILES;
		$d = @dir($jrePath);
		if ($d) {
			while (false !== ($entry = $d->read())) {
				$filename = $entry;
