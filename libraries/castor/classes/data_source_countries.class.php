...
class data_source_countries extends castor_data_source_base
{
	public function build_data_cache_file()
	{
		...
		$query = "SELECT  `constant` , `customtext` , `language` FROM #__castor_custom_text WHERE `constant` LIKE '_CASTOR_CUSTOMTEXT_COUNTRIES_%'";
		...
		$query = "SELECT `id`,`countrycode`,`countryname` FROM #__castor_countries WHERE `countrycode` IN (SELECT DISTINCT `property_country` FROM #__castor_propertys WHERE published = 1) ORDER BY `countryname`";
			...
			foreach ($result as $country) {
				foreach ($this->cms_languages as $lang) {
					if (isset($custom_country_names[$lang])) {
						$const = "_CASTOR_CUSTOMTEXT_COUNTRIES_".$country->id;
