	...
	class castorHTML
	{
		...
		public static function makeOption($val, $text = '', $value_name = 'value', $text_name = 'text')
		{
			...
			document.addEventListener(\'DOMContentLoaded\', function(){
				castorJquery(\'div.btn-group[data-toggle-name="*"]\').each(function(){
					var group   = castorJquery(this);
					var form	= group.parents(\'form\').eq(0);
					var name	= group.attr(\'data-toggle-name\');
					var hidden  = castorJquery(\'input[name="\' + name + \'"]\', form);
					castorJquery(\'button\', group).each(function(){

						var button = castorJquery(this);
						button.live(\'click\', function(){
							hidden.val(castorJquery(this).val());
					});
		...
		{
			$not_for_these_dropdowns = array('castorlang', 'castor_editing_mode', 'user_timezone', 'existingCustomers', 'management_process', 'all_guests', 'force[]', 'guestnumber', 'country', 'region', 'town', 'show_all', 'deposit_paid', 'booked_in', 'fixedPeriod_periodsRequested');
