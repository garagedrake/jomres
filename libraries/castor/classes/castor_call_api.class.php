...
// This allows Castor to call it's own API. As more functionality is added to the API then there are times that that will be the choice of connection.
	...
	class castor_call_api
	{
		...
		public function __construct($user_id = 'system', $token = '')
		{
			...
			$this->server = get_showtime('live_site').'/'.CASTOR_ROOT_DIRECTORY.'/api/';
			$this->init();
		}
		...
		public function init_manager()
		{
			...
			$query = "DELETE FROM #__castor_oauth_clients WHERE user_id = 99999999999999999999 OR user_id = 4294967295";
			doInsertSql($query);


			$query = "SELECT client_id,scope FROM #__castor_oauth_clients WHERE client_id = '".$this->user->username."' LIMIT 1";
			...
			$query = "SELECT client_secret FROM #__castor_oauth_clients WHERE client_id = '".$this->user->username."' LIMIT 1";
		...
		public function send_request($method = '', $endpoint = '', $data = array(), $headers = array())
		{
			if ($this->token != '') {
				...
				if ($response['response_code'] == '200' || $response['response_code'] == '204') {
					$can_decode = json_decode($response['response']);
					if ($can_decode != false && $can_decode != null) {
		...
		private function query_api($method = 'GET', $endpoint = '', $data = array(), $headers = array())
		{
			...
			// castor_call_api is used by the system to call itself and use rest api features
			...
			logging::log_message("Sending ".$method." to ".$this->server.$endpoint." returned response code ".$response_code, 'castor_call_api', 'DEBUG', serialize($result));
			//logging::log_message("Response code : ".$response_code , 'castor_call_api', 'DEBUG' , $headerSent );
			//logging::log_message("Response ".$result , 'castor_call_api', 'DEBUG' , $headerSent );
			// logging::log_message("Status ".json_encode($status) , 'castor_call_api', 'DEBUG' , $headerSent );
