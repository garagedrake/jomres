...
/**
 * Creates the Castor currency format object.
#
...
class castor_currency_format
{
	...
	public function get_currency_format_dropdowninput()
	{
		...
		foreach ($this->currency_formats as $key => $format) {
			$fmts[ ] = castorHTML::makeOption($key, $format);
		}
		$dropdown = castorHTML::selectList($fmts, 'cfg_cformat', '', 'value', 'text', $mrConfig[ 'cformat' ]);
