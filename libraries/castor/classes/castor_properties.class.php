...
class castor_properties
{
	...
	public function get_all_properties()
	{
		...
		$query = 'SELECT `propertys_uid`, `published` FROM #__castor_propertys ORDER BY propertys_uid ASC';
		$result = doSelectSql($query);

		$numberOfPropertiesInSystem = count($result);
		
		if ($numberOfPropertiesInSystem == 0) {
			echo 'Error, no properties installed. Before you can use Castor you need to have at least 1 property installed.</a>.';
			die();
		}
	...
	public function commit_new_property()
	{
		...
		$castor_property_types->get_property_type($this->ptype_id);
		...
		// 4 - real estate
		$realestate = false;
		if ($castor_property_types->property_type['mrp_srp_flag'] == 4) {
			$realestate = true;
		}
		...
		$castor_room_types->get_xrefs();
		
		$default_room_type = 0;
		if (isset($castor_room_types->all_ptype_rtype_xrefs[$this->ptype_id][0])) {
			$default_room_type = (int)$castor_room_types->all_ptype_rtype_xrefs[$this->ptype_id][0];
		}

		//insert new property details
		$query = "INSERT INTO #__castor_propertys 
							(
							`property_name`,
							`property_email`,
							`property_country`,
		...
		$this->propertys_uid = doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_INSERT_PROPERTY', '_CASTOR_MR_AUDIT_INSERT_PROPERTY', false));
		...
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_NAME', $this->property_name, true, $this->propertys_uid);
		...
		if (!$realestate) {
			...
			$query = "INSERT INTO #__castor_settings 
			(
			`property_uid`,
			`akey`,
			`value`
		...
		VALUES 
			...
			if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_EDIT_PROPERTY_SETTINGS', '_CASTOR_MR_AUDIT_EDIT_PROPERTY_SETTINGS', false))) {
			...
			if ($mrp_srp_flag == '0') {
				...
				if ($default_room_type > 0) {
					$query = 'INSERT INTO #__castor_rooms 
										(
										`room_classes_uid`,
										`propertys_uid`,
										`max_people`
		...
		if (!$thisJRUser->userIsManager) {
			...
			$castor_users->cms_user_id = (int)$thisJRUser->id;
			$castor_users->access_level = 70;
			$castor_users->currentproperty = (int)$this->propertys_uid;
			$castor_users->apikey = createNewAPIKey();
	...
	public function commit_update_property()
	{
		...
		$query = 'SELECT `apikey` FROM #__castor_propertys WHERE `propertys_uid` = '.(int) $this->propertys_uid;
		...
		if (!empty($this->property_features)) {
			$property_features = ','.castor_implode($this->property_features).',';
		} else {
			$property_features = '';
		}

		//update property
		$query = "UPDATE #__castor_propertys SET
		...
		if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_UPDATE_PROPERTY', '_CASTOR_MR_AUDIT_UPDATE_PROPERTY', false))) {
			throw new Exception('Error: Property update intert failed.');
		}

		//update custom text
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_NAME_'.$this->propertys_uid, $this->property_name, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_STREET_'.$this->propertys_uid, $this->property_street, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_TOWN_'.$this->propertys_uid, $this->property_town, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DESCRIPTION_'.$this->propertys_uid, $this->property_description, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_CHECKINTIMES_'.$this->propertys_uid, $this->property_checkin_times, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_AREAACTIVITIES_'.$this->propertys_uid, $this->property_area_activities, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DIRECTIONS_'.$this->propertys_uid, $this->property_driving_directions, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_AIRPORTS_'.$this->propertys_uid, $this->property_airports, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_OTHERTRANSPORT_'.$this->propertys_uid, $this->property_othertransport, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DISCLAIMERS_'.$this->propertys_uid, $this->property_policies_disclaimers, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METATITLE_'.$this->propertys_uid, $this->metatitle, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METADESCRIPTION_'.$this->propertys_uid, $this->metadescription, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METAKEYWORDS_'.$this->propertys_uid, $this->metakeywords, true, $this->propertys_uid);
		...
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_NAME', $this->property_name, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_STREET', $this->property_street, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_TOWN', $this->property_town, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DESCRIPTION', $this->property_description, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_CHECKINTIMES', $this->property_checkin_times, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_AREAACTIVITIES', $this->property_area_activities, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DIRECTIONS', $this->property_driving_directions, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_AIRPORTS', $this->property_airports, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_OTHERTRANSPORT', $this->property_othertransport, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOMTYPE_DISCLAIMERS', $this->property_policies_disclaimers, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METATITLE', $this->metatitle, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METADESCRIPTION', $this->metadescription, true, $this->propertys_uid);
		updateCustomText('_CASTOR_CUSTOMTEXT_PROPERTY_METAKEYWORDS', $this->metakeywords, true, $this->propertys_uid);
	...
	private function check_mrp_srp_flags_match($ptype_id1 = 0, $ptype_id2 = 0)
	{
		...
		if (isset($castor_property_types->property_types[$ptype_id1]['mrp_srp_flag'])) {
			$mrp_srp_1 = $castor_property_types->property_types[$ptype_id1]['mrp_srp_flag'];
		} else {
			throw new Exception('Error: Property type id 1 doesn`t exist.');
		}

		if (isset($castor_property_types->property_types[$ptype_id2]['mrp_srp_flag'])) {
			$mrp_srp_2 = $castor_property_types->property_types[$ptype_id2]['mrp_srp_flag'];
		} else {
	...
	public function delete_rooms_tariffs_settings($delete_rooms = true, $delete_tariffs = true, $delete_settings = true)
	{
		...
		//delete rooms
		if ($delete_rooms) {
			$query = 'DELETE FROM #__castor_rooms WHERE `propertys_uid` = '.(int) $this->propertys_uid.' LIMIT 10000';
			if (!doInsertSql($query, '')) {
				throw new Exception('Error: Could not delete from rooms table.');
			}

			//delete room features
			$query = 'DELETE FROM #__castor_room_features WHERE `property_uid` = '.(int) $this->propertys_uid.' LIMIT 10000';
			if (!doInsertSql($query, '')) {
		...
		//delete tariffs
		if ($delete_tariffs) {
			$query = 'DELETE FROM #__castor_rates WHERE `property_uid` = '.(int) $this->propertys_uid.' LIMIT 10000';
			if (!doInsertSql($query, '')) {
		...
		if ($delete_settings) {
			$query = 'DELETE FROM #__castor_settings WHERE `property_uid` = '.(int) $this->propertys_uid.' LIMIT 10000';
			if (!doInsertSql($query, '')) {
	...
	private function update_property_settings()
	{
		...
		// 3 - tours
		// 4 - real estate
		switch ($castor_property_types->property_type['mrp_srp_flag']) {
			case 1:
				$singleRoomProperty = 1;
	...
	public function approve_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `approved` = 1 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
	...
	public function unapprove_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `approved` = 0 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
	...
	public function publish_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `published` = 1 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
	...
	public function unpublish_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `published` = 0 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
	...
	private function complete_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `completed` = 1 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
	...
	private function incomplete_property()
	{
		...
		$query = "UPDATE #__castor_propertys SET `completed` = 0 WHERE `propertys_uid` = ".(int)$this->propertys_uid;
