 ...
 *
 * Castor 10.7 Added the ability to have override scripts which can control which tasks are available based on the property type.
 *
...
class castor_access_control
{
    ...
    {
        $query = 'SELECT `id`, `scriptname`, `access_level` FROM #__castor_access_control';
    ...
    private function delete_controlled_task($task = '')
    {
        ...
        $query = "DELETE FROM #__castor_access_control WHERE `scriptname` = '".$task."'";
    ...
    public function update_task_access_level($task = '', $access_level = '-1')
    {
        ...
        if (isset($this->controlled[$task])) {
            ...
            if ($access_level == '-1') {
                ...
                $query = "UPDATE #__castor_access_control SET `access_level` = '".$access_level."' WHERE `scriptname` = '".$task."'";
            ...
            $query = "INSERT INTO #__castor_access_control (`scriptname`, `access_level`) VALUES ('".$task."', '".$access_level."')";
    ...
    public function generate_access_control_dropdown($task = '', $min_access_level = 0)
    {
        ...
        if (!$this->options) {
            $this->options = array();
            $this->options['-1'] = castorHTML::makeOption('-1', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_DEFAULT', '_CASTOR_ACCESS_CONTROL_LEVELS_DEFAULT'));
            $this->options['0'] = castorHTML::makeOption('0', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_ANYBODY', '_CASTOR_ACCESS_CONTROL_LEVELS_ANYBODY'));
            $this->options['1'] = castorHTML::makeOption('1', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_REGISTERED', '_CASTOR_ACCESS_CONTROL_LEVELS_REGISTERED'));
            $this->options['50'] = castorHTML::makeOption('50', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_RECEPTIONIST', '_CASTOR_ACCESS_CONTROL_LEVELS_RECEPTIONIST'));
            $this->options['70'] = castorHTML::makeOption('70', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_MANAGER', '_CASTOR_ACCESS_CONTROL_LEVELS_MANAGER'));
            $this->options['90'] = castorHTML::makeOption('90', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_SUPERMANAGER', '_CASTOR_ACCESS_CONTROL_LEVELS_SUPERMANAGER'));
            $this->options['-2'] = castorHTML::makeOption('-2', jr_gettext('_CASTOR_ACCESS_CONTROL_LEVELS_NOBODY', '_CASTOR_ACCESS_CONTROL_LEVELS_NOBODY'));
        }
        ...
        return castorHTML::selectList($options, '', ' autocomplete="off" '.$javascript.'', 'value', 'text', $current_level);
    }
