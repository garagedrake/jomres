...
//Originally based on Xoriant Reviews by Manish Khanchandani http://www.phpclasses.org/browse/package/5542.html, it's been hugely rewritten for use in Castor.
...
class castor_reviews
{
	...
	public function this_user_can_review_this_property()
	{
		...
		if ($jrConfig[ 'only_guests_can_review' ] == '1') {
			$sql = 'SELECT count(guests_uid) FROM #__castor_guests WHERE property_uid = '.(int) $this->property_uid.' and mos_userid = '.(int) $this->userid.' ';
		...
		$sql = 'SELECT count(rating_id) as cnt1 FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid." and rating_ip = '".$this->ip."' ";
		$sql .= 'UNION SELECT count(rating_id) as cnt2 FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid.' and user_id = '.(int) $this->userid.' ';
	...
	public function save_confirmation($rating_id, $state)
	{
		...
		if ($property_uid && $rating_id > 0) {
			$query = 'INSERT INTO #__castor_reviews_ratings_confirm SET
	...
	public function save_review($rating, $title, $description, $pros, $cons, $user_name)
	{
		...
		if (isset($_POST[ 'contract_uid' ])  && (int)$_POST[ 'contract_uid' ] > 0 && !$test_mode) {
			...
			$query = 'SELECT contract_uid FROM #__castor_contracts WHERE guest_uid IN ('.castor_implode($allGuestUids).') AND cancelled = 0 ORDER BY tag';
		...
		$query = "INSERT INTO #__castor_reviews_ratings SET
	...
	{
		$query = "INSERT INTO #__castor_reviews_reports SET
			`rating_id`='" .(int) $rating_id."',
			`user_id`=" .(int) $this->userid.",
	...
	{
		if ((int) $rating_id > 0) {
			$query = 'DELETE FROM #__castor_reviews_ratings WHERE `rating_id`='.(int) $rating_id.'';
			$result = doInsertSql($query, '');
			if ($result) {
				$query = 'DELETE FROM #__castor_reviews_ratings_confirm WHERE `rating_id`='.(int) $rating_id.'';
				$result = doInsertSql($query, '');
				if ($result) {
					$all_reports = $this->get_all_reports_index_by_rating_id();
					if (isset($all_reports[ $rating_id ])) {
						$query = 'DELETE FROM #__castor_reviews_reports WHERE `rating_id`='.(int) $rating_id.'';
						$result = doInsertSql($query, '');
					}
				}
				$query = 'DELETE FROM #__castor_reviews_ratings_detail WHERE `rating_id`='.(int) $rating_id.'';
				$result = doInsertSql($query, '');
				if ($result) {
					$webhook_notification							   	= new stdClass();
	...
	{
		$query = 'UPDATE #__castor_reviews_ratings SET published = 1 WHERE rating_id = '.$rating_id;
		$result = doInsertSql($query, '');
		if ($result) {
			$webhook_notification								= new stdClass();
	...
	{
		$query = 'UPDATE #__castor_reviews_ratings SET published = 0 WHERE rating_id = '.$rating_id;
		$result = doInsertSql($query, '');
		if ($result) {
			$webhook_notification							   	= new stdClass();
	...
	{
		$reviews = array();
		$query = 'SELECT * FROM #__castor_reviews_ratings ORDER BY rating_date';
	...
	{
		$sql = 'SELECT count(*) FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid." and rating_ip = '".$this->ip."'";
	...
	{
		$sql = 'SELECT count(*) as cnt FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid.' and user_id = '.(int) $this->userid.' ';
		$result = doSelectSql($sql, 1);
	...
	{
		$sql = 'SELECT count(*) as cnt FROM #__castor_reviews_ratings_confirm WHERE item_id = '.(int) $this->property_uid." and confirm_ip = '".$this->ip."' and rating_id = '".(int) $rating_id."'";
		$result = doSelectSql($sql, 1);
	...
	{
		$sql = 'SELECT count(*) as cnt FROM #__castor_reviews_ratings_confirm WHERE item_id = '.(int) $this->property_uid.' and confirm_user_id = '.$this->userid.' and rating_id = '.(int) $rating_id.' ';
		$result = doSelectSql($sql, 1);
	...
	public function getRatingsMulti($property_uids)
	{
		...
		$sql = 'SELECT item_id, AVG(rating) as avg_rating, COUNT(rating) as counter, SUM(rating) as sumrating FROM #__castor_reviews_ratings WHERE item_id IN ('.castor_implode($property_uids).') AND published = 1 GROUP BY item_id';
		$result = doSelectSql($sql);
		$arr = array();
		if (!empty($result)) {
			foreach ($result as $res) {
	...
	public function showRating($item_id)
	{
		if (!isset($this->multi_query_result[ $item_id ])) {
			$arr = array();
			$sql = 'SELECT item_id, AVG(rating) as avg_rating, COUNT(rating) as counter, SUM(rating) as sumrating FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid.' AND published = 1 GROUP BY item_id';
			$result = doSelectSql($sql);
			if (!empty($result)) {
				foreach ($result as $res) {
	...
	public function showReviews($item_id, $max = -1, $pageNum = 0)
	{
		...
		$sql = 'SELECT item_id, rating_id, AVG(confirm) as avg_rating, COUNT(confirm) as counter, SUM(confirm) as total FROM #__castor_reviews_ratings_confirm WHERE item_id = '.(int) $this->property_uid.' GROUP BY rating_id';
		$rs = doSelectSql($sql);
		foreach ($rs as $r) {
		...
		$sql = 'SELECT rating_id, detail_rating FROM #__castor_reviews_ratings_detail WHERE `item_id` = '.(int) $this->property_uid.' ';
		$rs = doSelectSql($sql);
		foreach ($rs as $r) {
		...
		if ($max == '-1') {
			$sql = 'SELECT * FROM #__castor_reviews_ratings WHERE item_id = '.(int) $this->property_uid.' AND published = 1 order by rating_date desc';
			$rs = doSelectSql($sql);
	...
	public function showConfirm($rating_id)
	{
		...
		$sql = 'SELECT confirm,confirm_ip FROM #__castor_reviews_ratings_confirm WHERE item_id = '.(int) $this->property_uid.' and rating_id = '.(int) $rating_id.' ';
		$result = doSelectSql($sql);
		foreach ($result as $res) {
	...
	{
		$query = 'SELECT item_id FROM #__castor_reviews_ratings WHERE `rating_id` = '.(int) $rating_id.' LIMIT 1';
	...
	{
		$reports = array();
		$query = 'SELECT * FROM #__castor_reviews_reports';
		$result = doSelectSql($query);
		if (!empty($result)) {
			foreach ($result as $res) {
	...
	{
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.', `rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_1.'';
		$result = doInsertSql($query, '');
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.',`rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_2.'';
		$result = doInsertSql($query, '');
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.',`rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_3.'';
		$result = doInsertSql($query, '');
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.',`rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_4.'';
		$result = doInsertSql($query, '');
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.',`rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_5.'';
		$result = doInsertSql($query, '');
		$query = 'INSERT INTO #__castor_reviews_ratings_detail SET `item_id`='.(int) $property_uid.',`rating_id` = '.(int) $rating_id.',`detail_rating`='.(int) $rating_6.'';
		$result = doInsertSql($query, '');
	}
	...
	public function get_review_replies_for_review_ids($review_ids = array())
	{
		...
		$query = "SELECT `rating_id` , `replier_id` ,`replier_name` ,`reply`  FROM `#__castor_reviews_ratings_replies` WHERE `rating_id` IN ".$txt;
		$result = doSelectSql($query);
	...
	public function save_review_reply($user_id = 0, $reply = '', $rating_id = 0)
	{
		...
		$query = "SELECT enc_firstname , enc_surname , cms_user_id FROM #__castor_guest_profile WHERE cms_user_id = ".(int)$user_id;
		...
		if (!empty($replier_details)) {
			foreach ($replier_details as $guest) {
				$manager_name = $this->castor_encryption->decrypt($guest->enc_firstname)." ".$this->castor_encryption->decrypt($guest->enc_surname);
			}
		} else {
		...
		$query = "INSERT INTO `#__castor_reviews_ratings_replies` 
			(
			`rating_id` , 
			`replier_id` ,
			`replier_name` ,
	...
	public function generate_review_rating_text($average_rating = 0)
	{
		if ($average_rating > 9.8) {
			return jr_gettext('CASTOR_REVIEW_SCORE_TOP_1', 'CASTOR_REVIEW_SCORE_TOP_1', false);
		}
		if ($average_rating > 9.5) {
			return jr_gettext('CASTOR_REVIEW_SCORE_TOP_2', 'CASTOR_REVIEW_SCORE_TOP_2', false);
		}
		if ($average_rating > 9.0) {
			return jr_gettext('CASTOR_REVIEW_SCORE_TOP_3', 'CASTOR_REVIEW_SCORE_TOP_3', false);
		}
		if ($average_rating > 8.5) {
			return jr_gettext('CASTOR_REVIEW_SCORE_TOP_4', 'CASTOR_REVIEW_SCORE_TOP_4', false);
		}
		if ($average_rating > 8.0) {
			return jr_gettext('CASTOR_REVIEW_SCORE_TOP_5', 'CASTOR_REVIEW_SCORE_TOP_5', false);
		}
		if ($average_rating > 7) {
			return jr_gettext('CASTOR_REVIEW_SCORE_VERY_GOOD', 'CASTOR_REVIEW_SCORE_VERY_GOOD', false);
		}
		if ($average_rating > 6) {
			return jr_gettext('CASTOR_REVIEW_SCORE_GOOD', 'CASTOR_REVIEW_SCORE_GOOD', false);
		}
	}

	// public function get_highest_rated_and_ratings($limit)
	// {
	// $query = "SELECT * FROM #__castor_reviews_ratings ORDER BY item_id";
	// $result = doSelectSql($query);
	// if (count($result)>0)
	// {
	// foreach ($result as $res)
