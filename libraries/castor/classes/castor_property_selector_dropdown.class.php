...
// ################################################################
	...
	class castor_property_selector_dropdown
	{
		...
		public function get_dropdown($truncated_names = false)
		{
			...
			foreach ($properties as $key => $val) {
				...
				$options[] = castorHTML::makeOption($url, $val.' ('.$key.')');
			}

			$javascript = 'onchange="window.location=this.value;"';
			$selected_property_url = str_replace ( "?%3F" , "?" , $selected_property_url );
			return castorHTML::selectList($options, 'switch_property', ' autocomplete="off" '.$javascript.'', 'value', 'text', $selected_property_url, false);
		}
	}
