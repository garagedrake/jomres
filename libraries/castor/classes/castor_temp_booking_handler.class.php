...
// ################################################################
	...
	class castor_temp_booking_handler
	{
		/**
		 *
		 *
		...
		public function __construct()
		{
			...
			// Castor asynchronous tasks for triggering scheduled tasks so we will also mark it as a bot to prevent it's session from being stored
			if (isset($_SERVER['HTTP_USER_AGENT']) && $_SERVER['HTTP_USER_AGENT'] == 'Castor Asynchronous task trigger') {
			...
			$this->castorsession = '';
			$this->castorsession_db = '';
			$this->sessionfile = '';
			$this->session_directory = CASTOR_SESSIONS_ABSPATH;
			...
			if ((!isset($_COOKIE['castor_gdpr_consent_form_processed']) || (int)$_COOKIE['castor_gdpr_consent_form_processed'] == 0) && !AJAXCALL) {
				$this->ip = '0.0.0.0';
			}
		...
		public function saveCustomFields($allCustomFields = array())
		{
			...
			foreach ($allCustomFields as $f) {
				$uid = $f[ 'uid' ];
				$newBookingfieldName = $f[ 'fieldname' ].'_'.$uid;
				$data = castorGetParam($_POST, $f[ 'fieldname' ].'_'.$uid, '');
				if (!isset($this->tmpbooking[ $newBookingfieldName ])) {
					$this->addNewBookingField($newBookingfieldName);
				}
		...
		public function initBookingSession($castorsession = '')
		{
			...
			if (isset($all_headers['X-CASTOR-BOOKING-SESSION-ID'])) {
				$castorsession = filter_var($all_headers['X-CASTOR-BOOKING-SESSION-ID'], FILTER_SANITIZE_SPECIAL_CHARS);
			}

			if (isset($_REQUEST['jsid']) && $castorsession == '') { // jsid is passed by gateway services sending response codes
				$session_id = castorGetParam($_REQUEST, 'jsid', '');
			} elseif (strlen($castorsession) > 0) {
				$session_id = $castorsession;
			} else { // If the first two options don't result in the session id, we'll ask the CMS what it's session id is
				$session_id = castor_cmsspecific_getsessionid();
			}

			//we`ll check to see if session id is set and if we haven`t already loaded it..
			if (strlen($session_id) > 0 && $this->castorsession != $session_id) {
				$this->castorsession = $session_id;

				$hash = sha1($this->castorsession);
				$this->sessionfile = $this->session_directory.$hash.'.txt';

				$this->castorsession_db = substr($this->castorsession, 0, 50);

				$castor_custom_field_handler = castor_singleton_abstract::getInstance('castor_custom_field_handler');
				$castor_custom_field_handler->get_all_custom_fields();

				$this->initCustomFields($castor_custom_field_handler->custom_fields);

				$this->session_castor_start();
			}
		}
		...
		public function session_castor_start()
		{
			if ($this->session_handler == 'file') {
				$this->_castor_session_start_file();
			} else {
				$this->_castor_session_start_database();
			}
		...
		private function _castor_session_start_file()
		{
		...
		private function _castor_session_start_database()
		{
			if ($this->is_crawler) {
				return;
			}

			$query = "SELECT `data` FROM #__castor_sessions WHERE `session_id` = '".$this->castorsession_db."'";
			...
			if (!empty($result)) {
				...
				$query = "INSERT INTO #__castor_sessions (`session_id`, `data` ) VALUES ('".$this->castorsession_db."', '".$data."' )";
		...
		public function close_castor_session()
		{
			if ($this->is_crawler) {
				return;
			}

			if ($this->info != $this->_info ||
				...
				if ($this->session_handler == 'file') {
					...
					$query = "UPDATE #__castor_sessions SET `data` = '".$data."' WHERE `session_id` = '".$this->castorsession_db."'";
					if (!doInsertSql($query, '')) {
		...
		public function getCastorsession()
		{
			return $this->castorsession;
		}
