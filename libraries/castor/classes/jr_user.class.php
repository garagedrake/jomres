...
class jr_user
{
	...
	{
		//castor user role
		$this->castor_manager_id 				= 0;						//user/manager id in the _castor_managers table
		...
		if ($id == 0) {
			if (defined("CASTOR_API_CMS_ROOT")) {
				...
				$this->id = castor_cmsspecific_getcurrentusers_id();
			}
		} else {
			$this->id = $id;
		}
	...
	public function get_user_profile()
	{
		...
		if (!empty($result)) {
			foreach ($result as $r) {
				...
				$this->preferences						= $this->castor_encryption->decrypt($r->enc_preferences);
			...
			/* $query = "INSERT INTO #__castor_guest_profile (`cms_user_id`) VALUES ( '".(int) $this->id."' )";
			doInsertSql($query , '' ); */
		}
		
		$this->castor_encryption = null;
		return true;
	}
	...
	public function get_user_role()
	{
		...
		if (!empty($result)) {
			...
			foreach ($result as $r) {
				$this->castor_manager_id = $r->manager_uid;
				$this->userid = $r->userid;
				$this->accesslevel = $r->access_level;
				$this->currentproperty = $r->currentproperty;
				$this->username = castor_cmsspecific_getcurrentusers_username();
				...
				if ($this->accesslevel >= 90) { //this user is a super property manager (or even higher) and has access to all properties
					...
					$query = 'SELECT `property_uid` FROM #__castor_managers_propertys_xref WHERE `manager_id` = '.(int) $this->id;
					...
					if (empty($this->authorisedProperties) && !defined('_CASTOR_INITCHECK_ADMIN')) {
	...
	public function update_params()
	{
		...
		if ($this->profile_id == 0) {
			$query = "INSERT INTO #__castor_guest_profile (`cms_user_id`,`enc_firstname`,`enc_surname`,`enc_house`,`enc_street`,`enc_town`,`enc_county`,`enc_country`,`enc_postcode`,`enc_tel_landline`,`enc_tel_mobile`,`enc_email`,`enc_vat_number`,`vat_number_validated`) VALUES ('".(int) $this->id."','','','','','','','','','','','','',0)";
		...
		//update user params
		$query = "UPDATE #__castor_guest_profile SET `params` = '".json_encode($this->params)."' WHERE `cms_user_id` = " . (int)$this->id;
	...
	{
		$query = 'SELECT `propertys_uid` FROM #__castor_propertys WHERE `propertys_uid` = '.(int) $this->currentproperty.' LIMIT 1';
	...
	public function set_currentproperty($currentProperty)
	{
		if (in_array($currentProperty, $this->authorisedProperties)) {
			$query = 'UPDATE #__castor_managers SET `currentproperty`='.(int) $currentProperty.' WHERE `userid` = '.(int) $this->id;
	...
	public function setToAnyAuthorisedProperty()
	{
		...
		} elseif (!defined('_CASTOR_INITCHECK_ADMIN')) {
