...
class jrportal_rooms
{
	...
	public function init_room()
	{
		...
		$this->max_people = 2;					// room`s max guests
		$this->max_adults = 2;					// room`s max guests
		$this->max_children = 0;					// room`s max guests
	...
	public function commit_new_room()
	{
		...
		if (!empty($this->room_features_uid)) {
			$room_features = castor_implode($this->room_features_uid);
		} else {
			$room_features = '';
		}

		$this->max_people =  $this->max_adults + $this->max_children;
		...
		$query = "INSERT INTO #__castor_rooms 
						...
						VALUES 
							...
							'".castor_implode($this->room_features_uid)."',
							'".$this->room_name."',
							'".$this->room_number."',
							'".$this->room_floor."',
							'".(int)$this->max_people."',
		...
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOM_TAGLINE'.$this->room_uid, $this->tagline, true);
		updateCustomText('_CASTOR_CUSTOMTEXT_ROOM_DESCRIPTION_'.$this->room_uid, $this->description, true);
		...
		$castor_calculate_accommodates_value = new castor_calculate_accommodates_value($this->propertys_uid);
	...
	public function commit_update_room()
	{
		...
		if (!doInsertSql($query, jr_gettext('_CASTOR_COM_MR_VRCT_ROOM_SAVE_UPDATE', '_CASTOR_COM_MR_VRCT_ROOM_SAVE_UPDATE', false))) {
	...
	private function update_tariffs()
	{
		...
		$query = 'UPDATE #__castor_rates SET `roomclass_uid` = '.(int) $this->room_classes_uid.' WHERE `property_uid` = '.(int) $this->propertys_uid;
		if (!doInsertSql($query, jr_gettext('_CASTOR_MR_AUDIT_UPDATE_TARIFF', '_CASTOR_MR_AUDIT_UPDATE_TARIFF', false))) {
	...
	public function delete_room()
	{
		...
		if ($this->room_can_be_deleted()) {
			$query = 'DELETE FROM #__castor_rooms WHERE `room_uid` = '.(int) $this->room_uid.' AND `propertys_uid` = '.(int) $this->propertys_uid;
			if (!doInsertSql($query, jr_gettext('_CASTOR_COM_MR_ROOM_DELETED', '_CASTOR_COM_MR_ROOM_DELETED', false))) {
	...
	private function room_can_be_deleted()
	{
		...
		$query = 'SELECT `room_bookings_uid` FROM #__castor_room_bookings WHERE `room_uid` = '.(int)$this->room_uid.' AND property_uid = '.(int)$this->propertys_uid." AND DATE_FORMAT(`date`, '%Y/%m/%d') >= DATE_FORMAT('".date('Y/m/d')."', '%Y/%m/%d') LIMIT 1";
	...
	public function commit_new_rooms()
	{
		...
		if ($this->rooms_generator['delete_existing_rooms']) {
			$query = "SELECT `room_uid` FROM #__castor_rooms WHERE `propertys_uid` = ".(int) $this->rooms_generator['propertys_uid'];
			...
			$query = 'DELETE FROM #__castor_rooms WHERE `propertys_uid` = '.(int) $this->rooms_generator['propertys_uid'];
		...
		$query = 'SELECT MAX(`room_number` + 0) FROM #__castor_rooms WHERE `propertys_uid` = '.(int) $this->rooms_generator['propertys_uid'].' LIMIT 1';  // +0 converts room number from string to integer for the purpose of this query
		...
		jr_import('castor_calculate_accommodates_value');
		$castor_calculate_accommodates_value = new castor_calculate_accommodates_value($this->rooms_generator['propertys_uid']);
		$castor_calculate_accommodates_value->calculate_accommodates_value();
