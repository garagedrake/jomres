...
class basic_room_details
{
	...
	public function get_all_rooms($property_uid = 0)
	{
		...
		foreach ($result as $r) {
			...
			$this->rooms[$r->room_uid]['room_name']					= jr_gettext('_CASTOR_CUSTOMTEXT_ROOMNAME_TITLE'.$r->room_uid, stripslashes($r->room_name), false);
			...
			$this->rooms[$r->room_uid]['tagline']		 			= jr_gettext('_CASTOR_CUSTOMTEXT_ROOM_TAGLINE'.$r->room_uid, stripslashes($r->tagline), false);
			$this->rooms[$r->room_uid]['description']				= castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOM_DESCRIPTION_'.$r->room_uid, stripslashes($r->description), false));
	...
	public function get_room($room_uid = 0)
	{
		...
		foreach ($result as $r) {
			...
			$this->room['room_name']				= jr_gettext('_CASTOR_CUSTOMTEXT_ROOMNAME_TITLE'.$r->room_uid, stripslashes($r->room_name), false);
			if (is_null($r->room_number)) {
				$r->room_number = '';
			}
			...
			$this->room['tagline']					= jr_gettext('_CASTOR_CUSTOMTEXT_ROOM_TAGLINE'.$r->room_uid, stripslashes($r->tagline), false);
			if (is_null($r->description)) {
				$r->description = '';
			}
			$this->room['description']				= castor_decode(jr_gettext('_CASTOR_CUSTOMTEXT_ROOM_DESCRIPTION_'.$r->room_uid, stripslashes($r->description), false));
	...
	public function get_rooms_features($room_feature_uids = array())
	{
		...
		$query = 'SELECT 
						...
						`ptype_xref`, 
						`image` 
					FROM #__castor_room_features 
					WHERE `room_features_uid` IN ('.castor_implode($room_feature_uids).') 
					';
		$result = doSelectSql($query);
		...
		if (!empty($result)) {
			...
			foreach ($result as $r) {
				...
				$this->all_room_features[ $r->room_features_uid ][ 'feature_description' ] = jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION'.(int) $r->room_features_uid, stripslashes($r->feature_description), false, false);
				...
				//feature images
				$feature_image = $castor_media_centre_images->multi_query_images['noimage-small'];

				if ($r->image != '') {
					$feature_image = CASTOR_IMAGELOCATION_RELPATH.'rmfeatures/'.stripslashes($r->image);
				} elseif (isset($castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['small'])) {
					$feature_image = $castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['small'];
				}
				
				$this->all_room_features[ $r->room_features_uid ][ 'image' ] = $feature_image;

				if (isset($castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['small'])) {
					$this->all_room_features[ $r->room_features_uid ][ 'small' ] = $castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['small'];
					$this->all_room_features[ $r->room_features_uid ][ 'medium' ] =$castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['medium'];
					$this->all_room_features[ $r->room_features_uid ][ 'large' ] =$castor_media_centre_images->images['room_features'][ $r->room_features_uid ][0]['large'];
				}

				if ( !defined('API_STARTED') ) {
					$this->all_room_features[$r->room_features_uid]['tooltip'] = castor_makeTooltip(jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION' . (int)$r->room_features_uid, stripslashes($r->feature_description), false, false), jr_gettext('_CASTOR_CUSTOMTEXT_ROOMFEATURE_DESCRIPTION' . (int)$r->room_features_uid, stripslashes($r->feature_description), false, false), '', $feature_image, '', 'property_feature', array());
				}
			}
		}
