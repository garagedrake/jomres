...
class data_source_towns extends castor_data_source_base
{
	public function build_data_cache_file()
	{
        ...
        $query = "SELECT  `constant` , `customtext` , `language` FROM #__castor_custom_text WHERE `constant` LIKE '_CASTOR_CUSTOMTEXT_REGIONS_%'";
		...
		FROM  #__castor_custom_text
		RIGHT JOIN #__castor_propertys
		ON #__castor_custom_text.property_uid = #__castor_propertys.propertys_uid
		WHERE #__castor_propertys.published = 1 AND 
		#__castor_custom_text.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_TOWN_%'";
		$custom_town_names_result = doSelectSql($query);*/

        $query = "SELECT DISTINCT (CASE WHEN (a.propertys_uid = b.property_uid 
			    AND b.constant LIKE '_CASTOR_CUSTOMTEXT_PROPERTY_TOWN%' ) 
				THEN b.customtext 
            ...
            foreach ($result as $town) {
                foreach ($this->cms_languages as $lang) {
                    if (isset($custom_region_names[$lang])) {
                        $const = "_CASTOR_CUSTOMTEXT_REGIONS_".$town->property_region;
                        if (isset($custom_region_names[$lang][$const])) {
                            $region_name = $custom_region_names[$lang][$const];
                        } else {
                            $region_name = $region_name=jr_gettext("_CASTOR_CUSTOMTEXT_REGIONS_".$town->property_region,$castor_regions->get_region_name($town->property_region),false,false);
                        }
                    } else {
                        $region_name = $region_name=jr_gettext("_CASTOR_CUSTOMTEXT_REGIONS_".$town->property_region,$castor_regions->get_region_name($town->property_region),false,false);
                    }
