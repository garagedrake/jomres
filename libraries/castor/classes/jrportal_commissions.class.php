...
class jrportal_commissions // Functions supplied as a class so that they can be extended easily without rewriting  the functions
{
	...
	{
		$query = 'SELECT crate_id FROM #__castorportal_properties_crates_xref WHERE property_id = '.$property_uid.' LIMIT 1';
	...
	{
		$clause = 'WHERE id IN ('.castor_implode($idArray).')';
	...
	{
		$query = 'SELECT `id`,`title`,`type`,`value`,`currencycode`,`tax_rate` FROM #__castorportal_c_rates '.$clause;
	...
	{
		if ((int) $this->id > 0) {
			$query = 'SELECT `id`,`title`,`type`,`value`,`currencycode`,`tax_rate` FROM #__castorportal_c_rates WHERE `id`= '.(int) $this->id.' LIMIT 1';
			$result = doSelectSql($query);
	...
	{
		if ((int) $this->id < 1) {
			$query = "INSERT INTO #__castorportal_c_rates
				(
				`title`,
				`type`,
				`value`,
	...
	{
		if ((int) $this->id > 0) {
			$query = "UPDATE #__castorportal_c_rates SET
				`title` 			= '$this->title',
				`type` 				= ".(int) $this->type.',
	...
	public function deleteCrates($id = false)
	{
		...
		if ($id) {
			if (is_array($id)) {
				if (!empty($id)) {
					$clause = 'id IN ('.castor_implode($id).') ';
				}
			} elseif ((int) $id > 0) {
				$clause = 'id = '.(int) $id.' ';
			}
			...
			if ($clause != '') {
				$query = 'DELETE FROM #__castorportal_c_rates WHERE '.$clause;
	...
	public function makeCrateDropdown($selected = 1, $idx = null, $params = array())
	{
		...
		foreach ($this->crates as $v) {
			$options[] = castorHTML::makeOption($v[ 'id' ], $v[ 'title' ]);
		}
		...
		return castorHTML::selectList($options, "crates[$idx]", ' size="1"'.$js, 'value', 'text', $selected, false);
	}
	...
	public function makeCrateTypeDropdown($selected = 1)
	{
		...
		return castorHTML::selectList($options, 'type', '', 'value', 'text', $selected);
	}
	...
	public function assignDefaultCrate($property_uid = 0, $crate_id = 0)
	{
		...
		$query = 'INSERT INTO #__castorportal_properties_crates_xref ( `property_id`,`crate_id` ) VALUES ( '.(int) $property_uid.', '.$crate_id.' )';
		$result = doInsertSql($query, '');
	...
	public function getCurrentCrateInvoiceIdForManagerId($manager_id = 0)
	{
		...
		$query = 'SELECT id FROM #__castorportal_invoices WHERE cms_user_id = '.(int) $manager_id." AND `is_commission` = 1 AND `raised_date` <= '1970-01-01 00:00:01' ";
